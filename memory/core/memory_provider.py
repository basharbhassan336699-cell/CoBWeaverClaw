...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `cobweaverclaw backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - cobweaverclaw_home (str): The active COBWEAVERCLAW_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.cobweaverclaw``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "cobweaverclaw").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'cobweaverclaw memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], cobweaverclaw_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'cobweaverclaw memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``cobweaverclaw_home`` is the active COBWEAVERCLAW_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE COBWEAVERCLAW_HOME.

        ``cobweaverclaw backup`` only walks COBWEAVERCLAW_HOME, so any provider state kept
        ...
        of the archive, and ``cobweaverclaw import`` restores them to their original
