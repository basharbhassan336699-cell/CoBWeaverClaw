...
def memory_provider_tools_enabled(enabled_toolsets: Optional[List[str]]) -> bool:
    ...
    try:
        from memory.core._compat import resolve_toolset
...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        from memory.core._compat import _COBWEAVERCLAW_CORE_TOOLS

        _core_tool_names = set(_COBWEAVERCLAW_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, CoBWeaverClaw expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``cobweaverclaw_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_cobweaverclaw_home()`` themselves.
        """
        if "cobweaverclaw_home" not in kwargs:
            from cobweaverclaw_constants import get_cobweaverclaw_home
            kwargs["cobweaverclaw_home"] = str(get_cobweaverclaw_home())
        for provider in self._providers:
            try:
