...
```bash
cobweaverclaw memory setup    # select "holographic"
```

Or manually:
```bash
cobweaverclaw config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.cobweaverclaw-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$COBWEAVERCLAW_HOME/memory_store.db` | SQLite database path |
