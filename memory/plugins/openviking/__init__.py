...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — CoBWeaverClaw peer ID in OpenViking (default: cobweaverclaw)
...
from memory.plugins._compat import flatten_message_text
from memory.core.memory_provider import MemoryProvider
from memory.skills.skill_commands import extract_user_instruction_from_skill_message
from tools.registry import tool_error
from memory.plugins._compat import atomic_json_write, env_var_enabled

logger = logging.getLogger(__name__)

_DEFAULT_ENDPOINT = "http://127.0.0.1:1933"
_OPENVIKING_SERVICE_ENDPOINT = "https://api.vikingdb.cn-beijing.volces.com/openviking"
_DEFAULT_AGENT = "cobweaverclaw"
_AGENT_PROMPT_LABEL = "CoBWeaverClaw peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "COBWEAVERCLAW_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip CoBWeaverClaw slash-skill scaffolding before sending content to OpenViking.
...
def _load_cobweaverclaw_openviking_config() -> dict:
    try:
        from memory.plugins._compat import load_config
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = get_cobweaverclaw_home()
    except Exception:
        home = Path(os.environ.get("COBWEAVERCLAW_HOME", "")).expanduser() if os.environ.get("COBWEAVERCLAW_HOME") else Path.home() / ".cobweaverclaw"
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this CoBWeaverClaw run."
    )
...
def _save_cobweaverclaw_only_config(
    *,
    config: dict,
    provider_config: dict,
    env_path: Path,
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new CoBWeaverClaw session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in CoBWeaverClaw only", "write values only to CoBWeaverClaw .env"),
    ...
    _save_cobweaverclaw_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to CoBWeaverClaw .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See cobweaverclaw#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_cobweaverclaw_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "CoBWeaverClaw peer ID in OpenViking, sent as the actor peer and "
                    "used for peer-scoped memories"
                ),
                "default": "cobweaverclaw",
    ...
    def post_setup(self, cobweaverclaw_home: str, config: dict) -> None:
        ...
        from memory.plugins._compat import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        cobweaverclaw_home_path = Path(cobweaverclaw_home)
        env_path = cobweaverclaw_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_cobweaverclaw_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this CoBWeaverClaw run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of CoBWeaverClaw' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert CoBWeaverClaw canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See cobweaverclaw#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/cobweaverclaw/.overview.md).
