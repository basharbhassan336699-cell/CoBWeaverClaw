...
_DEFAULT_CONTAINER_TAG = "cobweaverclaw"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=cobweaverclaw"
...
def _load_supermemory_config(cobweaverclaw_home: str) -> dict:
    config = _default_config()
    config_path = Path(cobweaverclaw_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, cobweaverclaw_home: str) -> None:
    config_path = Path(cobweaverclaw_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "cobweaverclaw"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes CoBWeaverClaw writes into the "CoBWeaverClaw" Space in the Supermemory
        ...
        merged = {"sm_source": "cobweaverclaw", **(metadata or {})}
...
def _resolve_container_tag_for_setup(cobweaverclaw_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(cobweaverclaw_home)
...
def _probe_supermemory_connection(api_key: str, cobweaverclaw_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(cobweaverclaw_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(cobweaverclaw_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `cobweaverclaw memory setup`.
        # All other options are documented for $COBWEAVERCLAW_HOME/supermemory.json
    ...
    def save_config(self, values, cobweaverclaw_home):
        ...
        _save_supermemory_config(sanitized, cobweaverclaw_home)

    def get_status_config(self, provider_config: dict) -> dict:
        from cobweaverclaw_constants import get_cobweaverclaw_home

        del provider_config
        cobweaverclaw_home = str(get_cobweaverclaw_home())
        api_key = os.environ.get("SUPERMEMORY_API_KEY", "")
        status = _probe_supermemory_connection(api_key, cobweaverclaw_home)
    ...
    def post_setup(self, cobweaverclaw_home: str, config: dict) -> None:
        ...
        from memory.plugins._compat import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(cobweaverclaw_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from cobweaverclaw_constants import get_cobweaverclaw_home
        self._cobweaverclaw_home = kwargs.get("cobweaverclaw_home") or str(get_cobweaverclaw_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._cobweaverclaw_home)
