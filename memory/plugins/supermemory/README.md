...
- Supermemory API key from [app.supermemory.ai/integrations?connect=cobweaverclaw](http://app.supermemory.ai/integrations?connect=cobweaverclaw)

## Setup

```bash
cobweaverclaw memory setup    # select "supermemory"
```

Or manually:

```bash
cobweaverclaw config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.cobweaverclaw/.env
```

## Config

Config file: `$COBWEAVERCLAW_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `cobweaverclaw` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `cobweaverclaw-{identity}` → `cobweaverclaw-coder`). |
...
All Supermemory API calls send `x-sm-source: cobweaverclaw`, and document writes stamp
`metadata.sm_source: cobweaverclaw`. This is a **functional routing key, not telemetry**:
it groups CoBWeaverClaw-written memories into a dedicated "CoBWeaverClaw" Space in the
...
When enabled, CoBWeaverClaw can:
...
Use `{identity}` in the `container_tag` to scope memories per CoBWeaverClaw profile:

```json
{
  "container_tag": "cobweaverclaw-{identity}"
}
```

For a profile named `coder`, this resolves to `cobweaverclaw-coder`. The default profile resolves to `cobweaverclaw-default`. Without `{identity}`, all profiles share the same container.

## Multi-Container Mode

For advanced setups (e.g. CoBWeaverClaw-style multi-workspace), you can enable custom container tags so the agent can read/write across multiple named containers:

```json
{
  "container_tag": "cobweaverclaw",
