...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: cobweaverclaw)
...
Or via $COBWEAVERCLAW_HOME/hindsight/config.json (profile-scoped), falling back to
...
# Mirrors hindsight-integrations/cobweaverclaw — Hindsight 0.5.0 added
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so CoBWeaverClaw can degrade gracefully instead of repeatedly trying to start
...
# ---------------------------------------------------------------------------
# Hindsight API capability probe — mirrors hindsight-integrations/cobweaverclaw.
# ---------------------------------------------------------------------------
...
def _run_sync(coro, timeout: float = _DEFAULT_TIMEOUT):
    ...
    from memory.plugins._compat import safe_schedule_threadsafe
    loop = _get_loop()
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $COBWEAVERCLAW_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_cobweaverclaw_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "cobweaverclaw": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "cobweaverclaw"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this CoBWeaverClaw config."""
    profile = config.get("profile", "cobweaverclaw")
    return str(profile or "cobweaverclaw")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active CoBWeaverClaw profile name (from agent_identity)
      {workspace} — CoBWeaverClaw workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``cobweaverclaw-{user}`` with no user becomes ``cobweaverclaw``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between CoBWeaverClaw Agent and the User"
    ...
    def save_config(self, values, cobweaverclaw_home):
        """Write config to $COBWEAVERCLAW_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(cobweaverclaw_home) / "hindsight"
    ...
    def post_setup(self, cobweaverclaw_home: str, config: dict) -> None:
        ...
        from memory.plugins._compat import save_config
        from memory.plugins._compat import masked_secret_prompt

        from memory.plugins._compat import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(cobweaverclaw_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "cobweaverclaw")
        ...
        self.save_config(provider_config, cobweaverclaw_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(cobweaverclaw_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(cobweaverclaw_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "cobweaverclaw"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: cobweaverclaw-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between CoBWeaverClaw Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "cobweaverclaw"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "cobweaverclaw"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "cobweaverclaw", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "cobweaverclaw")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between CoBWeaverClaw Agent and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run CoBWeaverClaw as a non-root user, or switch "
                    "to cloud / local_external mode via 'cobweaverclaw memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # CoBWeaverClaw get sluggish. (issue #13125)
                try:
            ...
            def _start_daemon():
                import traceback
                log_dir = get_cobweaverclaw_home() / "logs"
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See cobweaverclaw#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When CoBWeaverClaw created/used that client on the shared async loop,
