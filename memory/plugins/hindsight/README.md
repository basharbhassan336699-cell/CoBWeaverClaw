...
```bash
cobweaverclaw memory setup    # select "hindsight"
```
...
```bash
cobweaverclaw config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.cobweaverclaw/.env
```
...
CoBWeaverClaw spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.cobweaverclaw/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p cobweaverclaw ui start
```
...
Config file: `~/.cobweaverclaw/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `cobweaverclaw` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `cobweaverclaw-{profile}` isolates memory per active CoBWeaverClaw profile. Empty placeholders collapse cleanly (e.g. `cobweaverclaw-{user}` with no user becomes `cobweaverclaw`). |
...
| `retain_context` | `conversation between CoBWeaverClaw Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.cobweaverclaw/.env` as `HINDSIGHT_LLM_API_KEY`.
