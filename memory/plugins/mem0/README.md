...
```bash
cobweaverclaw memory setup    # select "mem0"
```

Or manually:
```bash
cobweaverclaw config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.cobweaverclaw/.env
```

## Config

Behavioral settings live in `$COBWEAVERCLAW_HOME/mem0.json` (set them via `cobweaverclaw memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.cobweaverclaw/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `cobweaverclaw-user` | User identifier on Mem0 |
| `agent_id` | `cobweaverclaw` | Agent identifier |
...
```bash
cobweaverclaw memory setup mem0 --mode oss \
...
```bash
cobweaverclaw memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$COBWEAVERCLAW_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.cobweaverclaw/mem0_qdrant"}}
  }
}
```
...
```bash
cobweaverclaw memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
cobweaverclaw memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
ls -la ~/.cobweaverclaw/mem0_qdrant
...
- Check `user_id` matches between sessions (`$COBWEAVERCLAW_HOME/mem0.json`).
