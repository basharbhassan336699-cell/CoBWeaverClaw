...
Configuration
-------------
Secret (lives in $COBWEAVERCLAW_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $COBWEAVERCLAW_HOME/mem0.json, set via `cobweaverclaw memory
setup`):
  ...
  agent_id           — Agent identifier (default: cobweaverclaw)
...
_DEFAULT_USER_ID = "cobweaverclaw-user"
...
def _load_config() -> dict:
    """Load config from env vars, with $COBWEAVERCLAW_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "cobweaverclaw"),
        "oss": {},
    }
    ...
    config_path = get_cobweaverclaw_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, cobweaverclaw_home):
        """Write config to $COBWEAVERCLAW_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, cobweaverclaw_home: str, config: dict) -> None:
        ...
        post_setup(cobweaverclaw_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $COBWEAVERCLAW_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "cobweaverclaw")
