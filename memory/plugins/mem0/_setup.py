...
def _curses_select(title: str, items: list[tuple[str, str]], default: int = 0) -> int:
    ...
    from memory.plugins._compat import curses_radiolist
    display_items = [
...
def _save_mem0_json(cobweaverclaw_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(cobweaverclaw_home) / "mem0.json"
    existing = {}
...
def _setup_platform(cobweaverclaw_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "cobweaverclaw-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "cobweaverclaw"},
    ...
    from memory.plugins.mem0 import Mem0MemoryProvider
    provider = Mem0MemoryProvider()
    provider.save_config(provider_config, cobweaverclaw_home)

    if env_writes:
        _write_env(Path(cobweaverclaw_home) / ".env", env_writes)
...
def _setup_oss(cobweaverclaw_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(cobweaverclaw_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "cobweaverclaw-user")
    ...
    if env_writes:
        _write_env(Path(cobweaverclaw_home) / ".env", env_writes)
    _save_mem0_json(cobweaverclaw_home, {"mode": "oss", "user_id": user_id, "agent_id": "cobweaverclaw", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, cobweaverclaw_home: str) -> str:
...
_PGVECTOR_CONTAINER = "cobweaverclaw-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "cobweaverclaw"
...
def _setup_oss_interactive(cobweaverclaw_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], cobweaverclaw_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], cobweaverclaw_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'cobweaverclaw-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "cobweaverclaw-user")

    agent_id = input("  Agent ID [cobweaverclaw]: ").strip()
    agent_id = agent_id or "cobweaverclaw"
    ...
    if env_writes:
        _write_env(Path(cobweaverclaw_home) / ".env", env_writes)
    _save_mem0_json(cobweaverclaw_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(cobweaverclaw_home: str, config: dict) -> None:
    """Entry point called by cobweaverclaw memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(cobweaverclaw_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(cobweaverclaw_home, config, flags)
        return
