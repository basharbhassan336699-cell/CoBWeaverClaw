...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/cobweaverclaw>
...
```bash
cobweaverclaw memory setup honcho   # configure Honcho directly (works on a fresh install)
cobweaverclaw memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
cobweaverclaw config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.cobweaverclaw/.env
```

> `cobweaverclaw honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `cobweaverclaw memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$COBWEAVERCLAW_HOME/honcho.json` | Profile-local (isolated CoBWeaverClaw instances) |
| 2 | `~/.cobweaverclaw/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active CoBWeaverClaw profile: `cobweaverclaw` (default) or `cobweaverclaw_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `cobweaverclaw honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
**Setup — gateway identity tree.** `cobweaverclaw honcho setup` only asks about identity mapping when it detects a connected gateway platform (it inspects the gateway config; off-gateway the step is skipped because these keys do nothing without a runtime user ID). When it runs, it asks *who talks to this gateway?* and derives the keys:

- **just me** → `pinUserPeer: true`. Every non-agent gateway user collapses to `peerName`; the pin overrides all aliases, so pick this only when no user-side identity needs its own peer. Personal use where you connect CoBWeaverClaw to your own Telegram/Discord/etc. If separate agents reach the gateway and each needs a distinct peer, do **not** pin — leave `pinUserPeer: false` and map them via `userPeerAliases` (the `[e]` editor).
...
| 4 | `per-session` strategy | CoBWeaverClaw session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`cobweaverclaw`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`cobweaverclaw`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-cobweaverclaw`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening cobweaverclaw in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — CoBWeaverClaw session ID (timestamp + hex). Every `cobweaverclaw` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple CoBWeaverClaw profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:

```json
{
  "apiKey": "***",
  "workspace": "cobweaverclaw",
  "peerName": "yourname",
  "hosts": {
    "cobweaverclaw": {
      "aiPeer": "cobweaverclaw",
      "recallMode": "hybrid",
...
Both profiles see the same user (`yourname`) in the same shared environment (`cobweaverclaw`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active CoBWeaverClaw profile: `cobweaverclaw` (default) or `cobweaverclaw_<profile>` (e.g. `cobweaverclaw -p coder` -> host key `cobweaverclaw_coder`). Older `cobweaverclaw.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `COBWEAVERCLAW_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `cobweaverclaw`) |
...
|---------|-------------|
| `cobweaverclaw memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `cobweaverclaw honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `cobweaverclaw memory setup`) |
| `cobweaverclaw honcho status` | Show resolved config for active profile |
| `cobweaverclaw honcho enable` / `disable` | Toggle Honcho for active profile |
| `cobweaverclaw honcho mode <mode>` | Change recall or observation mode |
| `cobweaverclaw honcho peer --user <name>` | Update user peer name |
| `cobweaverclaw honcho peer --ai <name>` | Update AI peer name |
| `cobweaverclaw honcho tokens --context <N>` | Set context token budget |
| `cobweaverclaw honcho tokens --dialectic <N>` | Set dialectic max chars |
| `cobweaverclaw honcho map <name>` | Map current directory to a session name |
| `cobweaverclaw honcho sync` | Create host blocks for all CoBWeaverClaw profiles |
