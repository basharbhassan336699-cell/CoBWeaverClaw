...
Handles: cobweaverclaw honcho setup | status | sessions | map | peer
"""
...
from cobweaverclaw_constants import get_cobweaverclaw_home
from memory.plugins.honcho.client import _host_block, profile_host_key, resolve_active_host, resolve_config_path, HOST
from memory.plugins._compat import cfg_get
...
def _ensure_peer_exists(host_key: str | None = None) -> bool:
    ...
    """
    try:
        from memory.plugins.honcho.client import HonchoClientConfig, get_honcho_client
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "cobweaverclaw" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all CoBWeaverClaw profiles and creates host blocks for any that don't
    have one yet. Inherits settings from the default host block.
    """
    try:
        from memory.plugins._compat import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'cobweaverclaw honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'cobweaverclaw honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `cobweaverclaw update` -- no output, no exceptions.
    """
    try:
        from memory.plugins._compat import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current CoBWeaverClaw profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $COBWEAVERCLAW_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_cobweaverclaw_home() / "honcho.json"
...
def _write_config(cfg: dict, path: Path | None = None) -> None:
    ...
    from memory.plugins._compat import atomic_json_write
...
def _resolve_effective_identity_mapping(
    cfg: dict, cobweaverclaw_host: dict
    ...
    pin = False
    for val in (
        cobweaverclaw_host.get("pinUserPeer"),
        cobweaverclaw_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in cobweaverclaw_host:
        aliases_src = cobweaverclaw_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in cobweaverclaw_host:
        prefix_src = cobweaverclaw_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(cobweaverclaw_host: dict) -> None:
    ...
    for key in _IDENTITY_MAPPING_KEYS:
        cobweaverclaw_host.pop(key, None)
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom cobweaverclaw_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    cobweaverclaw_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        cobweaverclaw_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(cobweaverclaw_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = cobweaverclaw_host.get("userPeerAliases")
    prefix = cobweaverclaw_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(cobweaverclaw_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(cobweaverclaw_host)
    cobweaverclaw_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        cobweaverclaw_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        cobweaverclaw_host, current_prefix, prefix_from_root,
...
def _prompt(label: str, default: str | None = None, secret: bool = False) -> str:
    ...
    if secret:
        if sys.stdin.isatty():
            from memory.plugins._compat import masked_secret_prompt
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives CoBWeaverClaw persistent cross-session memory.")
    ...
    cobweaverclaw_host = hosts.setdefault(_host_key(), {})

    # Canonicalize any legacy pinPeerName before detection/writes.
    _migrate_pin_key(cfg)
    _migrate_pin_key(cobweaverclaw_host)
    ...
    if is_local:
        ...
        current_host_key = cobweaverclaw_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            cobweaverclaw_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        from memory.plugins.honcho.oauth import OAuthCredential
        existing_oauth = OAuthCredential.from_host_block(cobweaverclaw_host)
        ...
        if use_oauth:
            ...
            from memory.plugins.honcho.oauth_flow import authorize_via_loopback
            ...
            try:
                cred = authorize_via_loopback(
                    ...
                    source="cobweaverclaw-cli",
                    apply_config=False,
                    open_url=_open,
                )
            except Exception as e:
                print(f"  OAuth sign-in failed: {e}")
                print("  Re-run 'cobweaverclaw honcho setup' to retry, or choose an API key instead.\n")
                return
            cobweaverclaw_host["apiKey"] = cred.access_token
            cobweaverclaw_host["oauth"] = cred.oauth_block()
            # Default the peer prompt to the name entered at consent.
            if cred.consent_peer_name:
                cobweaverclaw_host["peerName"] = cred.consent_peer_name
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'cobweaverclaw honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = cobweaverclaw_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        cobweaverclaw_host["peerName"] = new_peer

    current_ai = cobweaverclaw_host.get("aiPeer") or cfg.get("aiPeer", "cobweaverclaw")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        cobweaverclaw_host["aiPeer"] = new_ai

    current_workspace = cobweaverclaw_host.get("workspace") or cfg.get("workspace", "cobweaverclaw")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        cobweaverclaw_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the CoBWeaverClaw GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, cobweaverclaw_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the CoBWeaverClaw gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = cobweaverclaw_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(cobweaverclaw_host)
            cobweaverclaw_host["pinUserPeer"] = False
            if prior_aliases:
                cobweaverclaw_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                cobweaverclaw_host, current_prefix, prefix_from_root,
            ...
            if merged:
                cobweaverclaw_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                cobweaverclaw_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                cobweaverclaw_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = cobweaverclaw_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        cobweaverclaw_host["observationMode"] = new_obs
    else:
        cobweaverclaw_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(cobweaverclaw_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        cobweaverclaw_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        cobweaverclaw_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = cobweaverclaw_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        cobweaverclaw_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = cobweaverclaw_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        cobweaverclaw_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                cobweaverclaw_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(cobweaverclaw_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            cobweaverclaw_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        cobweaverclaw_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        cobweaverclaw_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        cobweaverclaw_host["dialecticReasoningLevel"] = new_reasoning
    else:
        cobweaverclaw_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = cobweaverclaw_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        cobweaverclaw_host["sessionStrategy"] = new_strat

    cobweaverclaw_host["enabled"] = True
    cobweaverclaw_host.setdefault("saveMessages", True)
    ...
    try:
        from memory.plugins._compat import load_config, save_config
        cobweaverclaw_config = load_config()
        cobweaverclaw_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(cobweaverclaw_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: cobweaverclaw config set memory.provider honcho")

    # --- Test connection ---
    print("  Testing connection... ", end="", flush=True)
    try:
        from memory.plugins.honcho.client import HonchoClientConfig, get_honcho_client, reset_honcho_client
        reset_honcho_client()
    ...
    print("\n  Other commands:")
    print("    cobweaverclaw honcho status     -- show full config")
    print("    cobweaverclaw honcho mode       -- change recall/observation mode")
    print("    cobweaverclaw honcho tokens     -- tune context and dialectic budgets")
    print("    cobweaverclaw honcho peer       -- update peer names")
    print("    cobweaverclaw honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active CoBWeaverClaw profile name (respects --target-profile override)."""
    if _profile_override:
        return _profile_override
    try:
        from memory.plugins._compat import get_active_profile_name
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: cobweaverclaw honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            from memory.plugins.honcho.client import HonchoClientConfig
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'cobweaverclaw honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'cobweaverclaw honcho setup' to configure.\n")
...
def _show_peer_cards(hcfg, client) -> None:
    ...
    """
    try:
        from memory.plugins.honcho.session import HonchoSessionManager
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: cobweaverclaw honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        cobweaverclaw = hosts.get(_host_key(), {})
        user = cobweaverclaw.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = cobweaverclaw.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = cobweaverclaw.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = cobweaverclaw.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    CoBWeaverClaw' identity in Honcho. Seed with 'cobweaverclaw honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: cobweaverclaw honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: cobweaverclaw honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = cobweaverclaw.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = cobweaverclaw.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = cobweaverclaw.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. CoBWeaverClaw asks Honcho's AI peer a question")
        ...
        print("\n  Set with: cobweaverclaw honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'cobweaverclaw honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'cobweaverclaw honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    cobweaverclaw honcho identity --show        — show both peer representations")
        print("    cobweaverclaw honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: CoBWeaverClaw native memory → CoBWeaverClaw + Honcho."""
    from pathlib import Path

    # ── Detect CoBWeaverClaw native memory files ──────────────────────────────────
    cwd = Path(os.getcwd())
    cobweaverclaw_home = Path.home() / ".cobweaverclaw"
    ...
    for name in user_file_names:
        for d in [cwd, cobweaverclaw_home]:
            p = d / name
    ...
    print("\nHoncho migration: CoBWeaverClaw native memory → CoBWeaverClaw\n" + "─" * 50)
    print()
    print("  CoBWeaverClaw's native memory stores context in local markdown files")
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives CoBWeaverClaw persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  cobweaverclaw honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'cobweaverclaw honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'cobweaverclaw honcho setup' when ready, then re-run this walkthrough.")

    # ── Step 2: Detected files ────────────────────────────────────────────────
    print()
    print("Step 2  Detected CoBWeaverClaw memory files")
    print()
    if user_files or agent_files:
        ...
        print("  No CoBWeaverClaw native memory files found in cwd or ~/.cobweaverclaw/.")
        print("  If your files are elsewhere, copy them here before continuing,")
        print("  or seed them manually:  cobweaverclaw honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'cobweaverclaw'")
        print("  with Honcho configured and no prior session history.")
        print("  (CoBWeaverClaw calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    cobweaverclaw honcho migrate  — this step handles it interactively")
        if has_key:
            answer = _prompt("  Upload user memory files to Honcho now?", default="y")
            if answer.lower() in {"y", "yes"}:
                try:
                    from memory.plugins.honcho.client import (
                        HonchoClientConfig,
                        get_honcho_client,
                        reset_honcho_client,
                    )
            ...
            print("  Run 'cobweaverclaw honcho setup' first, then re-run this step.")
    else:
    ...
    print("  agent's character, capabilities, and behavioral rules. In CoBWeaverClaw")
    print("  these are injected via file search at prompt-build time.")
    print()
    print("  In CoBWeaverClaw, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'cobweaverclaw honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    cobweaverclaw honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  cobweaverclaw honcho identity <path/to/SOUL.md>")

    # ── Step 5: What changes ──────────────────────────────────────────────────
    print()
    print("Step 5  What changes vs. CoBWeaverClaw native memory")
    print()
    print("  Storage")
    print("    CoBWeaverClaw: markdown files on disk, searched via QMD at prompt-build time.")
    print("    CoBWeaverClaw:   cloud-backed Honcho peers. Files can stay on disk as source")
    print("              of truth; Honcho holds the live representation.")
    print()
    print("  Context injection")
    print("    CoBWeaverClaw: file excerpts injected synchronously before each LLM call.")
    print("    CoBWeaverClaw:   Honcho context fetched async at turn end, injected next turn.")
    print("              First turn has no Honcho context; subsequent turns are loaded.")
    print()
    print("  Memory growth")
    print("    CoBWeaverClaw: you edit files manually to update memory.")
    print("    CoBWeaverClaw:   Honcho observes every message and updates representations")
    ...
    print("    CoBWeaverClaw: no persistent session concept — files are global.")
    print("    CoBWeaverClaw:   per-session by default — each run gets its own session")
    print("              Map a custom name:  cobweaverclaw honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. cobweaverclaw honcho setup              — configure API key (required)")
        print("  2. cobweaverclaw honcho migrate            — re-run this walkthrough")
    else:
        print("  1. cobweaverclaw honcho status             — verify Honcho connection")
        print("  2. cobweaverclaw                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. cobweaverclaw honcho identity --show    — verify AI peer representation")
        print("  4. cobweaverclaw honcho tokens             — tune context and dialectic budgets")
        print("  5. cobweaverclaw honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'cobweaverclaw memory setup'...\n")
        from memory.plugins._compat import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``cobweaverclaw honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``cobweaverclaw honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to cobweaverclaw memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from cobweaverclaw-honcho to CoBWeaverClaw Honcho",
    )
