...
Config: Uses the existing Honcho config chain:
  1. $COBWEAVERCLAW_HOME/honcho.json (profile-scoped)
...
from memory.core.memory_manager import sanitize_context
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, cobweaverclaw_home):
        """Write config to $COBWEAVERCLAW_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(cobweaverclaw_home) / "honcho.json"
        existing = {}
    ...
    def post_setup(self, cobweaverclaw_home: str, config: dict) -> None:
        ...
        import types
        from memory.plugins.honcho.cli import cmd_setup
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            try:
                ...
                # behave as they did before. New setups via `cobweaverclaw honcho setup`
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps CoBWeaverClaw CLI/gateway startup responsive when Honcho is down,
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "cobweaverclaw-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        from memory.plugins.honcho.client import get_honcho_client
        from memory.plugins.honcho.session import HonchoSessionManager
        ...
        # Skip under per-session strategy: every CoBWeaverClaw run creates a fresh
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_cobweaverclaw_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "cobweaverclaw-default",
