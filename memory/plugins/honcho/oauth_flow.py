...
``cobweaverclaw://`` handler. Endpoints are env-overridable with local-dev defaults
...
from memory.plugins.honcho.client import resolve_active_host, resolve_config_path

logger = logging.getLogger(__name__)

# The loopback redirect registered for the CoBWeaverClaw OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "cobweaverclaw"
...
def begin_authorization(
    ...
    *,
    ...
    (``cobweaverclaw-desktop`` / ``cobweaverclaw-cli``) so the consent side can attribute
...
def complete_authorization(
    ...
    *,
    ...
    from memory.plugins.honcho.client import reset_honcho_client
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to CoBWeaverClaw.</div>"
)
...
def start_loopback_flow_background(
    *,
    config_path: Path | None = None,
    host: str | None = None,
    source: str = "cobweaverclaw-desktop",
    timeout: float = 300.0,
) -> dict[str, str]:
    ...
    # the request, where a context-local COBWEAVERCLAW_HOME override can't reach.
