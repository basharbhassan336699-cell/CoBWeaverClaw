...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with cobweaverclaw)
2. User-installed providers: ``$COBWEAVERCLAW_HOME/plugins/<name>/``
...
Usage:
    from memory.plugins._discovery import discover_memory_providers, load_memory_provider
...
_USER_NAMESPACE = "_cobweaverclaw_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_cobweaverclaw_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_cobweaverclaw_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$COBWEAVERCLAW_HOME/plugins/`` or None if unavailable."""
    try:
        from cobweaverclaw_constants import get_cobweaverclaw_home
        d = get_cobweaverclaw_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$COBWEAVERCLAW_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"memory.plugins.{name}" if _is_bundled else f"{_USER_NAMESPACE}.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"memory.plugins.{active_provider}.cli" if _is_bundled else f"{_USER_NAMESPACE}.{active_provider}.cli"
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _cobweaverclaw_user_memory.<name>.cli, usually
