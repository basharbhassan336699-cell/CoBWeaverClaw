...
```bash
cobweaverclaw memory setup    # select "byterover"
```

Or manually:
```bash
cobweaverclaw config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.cobweaverclaw/.env
```
...
Working directory: `$COBWEAVERCLAW_HOME/byterover/` (profile-scoped).
