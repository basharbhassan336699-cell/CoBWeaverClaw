...
def _frontmatter(text: str) -> dict[str, Any]:
    try:
        from memory.skills.skill_utils import parse_frontmatter
...
def _related(fm: dict[str, Any]) -> list[str]:
    raw = fm.get("related_skills") or (fm.get("metadata", {}).get("cobweaverclaw", {}) or {}).get("related_skills")
...
def _category(fm: dict[str, Any], skill_md: Path) -> str:
    cat = fm.get("category") or (fm.get("metadata", {}).get("cobweaverclaw", {}) or {}).get("category")
    if cat:
        return str(cat)
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        path = get_cobweaverclaw_home() / "skills" / ".usage.json"
        try:
...
def _memory_cards() -> list[dict[str, Any]]:
    ...
    """
    base = get_cobweaverclaw_home() / "memories"
...
def _skill_roots() -> list[tuple[str, Path]]:
    ...
    home_skills = get_cobweaverclaw_home() / "skills"
