...
from memory.core._compat import atomic_json_write
...
def _state_file() -> Path:
    return get_cobweaverclaw_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.cobweaverclaw/config.yaml. Tolerates missing file."""
    try:
        from cobweaverclaw_cli.config import load_config
        cfg = load_config()
    except Exception as e:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `cobweaverclaw update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``cobweaverclaw curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``cobweaverclaw update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``cobweaverclaw curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `cobweaverclaw curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_DRY_RUN_BANNER = (
    ...
    "`cobweaverclaw curator run` (no flag).\n"
    "\n"
...
CURATOR_REVIEW_PROMPT = (
    "You are running as CoBWeaverClaw' background skill CURATOR. This is an "
    ...
    "into ~/.cobweaverclaw/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: cobweaverclaw-config-*, cobweaverclaw-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.cobweaverclaw/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.cobweaverclaw/logs/curator/``)
    ...
    authored skill data in ``~/.cobweaverclaw/skills/``.

    ``ensure_cobweaverclaw_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_cobweaverclaw_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every cobweaverclaw install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: cobweaverclaw curator status
        keep an umbrella stable: cobweaverclaw curator pin document-tools
    ...
    lines.append("full report: cobweaverclaw curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: cobweaverclaw curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.cobweaverclaw/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.cobweaverclaw/skills/.archive/` for "
            "safety and can be restored via `cobweaverclaw curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.cobweaverclaw/skills/.archive/`. "
            "Restore any via `cobweaverclaw curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `cobweaverclaw curator restore <name>`")
    lines.append("- All archives live under `~/.cobweaverclaw/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``cobweaverclaw curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `cobweaverclaw curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `cobweaverclaw update` only if "
        ...
        # recorded in state so `cobweaverclaw curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``cobweaverclaw model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `cobweaverclaw model` → auxiliary
    ...
    try:
        from cobweaverclaw_cli.config import load_config
        from cobweaverclaw_cli.runtime_provider import resolve_runtime_provider
        _cfg = load_config()
