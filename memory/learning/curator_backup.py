...
A pre-run snapshot of ``~/.cobweaverclaw/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.cobweaverclaw/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.cobweaverclaw/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
from memory.skills.skill_utils import is_excluded_skill_path
...
def _backups_dir() -> Path:
    return get_cobweaverclaw_home() / "skills" / ".curator_backups"


def _skills_dir() -> Path:
    return get_cobweaverclaw_home() / "skills"


def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.cobweaverclaw/cron/jobs.json``)."""
    return get_cobweaverclaw_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.cobweaverclaw/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.cobweaverclaw/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.cobweaverclaw/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.cobweaverclaw/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `cobweaverclaw curator rollback --list` to see available snapshots)",
            None,
        )
