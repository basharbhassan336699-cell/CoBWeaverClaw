...
Storage
-------
Bundles live in ``~/.cobweaverclaw/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``COBWEAVERCLAW_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``cobweaverclaw bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under COBWEAVERCLAW_HOME.

    Honors ``COBWEAVERCLAW_BUNDLES_DIR`` for tests; falls back to
    ``<COBWEAVERCLAW_HOME>/skill-bundles``.
    """
    override = os.environ.get("COBWEAVERCLAW_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_cobweaverclaw_home() / "skill-bundles"
...
def build_bundle_invocation_message(
    ...
    from memory.skills.skill_commands import _load_skill_payload, _build_skill_message
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `cobweaverclaw bundles` CLI subcommand.
# ---------------------------------------------------------------------------
