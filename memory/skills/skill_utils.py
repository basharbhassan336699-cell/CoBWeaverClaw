...
from cobweaverclaw_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``COBWEAVERCLAW_KANBAN_TASK`` / ``COBWEAVERCLAW_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("COBWEAVERCLAW_KANBAN_TASK") or os.getenv("COBWEAVERCLAW_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        try:
            from cobweaverclaw_constants import is_container

            result = is_container()
        except Exception:
            result = False
    elif env == "s6":
        # The CoBWeaverClaw Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``cobweaverclaw_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``COBWEAVERCLAW_PLATFORM`` or
            ``COBWEAVERCLAW_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("COBWEAVERCLAW_PLATFORM")
        or get_session_env("COBWEAVERCLAW_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``cobweaverclaw`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.cobweaverclaw/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``cobweaverclaw`` cold-start time
    ...
    cobweaverclaw_home = get_cobweaverclaw_home()
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against COBWEAVERCLAW_HOME, not cwd
        if not p.is_absolute():
            p = (cobweaverclaw_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.cobweaverclaw/skills/`` first, then external.
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: CoBWeaverClaw can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    cobweaverclaw = metadata.get("cobweaverclaw") or {}
    if not isinstance(cobweaverclaw, dict):
        cobweaverclaw = {}
    return {
        "fallback_for_toolsets": cobweaverclaw.get("fallback_for_toolsets", []),
        "requires_toolsets": cobweaverclaw.get("requires_toolsets", []),
        "fallback_for_tools": cobweaverclaw.get("fallback_for_tools", []),
        "requires_tools": cobweaverclaw.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    cobweaverclaw = metadata.get("cobweaverclaw")
    if not isinstance(cobweaverclaw, dict):
        return []
    raw = cobweaverclaw.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes CoBWeaverClaw metadata, VCS, virtualenv/dependency, cache, and skill
