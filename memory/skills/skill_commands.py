...
from memory.skills.skill_preprocessing import (
...
#
# When a user invokes a /skill (or /bundle), CoBWeaverClaw expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_cobweaverclaw_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``COBWEAVERCLAW_PLATFORM`` env var and
    ``COBWEAVERCLAW_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("COBWEAVERCLAW_PLATFORM")
            or get_session_env("COBWEAVERCLAW_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("COBWEAVERCLAW_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        from memory.skills.skill_utils import get_external_skills_dirs
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.cobweaverclaw/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.cobweaverclaw.config``
    entries, their current values (from config.yaml or defaults) are appended
    as a ``[Skill config: ...]`` block so the agent knows the configured values
    without needing to read config.yaml itself.
    """
    try:
        from memory.skills.skill_utils import (
        ...
        lines = ["", f"[Skill config (from {display_cobweaverclaw_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.cobweaverclaw/skills/ and return a mapping of /command -> skill info.
    ...
    try:
        ...
        from memory.skills.skill_utils import get_external_skills_dirs, iter_skill_index_files
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.cobweaverclaw/skills/`` and any ``skills.external_dirs`` so the
