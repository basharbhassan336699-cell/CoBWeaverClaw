...
import type { CoBWeaverClawConfig } from "cobweaverclaw/plugin-sdk/config-contracts";
import {
...
} from "cobweaverclaw/plugin-sdk/memory-core-host-status";
import type { CoBWeaverClawPluginApi } from "cobweaverclaw/plugin-sdk/plugin-entry";
import {
  normalizeLowercaseStringOrEmpty,
  uniqueStrings,
} from "cobweaverclaw/plugin-sdk/string-coerce-runtime";
import { peekSystemEventEntries } from "cobweaverclaw/plugin-sdk/system-event-runtime";
...
type Logger = Pick<CoBWeaverClawPluginApi["logger"], "info" | "warn" | "error">;
...
export async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  config: ShortTermPromotionDreamingConfig;
  logger: Logger;
  subagent?: CoBWeaverClawPluginApi["runtime"]["subagent"];
...
export function registerShortTermPromotionDreaming(api: CoBWeaverClawPluginApi): void {
  ...
  const resolveCurrentConfig = (): CoBWeaverClawConfig =>
    (api.runtime.config?.current?.() ?? api.config) as CoBWeaverClawConfig;
  ...
  const reconcileManagedDreamingCron = async (params: {
    ...
    startupConfig?: CoBWeaverClawConfig;
