...
# SimCore: lazy import — لا يُحمَّل إلا عند الاستدعاء الفعلي
ZepGraphMemoryManager = None
def _load_zep():
    global ZepGraphMemoryManager
    if ZepGraphMemoryManager is None:
        try:
            from .zep_graph_memory_updater import ZepGraphMemoryManager as _ZGM
            ZepGraphMemoryManager = _ZGM
        except ImportError:
            pass
    return ZepGraphMemoryManager
from .simulation_ipc import SimulationIPCClient, CommandType, IPCResponse

logger = get_logger('simcore.simulation_runner')
                ...
                for line in f:
                    ...
                    if line:
                        try:
                            ...
                            if graph_updater:
                                _load_zep()
                                if ZepGraphMemoryManager: graph_updater.add_activity_from_dict(action_data, platform)
