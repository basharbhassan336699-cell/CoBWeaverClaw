...
statistics to the CoBWeaverClaw cloud for billing. Designed to be non-intrusive:
...
if TYPE_CHECKING:
    from cobweaverclaw_compress.proxy.server import CoBWeaverClawProxy

logger = logging.getLogger("cobweaverclaw_compress.telemetry.reporter")

# Grace period: if the cloud API is unreachable, use cached license for up to 7 days
GRACE_PERIOD_SECONDS = 7 * 24 * 3600  # 7 days

# Default cache location (workspace bucket, respects COBWEAVERCLAW_WORKSPACE_DIR).
...
class UsageReporter:
    ...
    usage stats to the CoBWeaverClaw cloud. If the cloud is unreachable, the proxy
    ...
    def __init__(
        self,
        license_key: str,
        cloud_url: str = "https://app.cobweaverclaw_compresslabs.ai",
        ...
        self._proxy: CoBWeaverClawProxy | None = None
    ...
    async def start(self, proxy: CoBWeaverClawProxy) -> None:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        if self._http_client is None:
            self._http_client = httpx.AsyncClient(
                ...
                headers={"User-Agent": "cobweaverclaw_compress-proxy"},
            )
