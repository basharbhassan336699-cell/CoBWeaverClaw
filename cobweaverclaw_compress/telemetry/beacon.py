"""Telemetry opt-in state for CoBWeaverClaw.

CoBWeaverClaw collects only **local**, aggregate telemetry (tokens saved, compression
ratios, timings) for the in-process collector and the ``/stats`` /
``/v1/telemetry`` endpoints. **Nothing is sent to CoBWeaverClaw Labs:** the anonymous
...
via ``COBWEAVERCLAW_OTEL_METRICS_*`` (a destination you control).

This module holds the ``COBWEAVERCLAW_TELEMETRY`` opt-in predicate (off by default)
...
def is_telemetry_enabled() -> bool:
    ...
    Fail-closed: only enabled when COBWEAVERCLAW_TELEMETRY is set to an explicit
    ...
    transmitted to CoBWeaverClaw Labs.
    """
    from cobweaverclaw_compress.offline import is_offline

    if is_offline():
        return False
    val = os.environ.get("COBWEAVERCLAW_TELEMETRY", "").lower().strip()
...
def is_telemetry_warn_enabled() -> bool:
    ...
    Set COBWEAVERCLAW_TELEMETRY_WARN=off to suppress startup/wrap notices.
    ...
    """
    val = os.environ.get("COBWEAVERCLAW_TELEMETRY_WARN", "on").lower().strip()
...
def format_telemetry_notice(*, prefix: str = "") -> str:
    ...
    return (
        ...
        "Disable: COBWEAVERCLAW_TELEMETRY=off or --no-telemetry"
    )
