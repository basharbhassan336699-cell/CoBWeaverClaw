...
* ``cobweaverclaw_compress_stack`` — how CoBWeaverClaw is being invoked
...
# from unbounded label explosion when clients send arbitrary X-CoBWeaverClaw-Stack
# header values.
...
def detect_install_mode(port: int) -> str:
    ...
    1. ``COBWEAVERCLAW_AGENT_TYPE`` env var set → ``wrapped`` (spawned by ``cobweaverclaw_compress wrap``).
    ...
    try:
        if os.environ.get("COBWEAVERCLAW_AGENT_TYPE"):
            return "wrapped"
...
def detect_stack(stats: dict[str, Any] | None = None) -> str:
    """Classify how CoBWeaverClaw is being invoked.

    Resolution order:

    1. ``COBWEAVERCLAW_STACK`` env var set → use that slug verbatim.
    2. ``COBWEAVERCLAW_AGENT_TYPE`` env var set → ``wrap_<agent>``.
    ...
    try:
        explicit = normalize_stack(os.environ.get("COBWEAVERCLAW_STACK"))
        if explicit:
            return explicit

        agent_type = os.environ.get("COBWEAVERCLAW_AGENT_TYPE")
        if agent_type:
