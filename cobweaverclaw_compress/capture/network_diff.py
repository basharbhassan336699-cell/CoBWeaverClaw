"""Differential network capture reporting for Claude Code vs CoBWeaverClaw.
...
dependency in the CoBWeaverClaw package.
"""
...
@dataclass(frozen=True)
class CaptureDiff:
    """Comparison result between a direct lane and a CoBWeaverClaw lane."""

    direct_count: int
    cobweaverclaw_compress_count: int
    only_direct: list[str]
    only_cobweaverclaw_compress: list[str]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "cobweaverclaw_compress_count": self.cobweaverclaw_compress_count,
            "only_direct": self.only_direct,
            "only_cobweaverclaw_compress": self.only_cobweaverclaw_compress,
            "paired": self.paired,
        }
...
def _header_delta(
    direct: dict[str, str], cobweaverclaw_compress: dict[str, str]
) -> tuple[list[str], list[str], list[str]]:
    direct_keys = {key.lower(): key for key in direct}
    cobweaverclaw_compress_keys = {key.lower(): key for key in cobweaverclaw_compress}
    only_direct = sorted(direct_keys[key] for key in set(direct_keys) - set(cobweaverclaw_compress_keys))
    only_cobweaverclaw_compress = sorted(cobweaverclaw_compress_keys[key] for key in set(cobweaverclaw_compress_keys) - set(direct_keys))
    changed: list[str] = []
    for lower in sorted(set(direct_keys) & set(cobweaverclaw_compress_keys)):
        d_key = direct_keys[lower]
        h_key = cobweaverclaw_compress_keys[lower]
        if direct[d_key] != cobweaverclaw_compress[h_key]:
            changed.append(d_key)
    return only_direct, only_cobweaverclaw_compress, changed
...
def _pair_exchanges(
    direct: list[CapturedExchange], cobweaverclaw_compress: list[CapturedExchange], *, pair_by: str = "path"
) -> tuple[list[tuple[CapturedExchange, CapturedExchange]], list[str], list[str]]:
    direct_by_key: dict[str, list[CapturedExchange]] = {}
    cobweaverclaw_compress_by_key: dict[str, list[CapturedExchange]] = {}
    for item in direct:
        key = item.route_key if pair_by == "route" else item.path_key
        direct_by_key.setdefault(key, []).append(item)
    for item in cobweaverclaw_compress:
        key = item.route_key if pair_by == "route" else item.path_key
        cobweaverclaw_compress_by_key.setdefault(key, []).append(item)

    pairs: list[tuple[CapturedExchange, CapturedExchange]] = []
    only_direct: list[str] = []
    only_cobweaverclaw_compress: list[str] = []
    for key in sorted(set(direct_by_key) | set(cobweaverclaw_compress_by_key)):
        direct_items = direct_by_key.get(key, [])
        cobweaverclaw_compress_items = cobweaverclaw_compress_by_key.get(key, [])
        shared = min(len(direct_items), len(cobweaverclaw_compress_items))
        pairs.extend(zip(direct_items[:shared], cobweaverclaw_compress_items[:shared], strict=False))
        only_direct.extend([item.route_key for item in direct_items[shared:]])
        only_cobweaverclaw_compress.extend([item.route_key for item in cobweaverclaw_compress_items[shared:]])
    return pairs, only_direct, only_cobweaverclaw_compress


def compare_captures(
    direct: list[CapturedExchange], cobweaverclaw_compress: list[CapturedExchange], *, pair_by: str = "path"
) -> CaptureDiff:
    pairs, only_direct, only_cobweaverclaw_compress = _pair_exchanges(direct, cobweaverclaw_compress, pair_by=pair_by)
    paired: list[dict[str, Any]] = []
    for direct_item, cobweaverclaw_compress_item in pairs:
        direct_paths = (
            _json_paths(direct_item.request_json) if direct_item.request_json is not None else {}
        )
        cobweaverclaw_compress_paths = (
            _json_paths(cobweaverclaw_compress_item.request_json)
            if cobweaverclaw_compress_item.request_json is not None
            else {}
        )
        only_direct_json = sorted(set(direct_paths) - set(cobweaverclaw_compress_paths))
        only_cobweaverclaw_compress_json = sorted(set(cobweaverclaw_compress_paths) - set(direct_paths))
        changed_json = sorted(
            path
            for path in set(direct_paths) & set(cobweaverclaw_compress_paths)
            if direct_paths[path] != cobweaverclaw_compress_paths[path]
        )
        headers_only_direct, headers_only_cobweaverclaw_compress, headers_changed = _header_delta(
            direct_item.request_headers, cobweaverclaw_compress_item.request_headers
        )
        paired.append(
            {
                "route": direct_item.route_key,
                "cobweaverclaw_compress_route": cobweaverclaw_compress_item.route_key,
                "direct_sequence": direct_item.sequence,
                "cobweaverclaw_compress_sequence": cobweaverclaw_compress_item.sequence,
                "status": {
                    "direct": direct_item.response_status,
                    "cobweaverclaw_compress": cobweaverclaw_compress_item.response_status,
                },
                "request_body_size": {
                    "direct": direct_item.request_body_size,
                    "cobweaverclaw_compress": cobweaverclaw_compress_item.request_body_size,
                    "delta": cobweaverclaw_compress_item.request_body_size - direct_item.request_body_size,
                },
                "request_body_sha256": {
                    "direct": direct_item.request_body_sha256,
                    "cobweaverclaw_compress": cobweaverclaw_compress_item.request_body_sha256,
                    "same": direct_item.request_body_sha256 == cobweaverclaw_compress_item.request_body_sha256,
                },
                "anthropic": {
                    "direct": _anthropic_request_summary(direct_item),
                    "cobweaverclaw_compress": _anthropic_request_summary(cobweaverclaw_compress_item),
                },
                "headers": {
                    "only_direct": headers_only_direct,
                    "only_cobweaverclaw_compress": headers_only_cobweaverclaw_compress,
                    "changed": headers_changed,
                },
                "json": {
                    "only_direct": only_direct_json,
                    "only_cobweaverclaw_compress": only_cobweaverclaw_compress_json,
    ...
    return CaptureDiff(
        ...
        cobweaverclaw_compress_count=len(cobweaverclaw_compress),
        only_direct=only_direct,
        only_cobweaverclaw_compress=only_cobweaverclaw_compress,
        paired=paired,
...
def render_markdown_report(diff: CaptureDiff) -> str:
    lines = [
        ...
        f"- CoBWeaverClaw exchanges: `{diff.cobweaverclaw_compress_count}`",
        f"- Paired exchanges: `{len(diff.paired)}`",
        f"- Only direct: `{len(diff.only_direct)}`",
        f"- Only CoBWeaverClaw: `{len(diff.only_cobweaverclaw_compress)}`",
        "",
    ]
    if diff.only_direct:
        lines.extend(["## Only Direct", "", *[f"- `{route}`" for route in diff.only_direct], ""])
    if diff.only_cobweaverclaw_compress:
        lines.extend(
            ["## Only CoBWeaverClaw", "", *[f"- `{route}`" for route in diff.only_cobweaverclaw_compress], ""]
        )
    ...
    for item in diff.paired:
        route = item["route"]
        if item.get("cobweaverclaw_compress_route") and item["cobweaverclaw_compress_route"] != route:
            route = f"{route} -> {item['cobweaverclaw_compress_route']}"
        status = f"{item['status']['direct']} -> {item['status']['cobweaverclaw_compress']}"
        sizes = item["request_body_size"]
        body = f"{sizes['direct']} -> {sizes['cobweaverclaw_compress']} ({sizes['delta']:+})"
        sha = "same" if item["request_body_sha256"]["same"] else "changed"
        headers = item["headers"]
        header_delta = (
            f"+{_list_or_dash(headers['only_cobweaverclaw_compress'])}; "
            f"-{_list_or_dash(headers['only_direct'])}; "
            f"changed={_list_or_dash(headers['changed'])}"
        )
        json_delta = item["json"]
        json_text = (
            f"+{_list_or_dash(json_delta['only_cobweaverclaw_compress'])}; "
        ...
        cobweaverclaw_compress_anthropic = anthropic.get("cobweaverclaw_compress", {})
        tool_delta = cobweaverclaw_compress_anthropic.get("tools_bytes", 0) - direct_anthropic.get(
            "tools_bytes", 0
        )
        json_text = (
            f"{json_text}; tools={direct_anthropic.get('tools_count', 0)}"
            f"->{cobweaverclaw_compress_anthropic.get('tools_count', 0)}"
            f" ({tool_delta:+} bytes)"
        )
        lines.append(
