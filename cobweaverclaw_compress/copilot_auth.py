...
from cobweaverclaw_compress import paths
from cobweaverclaw_compress._subprocess import run
from cobweaverclaw_compress.copilot_linux_secret import read_copilot_oauth_token as read_linux_secret_token
from cobweaverclaw_compress.copilot_macos_keychain import read_copilot_oauth_token as read_macos_keychain_token
...
def cobweaverclaw_compress_copilot_auth_path() -> Path:
    """Return the path where CoBWeaverClaw stores its Copilot OAuth token."""

    override = os.environ.get("COBWEAVERCLAW_COPILOT_AUTH_FILE", "").strip()
    if override:
...
def read_cobweaverclaw_compress_copilot_oauth_token() -> str | None:
    """Return CoBWeaverClaw's saved Copilot OAuth token, if one is available."""

    try:
        payload = json.loads(cobweaverclaw_compress_copilot_auth_path().read_text(encoding="utf-8"))
    except FileNotFoundError:
        return None
    except Exception as exc:
        logger.debug("Unable to read CoBWeaverClaw Copilot auth file: %s", exc)
        return None
...
def save_cobweaverclaw_compress_copilot_oauth_token(
    token: str,
    *,
    ...
    path = cobweaverclaw_compress_copilot_auth_path()
...
def iter_oauth_token_candidates() -> list[CopilotTokenCandidate]:
    ...
    cobweaverclaw_compress_copilot_token = read_cobweaverclaw_compress_copilot_oauth_token()
    if cobweaverclaw_compress_copilot_token:
        candidates.append(
            CopilotTokenCandidate(
                token=cobweaverclaw_compress_copilot_token,
                source=f"cobweaverclaw_compress-copilot-auth:{cobweaverclaw_compress_copilot_auth_path()}",
