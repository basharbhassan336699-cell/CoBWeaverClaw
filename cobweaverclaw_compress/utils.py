"""Shared utilities for CoBWeaverClaw SDK."""
...
# Marker format for CoBWeaverClaw modifications
MARKER_PREFIX = "<cobweaverclaw_compress:"
MARKER_SUFFIX = ">"
...
def create_marker(marker_type: str, **kwargs: Any) -> str:
    """
    Create a CoBWeaverClaw marker string.
...
def extract_markers(text: str) -> list[dict[str, Any]]:
    """
    Extract CoBWeaverClaw markers from text.

    Returns:
        List of dicts with marker_type and attributes.
    """
    pattern = re.compile(r"<cobweaverclaw_compress:(\w+)([^>]*)>")
    markers = []
