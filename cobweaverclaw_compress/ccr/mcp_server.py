"""CoBWeaverClaw MCP Server — Context engineering toolkit for AI coding tools.

Exposes CoBWeaverClaw's compression, retrieval, and observability as MCP tools
that any MCP-compatible host (Claude Code, Cursor, Codex, etc.) can use.

Tools:
    cobweaverclaw_compress_compress   — Compress content on demand (no proxy needed)
    cobweaverclaw_compress_retrieve   — Retrieve original uncompressed content by hash
    cobweaverclaw_compress_stats      — Session compression statistics

Usage:
    # As standalone server (stdio transport, called by AI coding tools)
    cobweaverclaw_compress mcp serve

    # Add to Claude Code
    cobweaverclaw_compress mcp install
...
CCR_TOOL_NAME = "cobweaverclaw_compress_retrieve"
COMPRESS_TOOL_NAME = "cobweaverclaw_compress_compress"
STATS_TOOL_NAME = "cobweaverclaw_compress_stats"
READ_TOOL_NAME = "cobweaverclaw_compress_read"

logger = logging.getLogger("cobweaverclaw_compress.ccr.mcp")

# Feature flag: enable cobweaverclaw_compress_read tool (file read caching via CCR)
# Set COBWEAVERCLAW_MCP_READ=on to enable
_READ_ENABLED = os.environ.get("COBWEAVERCLAW_MCP_READ", "off").lower().strip() in (
    "on",
    "true",
    "1",
    "yes",
...
DEFAULT_PROXY_URL = os.environ.get("COBWEAVERCLAW_PROXY_URL", "http://127.0.0.1:8787")
...
def _format_session_summary(
    ...
    lines: list[str] = []
    lines.append("CoBWeaverClaw Window-Scoped Session Summary")
    lines.append("=" * 40)
    ...
    without = cost.get("without_cobweaverclaw_compress_usd", 0)
    with_hr = cost.get("with_cobweaverclaw_compress_usd", 0)
    saved = cost.get("total_saved_usd", 0)
    pct = cost.get("savings_pct", 0)
    if without > 0:
        lines.append("Cost Impact:")
        lines.append(f"  Without CoBWeaverClaw:  ${without:.2f}")
        lines.append(f"  With CoBWeaverClaw:     ${with_hr:.2f}")
...
# cobweaverclaw_compress_stats aggregates across all instances within the session window.
# Respects COBWEAVERCLAW_WORKSPACE_DIR.
...
class CoBWeaverClawMCPServer:
    """MCP Server exposing CoBWeaverClaw's context engineering toolkit.

    Tools:
        cobweaverclaw_compress_compress — Compress content on demand. Stores original for
                           retrieval. Works without a proxy.
        cobweaverclaw_compress_retrieve — Retrieve original uncompressed content by hash.
                           Checks local store first, then proxy if configured.
        cobweaverclaw_compress_stats    — Session statistics: compressions, savings, cost.
    ...
    def __init__(
        ...
        self.server: Server = Server("cobweaverclaw_compress")
    ...
    def _compress_content(self, content: str) -> dict[str, Any]:
        """Compress content using CoBWeaverClaw's pipeline.

        Returns dict with compressed text, token counts, hash, etc.
        """
        from cobweaverclaw_compress.compress import compress
        ...
        # ``CompressResult`` in cobweaverclaw_compress/compress.py — "0.0 = no savings, 1.0 =
        ...
        return {
            ...
            "note": f"Original stored with hash={hash_key}. Use mcp__cobweaverclaw_compress__{CCR_TOOL_NAME} to get full content later.",
        }
    ...
    async def _retrieve_content(
        ...
        return {
            ...
            "compressed via cobweaverclaw_compress_compress is stored for the session; content "
    ...
    def _setup_handlers(self) -> None:
        ...
        async def list_tools() -> list[Tool]:
            tools = [
                Tool(
                    ...
                    description=(
                        ...
                        f"The original is stored and can be retrieved later via mcp__cobweaverclaw_compress__{CCR_TOOL_NAME}. "
                ...
                Tool(
                    ...
                    description=(
                        ...
                        "The hash comes from cobweaverclaw_compress_compress results or from compression "
            ...
            # Conditionally add cobweaverclaw_compress_read (behind feature flag)
            if _READ_ENABLED:
                tools.append(
                    Tool(
                        name=READ_TOOL_NAME,
                        description=(
                            ...
                            f"Use mcp__cobweaverclaw_compress__{CCR_TOOL_NAME} with the hash to get full content if needed. "
    ...
    async def _handle_compress(self, arguments: dict[str, Any]) -> list[TextContent]:
        """Handle cobweaverclaw_compress_compress tool call."""
        ...
        # Record durably so `cobweaverclaw_compress savings` reflects this compression across
    ...
    def _record_savings(self, result: dict[str, Any]) -> None:
        ...
        savings_ledger.record_savings_event(
            ...
            model=os.environ.get("COBWEAVERCLAW_MCP_MODEL"),
    ...
    def _current_client(self) -> str:
        ...
        override = os.environ.get("COBWEAVERCLAW_MCP_CLIENT")
        if override:
            return override
        try:
    ...
    async def _handle_retrieve(self, arguments: dict[str, Any]) -> list[TextContent]:
        """Handle cobweaverclaw_compress_retrieve tool call."""
    ...
    async def _handle_stats(self) -> list[TextContent]:
        """Handle cobweaverclaw_compress_stats tool call."""
    ...
    async def _handle_read(self, arguments: dict[str, Any]) -> list[TextContent]:
        """Handle cobweaverclaw_compress_read tool call — file read with session caching."""
        import hashlib
        ...
        # in cobweaverclaw_compress/ccr/ — use the centralized safe-log decoder so
                            ...
                            text=json.dumps(
                                {
                                    ...
                                    "note": (
                                        ...
                                        f"from the first read. Call mcp__cobweaverclaw_compress__{CCR_TOOL_NAME}(hash='{ccr_hash}') "
        ...
        ccr_hash = store.store(
            ...
            compressed_tokens=5,
            tool_name="cobweaverclaw_compress_read",
            ttl=MCP_SESSION_TTL,
        )
    ...
    async def run_stdio(self) -> None:
        ...
        async with stdio_server() as (read_stream, write_stream):
            logger.info(f"CoBWeaverClaw MCP Server starting (proxy: {self.proxy_url})")
...
) -> CoBWeaverClawMCPServer:
    """Create a CoBWeaverClaw MCP server instance.

    Args:
        proxy_url: URL of the CoBWeaverClaw proxy server (for retrieval fallback).
        direct_mode: Ignored (kept for backward compatibility).

    Returns:
        CoBWeaverClawMCPServer instance.
    """
    return CoBWeaverClawMCPServer(proxy_url=proxy_url)


async def main() -> None:
    """Run the CoBWeaverClaw MCP server."""
    parser = argparse.ArgumentParser(
        description="CoBWeaverClaw MCP Server — Context engineering toolkit"
    )
    parser.add_argument(
        "--proxy-url",
        default=DEFAULT_PROXY_URL,
        help=f"CoBWeaverClaw proxy URL for retrieval fallback (default: {DEFAULT_PROXY_URL})",
    )
    parser.add_argument(
        "--direct",
        action="store_true",
    ...
    server = CoBWeaverClawMCPServer(proxy_url=args.proxy_url)
