...
class AgentSavingsProfile:
    ...
    # ambient COBWEAVERCLAW_TARGET_RATIO / proxy default still applies). Workload
    ...
    def proxy_env(self) -> dict[str, str]:
        """Return env vars for CoBWeaverClaw proxy/wrapper entry points."""

        env = {
            "COBWEAVERCLAW_MODE": self.proxy_mode,
            "COBWEAVERCLAW_SAVINGS_PROFILE": self.name,
            "COBWEAVERCLAW_SAVINGS_TARGET": f"{self.target_savings:.2f}",
            "COBWEAVERCLAW_COMPRESS_USER_MESSAGES": ("1" if self.compress_user_messages else "0"),
            "COBWEAVERCLAW_COMPRESS_SYSTEM_MESSAGES": ("1" if self.compress_system_messages else "0"),
            "COBWEAVERCLAW_PROTECT_RECENT": str(self.protect_recent),
            "COBWEAVERCLAW_PROTECT_ANALYSIS_CONTEXT": ("1" if self.protect_analysis_context else "0"),
            "COBWEAVERCLAW_MIN_TOKENS": str(self.min_tokens_to_compress),
            "COBWEAVERCLAW_MAX_ITEMS": str(self.max_items_after_crush),
            "COBWEAVERCLAW_SMART_CRUSHER_COMPACTION": (
                "1" if self.smart_crusher_with_compaction else "0"
            ),
            "COBWEAVERCLAW_FORCE_KOMPRESS": "1" if self.force_kompress else "0",
            "COBWEAVERCLAW_ACCURACY_GUARD": self.accuracy_guard,
        }
        ...
        if self.target_ratio is not None:
            env["COBWEAVERCLAW_TARGET_RATIO"] = f"{self.target_ratio:.2f}"
        return env
...
_PROFILES: dict[str, AgentSavingsProfile] = {
    ...
    # MCP-vs-airgapped axis is the separate COBWEAVERCLAW_LOSSLESS toggle: markers
