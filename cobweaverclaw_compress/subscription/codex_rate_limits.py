...
(``x-codex-primary-used-percent`` etc.) and cobweaverclaw_compress captured those headers
...
CODEX_USAGE_URL = (
    os.environ.get("COBWEAVERCLAW_CODEX_USAGE_URL", "https://chatgpt.com/backend-api/wham/usage").strip()
...
class CodexRateLimitState(QuotaTracker):
    ...
    be registered with the :class:`~cobweaverclaw_compress.subscription.base.QuotaTrackerRegistry`.
    ...
    cobweaverclaw_compress, so :meth:`start` and :meth:`stop` are no-ops.
    """
