...
- Thread-safe state updates via threading.Lock (consistent with cobweaverclaw_compress patterns)
...
token breakdowns per window that enable:
  - CoBWeaverClaw efficiency metrics (tokens saved = raw - what proxy sent)
...
from cobweaverclaw_compress.subscription.base import QuotaTracker
from cobweaverclaw_compress.subscription.client import SubscriptionClient
from cobweaverclaw_compress.subscription.models import (
    CoBWeaverClawContribution,
    RateLimitWindow,
...
_PERSIST_FILE_ENV = _paths.COBWEAVERCLAW_SUBSCRIPTION_STATE_PATH_ENV
_DEFAULT_PERSIST_DIR = ".cobweaverclaw_compress"
...
# without uninstalling the binary or unsetting ``COBWEAVERCLAW_CONTEXT_TOOL``.
# Used for diagnostics and for environments where RTK is intentionally
# excluded from cobweaverclaw_compress accounting (e.g. shadow tests).
#
...
_RTK_WIRING_ENV = "COBWEAVERCLAW_RTK_WIRING"
...
# ``cobweaverclaw_compress/proxy/server.py``: a non-blocking ``fcntl.flock`` on
# ``COBWEAVERCLAW_RTK_POLL_LOCK`` (default under the workspace dir). Only the
...
_RTK_POLL_LOCK_ENV = "COBWEAVERCLAW_RTK_POLL_LOCK"
...
def _validate_rtk_env_at_startup() -> None:
    ...
    Raises ``ValueError`` loudly if ``COBWEAVERCLAW_RTK_WIRING`` is set to an
...
class SubscriptionTracker(QuotaTracker):
    ...
    Implements :class:`~cobweaverclaw_compress.subscription.base.QuotaTracker` so it can
    be registered with :func:`~cobweaverclaw_compress.subscription.base.get_quota_registry`
    ...
    def __init__(
        ...
        # :func:`~cobweaverclaw_compress.proxy.helpers._get_context_tool_stats`) so it
        ...
        # entire pre-CoBWeaverClaw RTK history as one fake delta. Switching to
    ...
    def update_contribution(
        ...
        *,
        ...
        """Update cobweaverclaw_compress contribution counters for the current session window.
        ...
        :func:`cobweaverclaw_compress.proxy.helpers._get_rtk_stats`) when the caller does
        ...
        reads 0 instead of the entire pre-CoBWeaverClaw RTK history.
    ...
    def _poll_rtk_delta(self) -> int:
        ...
        :func:`cobweaverclaw_compress.proxy.helpers._get_rtk_stats` and reads the
        ...
        - ``COBWEAVERCLAW_RTK_WIRING=disabled`` — operator opt-out.
        ...
        PR-G2 remediation (H1): ``COBWEAVERCLAW_RTK_WIRING`` is now validated at
        ...
        try:
            ...
            from cobweaverclaw_compress.proxy.helpers import _get_rtk_stats
        ...
        # the pre-CoBWeaverClaw RTK history. Falls back to the top-level
    ...
    def _try_acquire_rtk_poll_lock(self) -> bool:
        ...
        ``cobweaverclaw_compress/proxy/server.py`` (fcntl.flock, LOCK_EX | LOCK_NB).
        """
    ...
    def _compute_used_since_reset(self, window: RateLimitWindow | None) -> int | None:
        ...
        if now < window.resets_at:
            return None
        try:
            from cobweaverclaw_compress.subscription import session_tracking
    ...
    async def _maybe_poll(self) -> None:
        ...
        if not is_active:
            ...
            from cobweaverclaw_compress.subscription.client import read_cached_oauth_token
        ...
        try:
            from cobweaverclaw_compress.observability.metrics import get_otel_metrics
    ...
    def _maybe_reset_contribution(self, snapshot: SubscriptionSnapshot) -> None:
        ...
        if (
            ...
            logger.info("5h window rolled over; resetting cobweaverclaw_compress contribution counters")
            self._state.contribution = CoBWeaverClawContribution()
...
def configure_subscription_tracker(
    ...
    a typo (``COBWEAVERCLAW_RTK_WIRING=enabld``) crashes the proxy at startup
