...
Mirrors the Anthropic OAuth usage API response exactly, including:
  ...
  - CoBWeaverClaw contribution: tokens conserved by compression, CLI filtering, cache
...
# ---------------------------------------------------------------------------
# CoBWeaverClaw contribution estimate
# ---------------------------------------------------------------------------


@dataclass
class CoBWeaverClawContribution:
    """Tokens conserved within the current 5h window by CoBWeaverClaw's layers.
    ...
    def raw_without_cobweaverclaw_compress(self) -> int:
        return self.tokens_submitted + self.tokens_saved_compression + self.cli_filtering_saved()

    def efficiency_pct(self) -> float:
        raw = self.raw_without_cobweaverclaw_compress()
        if raw == 0:
            return 0.0
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "raw_without_cobweaverclaw_compress": self.raw_without_cobweaverclaw_compress(),
...
class SubscriptionState:
    ...
    contribution: CoBWeaverClawContribution = field(default_factory=CoBWeaverClawContribution)
