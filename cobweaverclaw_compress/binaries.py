...
`pip install cobweaverclaw_compress-ai` pulls `ast-grep-cli` as a proper PyPI binary wheel
...
Env vars:
    COBWEAVERCLAW_BINARIES_MIRROR   base URL that replaces https://github.com
    COBWEAVERCLAW_BINARIES_CACHE    override cache dir
    COBWEAVERCLAW_BINARIES_OFFLINE  if set, never reach the network
"""
...
class OfflineError(BinaryError):
    """Raised when a network fetch is required but COBWEAVERCLAW_BINARIES_OFFLINE is set."""
...
def cache_dir() -> Path:
    override = os.environ.get("COBWEAVERCLAW_BINARIES_CACHE")
    if override:
        return Path(override).expanduser().resolve()
    if sys.platform.startswith("win"):
        base = os.environ.get("LOCALAPPDATA") or str(Path.home() / "AppData" / "Local")
        return Path(base) / "cobweaverclaw_compress" / "bin"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Caches" / "cobweaverclaw_compress" / "bin"
    xdg = os.environ.get("XDG_CACHE_HOME")
    base = Path(xdg) if xdg else Path.home() / ".cache"
    return base / "cobweaverclaw_compress" / "bin"
...
def _asset_for_platform(tool: str, plat: PlatformKey) -> dict[str, Any]:
    ...
    if _is_pypi_tool(tool):
        raise PlatformNotSupported(
            f"{tool}: distributed via PyPI only; `pip install cobweaverclaw_compress-ai` "
...
def _mirror_url(url: str) -> str:
    mirror = os.environ.get("COBWEAVERCLAW_BINARIES_MIRROR")
    if not mirror:
        return url
...
def _download(url: str, dest: Path, *, progress: bool = True) -> None:
    if os.environ.get("COBWEAVERCLAW_BINARIES_OFFLINE"):
        raise OfflineError(f"offline mode (COBWEAVERCLAW_BINARIES_OFFLINE=1) but fetch required: {url}")
    ...
    req = urllib.request.Request(final_url, headers={"User-Agent": "cobweaverclaw_compress-binaries/1"})
    try:
...
def resolve(tool: str) -> Path:
    ...
    OfflineError if a fetch is required but COBWEAVERCLAW_BINARIES_OFFLINE is set,
    ...
    with tempfile.TemporaryDirectory(prefix="cobweaverclaw_compress-fetch-") as tmp:
        tmp_dir = Path(tmp)
        ...
        # `cobweaverclaw_compress proxy` starts don't race on the same .partial path.
...
def ensure_tools(quiet: bool = False) -> dict[str, Path | None]:
    ...
    Called at proxy startup and on first `cobweaverclaw_compress` CLI invocation so that no
    ...
    for name in _registry().get("tools", {}):
        try:
            ...
            out[name] = None
            if not quiet:
                print(f"cobweaverclaw_compress: skipping {name}: {e}", file=sys.stderr)
    return out
...
def status() -> list[dict[str, Any]]:
    ...
    Used by `cobweaverclaw_compress tools doctor`. Never fetches — only inspects.
    """
