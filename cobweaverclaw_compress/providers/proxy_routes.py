...
from cobweaverclaw_compress.proxy.handlers.openai import _resolve_codex_routing_headers

logger = logging.getLogger("cobweaverclaw_compress.proxy.routes")
...
async def _handle_chatgpt_codex_images(
    ...
    from cobweaverclaw_compress.proxy.helpers import _strip_internal_headers
...
def register_provider_routes(app: FastAPI, proxy: Any) -> None:
    ...
    # cobweaverclaw_compress/proxy/handlers/bedrock.py for the SigV4 caveat.
    ...
    async def passthrough(request: Request, path: str):
        custom_base = request.headers.get("x-cobweaverclaw_compress-base-url")
        if custom_base:
