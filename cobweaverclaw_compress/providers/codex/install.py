...
from cobweaverclaw_compress.install.models import ConfigScope, DeploymentManifest, ManagedMutation, ToolTarget
from cobweaverclaw_compress.install.paths import codex_config_path

from .runtime import proxy_base_url
from .threads import retag_to_cobweaverclaw_compress, retag_to_native

_CODEX_MARKER_START = "# --- CoBWeaverClaw persistent provider ---"
_CODEX_MARKER_END = "# --- end CoBWeaverClaw persistent provider ---"
...
_ORPHAN_MODEL_PROVIDER = re.compile(r'(?m)^[ \t]*model_provider[ \t]*=[ \t]*"cobweaverclaw_compress"[ \t]*\r?\n')
_ORPHAN_OPENAI_BASE_URL = re.compile(
    r'(?m)^[ \t]*openai_base_url[ \t]*=[ \t]*"http://127\.0\.0\.1:\d+/v1"[ \t]*\r?\n'
)
_ORPHAN_COBWEAVERCLAW_TABLE = re.compile(
...
def apply_provider_scope(manifest: DeploymentManifest) -> ManagedMutation | None:
    ...
    section = (
        ...
        + build_provider_section(
            port=manifest.port,
            name="CoBWeaverClaw persistent proxy",
...
def revert_provider_scope(mutation: ManagedMutation, manifest: DeploymentManifest) -> None:
    ...
    # left outside the marker block (mirrors wrap.py _strip_codex_cobweaverclaw_compress_blocks).
    content = _ORPHAN_MODEL_PROVIDER.sub("", content)
    content = _ORPHAN_OPENAI_BASE_URL.sub("", content)
    content = _ORPHAN_COBWEAVERCLAW_TABLE.sub("", content)
    path.write_text(content.strip() + "\n", encoding="utf-8")
    # Hand the threads back to the native-provider menu so the full history stays
    # visible once Codex no longer routes through CoBWeaverClaw. Best-effort.
