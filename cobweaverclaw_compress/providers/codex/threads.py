"""Reconcile Codex thread provider tags across the CoBWeaverClaw proxy boundary.

Codex stamps every thread with the ``model_provider`` it ran under and filters
its history/projects menu by the active provider set.  When CoBWeaverClaw rewrites
Codex's config to route through the custom ``cobweaverclaw_compress`` provider (see
:mod:`cobweaverclaw_compress.providers.codex.install`), threads created through CoBWeaverClaw are
tagged ``cobweaverclaw_compress`` while native threads keep ``openai`` -- so the two sets never
appear in the same menu, and connecting CoBWeaverClaw appears to "lose" history.

To keep the menu whole we retag threads to match whichever provider is active:
``openai -> cobweaverclaw_compress`` when CoBWeaverClaw is enabled, ``cobweaverclaw_compress -> openai`` when it is
...
COBWEAVERCLAW_PROVIDER = "cobweaverclaw_compress"
...
def retag_to_cobweaverclaw_compress(codex_home: Path) -> None:
    """Pull existing native threads into the cobweaverclaw_compress-provider menu (on enable)."""
    retag_thread_providers(codex_home, frm=NATIVE_PROVIDER, to=COBWEAVERCLAW_PROVIDER)


def retag_to_native(codex_home: Path) -> None:
    """Hand threads back to the native-provider menu (on revert)."""
    retag_thread_providers(codex_home, frm=COBWEAVERCLAW_PROVIDER, to=NATIVE_PROVIDER)
