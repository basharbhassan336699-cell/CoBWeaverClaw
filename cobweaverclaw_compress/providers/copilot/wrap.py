...
def resolve_provider_type(
    ...
    if env_type in {"anthropic", "openai"}:
        return env_type
    effective_backend = backend or env.get("COBWEAVERCLAW_BACKEND") or "anthropic"
...
def strip_auto_model_args(copilot_args: tuple[str, ...]) -> tuple[str, ...]:
    ...
    to ``cobweaverclaw_compress wrap copilot --subscription``, we strip it before launching
