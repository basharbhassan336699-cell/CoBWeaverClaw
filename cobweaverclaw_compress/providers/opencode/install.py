...
def apply_provider_scope(manifest: DeploymentManifest) -> ManagedMutation | None:
    ...
    snapshot_opencode_config_if_unwrapped(
        config_file, config_file.with_suffix(".json.cobweaverclaw_compress-backup")
    )
...
def revert_provider_scope(mutation: ManagedMutation, manifest: DeploymentManifest) -> None:
    ...
    cobweaverclaw_compress provider from the config file.
    """
    del manifest
    if not mutation.path:
        return
    path = Path(mutation.path)
    backup_file = path.with_suffix(".json.cobweaverclaw_compress-backup")
