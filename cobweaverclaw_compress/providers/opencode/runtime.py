...
from cobweaverclaw_compress.mcp_registry.install import DEFAULT_PROXY_URL

from .config import COBWEAVERCLAW_OPENCODE_PLUGIN, cobweaverclaw_compress_provider_entry
...
def cobweaverclaw_compress_opencode_plugin_path() -> str | None:
    ...
    ``COBWEAVERCLAW_OPENCODE_PLUGIN_PATH`` overrides the resolved path.
    """
    override = os.environ.get("COBWEAVERCLAW_OPENCODE_PLUGIN_PATH", "").strip()
    if override:
        return override if Path(override).is_file() else None
    # runtime.py → opencode → providers → cobweaverclaw_compress → <repo root>
    candidate = (
...
def build_opencode_config_content(
    *,
    ...
    2. **Transparent transport plugin** — when the local plugin is built, it is
       ...
       ``x-cobweaverclaw_compress-base-url``. This covers providers we don't name (Gemini,
       Copilot, custom gateways) and providers added mid-session. The plugin
       self-configures from ``COBWEAVERCLAW_PROXY_URL`` (set in :func:`build_launch_env`).
    ...
    config: dict[str, object] = {
        "provider": {
            ...
            "cobweaverclaw_compress": cobweaverclaw_compress_provider_entry(port),
        }
    }
    if include_mcp:
        ...
        mcp_entry: dict[str, object] = {
            "type": "local",
            "command": ["cobweaverclaw_compress", "mcp", "serve"],
            "enabled": True,
        }
        if proxy_url != DEFAULT_PROXY_URL:
            mcp_entry["environment"] = {"COBWEAVERCLAW_PROXY_URL": proxy_url}
        config["mcp"] = {
            "cobweaverclaw_compress": mcp_entry,
        }
    if include_plugin:
        plugin_path = cobweaverclaw_compress_opencode_plugin_path()
        if plugin_path:
            # Plain absolute-path string; the plugin reads COBWEAVERCLAW_PROXY_URL
...
def build_launch_env(
    ...
    *,
    ...
    """Build environment variables for launching OpenCode through CoBWeaverClaw.

    ``OPENCODE_CONFIG_CONTENT`` carries CoBWeaverClaw provider/MCP/plugin config.
    Existing provider/base URL environment variables are preserved. When the
    transport plugin is loaded, ``COBWEAVERCLAW_PROXY_URL`` tells it which proxy to
    route to.
    """
    ...
    display = ["OPENCODE_CONFIG_CONTENT={provider: cobweaverclaw_compress}"]
    if "plugin" in config_content:
        env["COBWEAVERCLAW_PROXY_URL"] = f"http://127.0.0.1:{port}"
        display.append(f"plugin={COBWEAVERCLAW_OPENCODE_PLUGIN}")

    if project and "COBWEAVERCLAW_PROJECT" not in env:
        env["COBWEAVERCLAW_PROJECT"] = project
