...
from cobweaverclaw_compress.install.paths import opencode_config_path

# CoBWeaverClaw-managed JSON marker comments for idempotent block injection.
_PROVIDER_MARKER_START = "// --- CoBWeaverClaw proxy provider ---"
_PROVIDER_MARKER_END = "// --- end CoBWeaverClaw proxy provider ---"
_MCP_MARKER_START = "// --- CoBWeaverClaw MCP server ---"
_MCP_MARKER_END = "// --- end CoBWeaverClaw MCP server ---"

# Regex to strip cobweaverclaw_compress blocks (including the marker comments).
...
COBWEAVERCLAW_OPENCODE_PLUGIN = "cobweaverclaw_compress-opencode"

# Models exposed by the injected `cobweaverclaw_compress` provider. OpenCode only resolves
# `cobweaverclaw_compress/<id>` for ids listed in the provider's `models` map, so an empty
# map means every documented `cobweaverclaw_compress/*` model fails with "Model not found".
# Keep in sync with DEFAULT_MODELS in plugins/opencode/src/provider.ts and the
# table in plugins/opencode/README.md.
COBWEAVERCLAW_OPENCODE_MODELS: dict[str, Any] = {
    "claude-sonnet-4-6": {
...
def cobweaverclaw_compress_provider_entry(port: int) -> dict[str, Any]:
    """Return the `cobweaverclaw_compress` provider block pointed at the local proxy."""
    return {
        "npm": "@ai-sdk/openai-compatible",
        "name": "CoBWeaverClaw Proxy",
        "options": {"baseURL": f"http://127.0.0.1:{port}/v1"},
        "models": COBWEAVERCLAW_OPENCODE_MODELS,
    }
...
def opencode_config_paths() -> tuple[Path, Path]:
    ...
    backup_file = config_file.with_suffix(".json.cobweaverclaw_compress-backup")
...
def snapshot_opencode_config_if_unwrapped(config_file: Path, backup_file: Path) -> None:
    ...
    Guarantees that ``cobweaverclaw_compress unwrap opencode`` can restore the user's
...
def strip_opencode_cobweaverclaw_compress_blocks(content: str, *, remove_mcp: bool = True) -> str:
    """Remove all CoBWeaverClaw-managed blocks from opencode JSON text.
...
def _render_provider_block(port: int) -> str:
    """Render a CoBWeaverClaw provider block as a JSON comment-wrapped snippet."""
    provider = {"cobweaverclaw_compress": cobweaverclaw_compress_provider_entry(port)}
    lines = [
...
def append_cobweaverclaw_compress_plugin(config: dict[str, object]) -> bool:
    """Append the optional OpenCode plugin entry if it is not already present."""
    plugin = config.get("plugin")
    if plugin is None:
        config["plugin"] = [COBWEAVERCLAW_OPENCODE_PLUGIN]
        return True
    ...
    for entry in plugin:
        if entry == COBWEAVERCLAW_OPENCODE_PLUGIN:
            return False
        if isinstance(entry, list) and entry and entry[0] == COBWEAVERCLAW_OPENCODE_PLUGIN:
            return False

    plugin.append(COBWEAVERCLAW_OPENCODE_PLUGIN)
    return True


def inject_opencode_provider_config(port: int) -> None:
    """Inject a CoBWeaverClaw model provider into OpenCode's config file.
    ...
    ``opencode.json.cobweaverclaw_compress-backup`` so ``cobweaverclaw_compress unwrap opencode``
    ...
    try:
        ...
        # Strip any prior CoBWeaverClaw-managed blocks before re-injecting.
        if _PROVIDER_MARKER_START in content or _MCP_MARKER_START in content:
            content = strip_opencode_cobweaverclaw_compress_blocks(content)
            data = _parse_json_loose(content)
