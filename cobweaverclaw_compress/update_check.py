"""Best-effort "is a newer CoBWeaverClaw released?" check.
...
Opt out with ``COBWEAVERCLAW_UPDATE_CHECK=off``. Also skipped in ``--stateless``
...
PACKAGE_NAME = "cobweaverclaw_compress-ai"
...
def is_update_check_enabled() -> bool:
    ...
    Disabled by ``COBWEAVERCLAW_UPDATE_CHECK=off``, offline mode
    (``COBWEAVERCLAW_OFFLINE``), stateless mode
    (``COBWEAVERCLAW_STATELESS=true``/``1``/``yes``/``on``, matching the proxy's own
    parsing), or any CI environment (``CI`` set).
    """
    from cobweaverclaw_compress.offline import is_offline

    if is_offline():
        return False
    if _env_off("COBWEAVERCLAW_UPDATE_CHECK"):
        return False
    if _env_on("COBWEAVERCLAW_STATELESS"):
        return False
...
def installed_version() -> str | None:
    ...
    ``cobweaverclaw_compress._version.get_version()`` — the latter computes a synthetic
...
def _cache_path() -> Path:
    from cobweaverclaw_compress.paths import workspace_dir
...
def write_cache(latest_version: str, *, now: float | None = None) -> None:
    ...
    try:
        from cobweaverclaw_compress.paths import ensure_workspace_dir
...
def fetch_latest_version(*, allow_pre: bool = False, timeout: float = 4.0) -> str | None:
    ...
    try:
        req = urllib.request.Request(
            _PYPI_JSON_URL,
            headers={"Accept": "application/json", "User-Agent": "cobweaverclaw_compress-update-check"},
        )
...
def run_check(*, allow_pre: bool = False, now: float | None = None) -> str | None:
    ...
    Synchronous — used directly by ``cobweaverclaw_compress update`` and indirectly by
...
def maybe_check_async() -> threading.Thread | None:
    ...
    try:
        ...
        thread = threading.Thread(target=_worker, name="cobweaverclaw_compress-update-check", daemon=True)
        thread.start()
        return thread
    except Exception:
...
def format_update_notice() -> str | None:
    ...
    try:
        ...
        return f"Update available: CoBWeaverClaw {latest} (you have {current}) - run: cobweaverclaw_compress update"
    except Exception:
