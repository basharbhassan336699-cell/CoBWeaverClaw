"""OpenTelemetry-backed operational metrics for CoBWeaverClaw."""
...
_SCOPE_NAME = "cobweaverclaw_compress"
_DEFAULT_EXPORT_INTERVAL_MS = 10000
_MILLISECONDS_TO_SECONDS = 1000.0

_metrics_lock = Lock()
_global_metrics: CoBWeaverClawOtelMetrics | None = None
_owned_meter_provider: Any | None = None
_owned_metrics_config: OTelMetricsConfig | None = None


def _cobweaverclaw_compress_version() -> str:
    try:
        return package_version("cobweaverclaw_compress-ai")
...
class OTelMetricsConfig:
    """Configuration for CoBWeaverClaw-managed OTEL metric export."""

    enabled: bool = False
    service_name: str = "cobweaverclaw_compress"
    ...
    @classmethod
    def from_env(cls, *, default_service_name: str = "cobweaverclaw_compress") -> OTelMetricsConfig:
        exporter_raw = (
            os.environ.get("COBWEAVERCLAW_OTEL_METRICS_EXPORTER", "otlp_http")
            .strip()
            .lower()
            .replace("-", "_")
        )
        if exporter_raw not in {"console", "otlp_http"}:
            logger.warning(
                "Unknown COBWEAVERCLAW_OTEL_METRICS_EXPORTER=%s; falling back to otlp_http",
                exporter_raw,
            )
            exporter_raw = "otlp_http"

        return cls(
            enabled=_parse_bool(os.environ.get("COBWEAVERCLAW_OTEL_METRICS_ENABLED"), default=False),
            service_name=os.environ.get("COBWEAVERCLAW_OTEL_SERVICE_NAME", default_service_name).strip()
            or default_service_name,
            exporter=exporter_raw,  # type: ignore[arg-type]
            endpoint=os.environ.get("COBWEAVERCLAW_OTEL_METRICS_ENDPOINT") or None,
            headers=_parse_key_value_pairs(os.environ.get("COBWEAVERCLAW_OTEL_METRICS_HEADERS")),
            export_interval_millis=_parse_int(
                os.environ.get("COBWEAVERCLAW_OTEL_METRICS_EXPORT_INTERVAL_MS"),
                _DEFAULT_EXPORT_INTERVAL_MS,
            ),
            resource_attributes=_parse_key_value_pairs(
                os.environ.get("COBWEAVERCLAW_OTEL_RESOURCE_ATTRIBUTES")
            ),
        )
...
class CoBWeaverClawOtelMetrics:
    """Shared OTEL metrics facade for CoBWeaverClaw operations."""

    def __init__(self, meter_provider: Any | None = None):
        if meter_provider is None:
            self._meter = metrics.get_meter(_SCOPE_NAME, _cobweaverclaw_compress_version())
        else:
            self._meter = meter_provider.get_meter(_SCOPE_NAME, _cobweaverclaw_compress_version())

        self._proxy_requests = self._meter.create_counter(
            "cobweaverclaw_compress.proxy.requests",
            description="Proxy requests handled by CoBWeaverClaw.",
            unit="1",
        )
        self._proxy_cached_requests = self._meter.create_counter(
            "cobweaverclaw_compress.proxy.requests.cached",
        ...
        self._proxy_cache_write_ttl_tokens = self._meter.create_counter(
            "cobweaverclaw_compress.proxy.cache.write_ttl_tokens",
        ...
        self._proxy_latency = self._meter.create_histogram(
            "cobweaverclaw_compress.proxy.request.duration",
            description="End-to-end proxy request duration.",
            unit="s",
        )
        self._proxy_overhead = self._meter.create_histogram(
            "cobweaverclaw_compress.proxy.overhead.duration",
            description="Time spent inside CoBWeaverClaw optimization logic.",
            unit="s",
        )
        self._proxy_ttfb = self._meter.create_histogram(
            "cobweaverclaw_compress.proxy.ttfb.duration",
            description="Upstream time to first byte observed by CoBWeaverClaw.",
            unit="s",
        )
        self._compression_runs = self._meter.create_counter(
            "cobweaverclaw_compress.compression.runs",
            description="Compression pipeline runs executed by CoBWeaverClaw.",
            unit="1",
        )
        self._compression_failures = self._meter.create_counter(
            "cobweaverclaw_compress.compression.failures",
        ...
        self._compression_input_tokens = self._meter.create_counter(
            "cobweaverclaw_compress.compression.tokens.input",
            description="Input tokens analyzed by CoBWeaverClaw compression.",
            unit="1",
        )
        self._compression_output_tokens = self._meter.create_counter(
            "cobweaverclaw_compress.compression.tokens.output",
            description="Output tokens produced by CoBWeaverClaw compression.",
            unit="1",
        )
        self._compression_saved_tokens = self._meter.create_counter(
            "cobweaverclaw_compress.compression.tokens.saved",
            description="Tokens removed by CoBWeaverClaw compression.",
            unit="1",
        )
        self._compression_duration = self._meter.create_histogram(
            "cobweaverclaw_compress.compression.pipeline.duration",
            description="Compression pipeline execution duration.",
            unit="s",
        )
        self._compression_stage_duration = self._meter.create_histogram(
            "cobweaverclaw_compress.compression.stage.duration",
            description="Per-stage compression timing emitted by the pipeline.",
            unit="s",
        )
        self._compression_transforms = self._meter.create_counter(
            "cobweaverclaw_compress.compression.transforms",
            description="Transforms applied during compression.",
            unit="1",
        )
        self._waste_signal_tokens = self._meter.create_counter(
            "cobweaverclaw_compress.compression.waste.tokens",
...
def get_otel_metrics() -> CoBWeaverClawOtelMetrics:
    global _global_metrics

    if _global_metrics is None:
        with _metrics_lock:
            if _global_metrics is None:
                _global_metrics = CoBWeaverClawOtelMetrics()

    return _global_metrics


def set_otel_metrics(otel_metrics: CoBWeaverClawOtelMetrics) -> CoBWeaverClawOtelMetrics:
    global _global_metrics
    with _metrics_lock:
...
def configure_otel_metrics(config: OTelMetricsConfig | None = None) -> CoBWeaverClawOtelMetrics:
    global _global_metrics
    ...
    try:
        ...
        logger.warning(
            ...
            "Install cobweaverclaw_compress-ai[otel] to enable managed OTEL metric export."
        )
        return get_otel_metrics()
    ...
    resource = Resource.create(
        {
            ...
            SERVICE_VERSION: _cobweaverclaw_compress_version(),
            **resolved.resource_attributes,
        }
    )
    meter_provider = MeterProvider(resource=resource, metric_readers=[reader])
    otel_metrics = CoBWeaverClawOtelMetrics(meter_provider=meter_provider)
...
def get_otel_metrics_status() -> dict[str, Any]:
    ...
    return OTelMetricsConfig.from_env(default_service_name="cobweaverclaw_compress-proxy").status()
