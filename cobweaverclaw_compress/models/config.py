"""Central configuration for all ML models used in CoBWeaverClaw.
...
Usage:
    ...
    # COBWEAVERCLAW_SENTENCE_TRANSFORMER=intfloat/e5-small-v2
    # COBWEAVERCLAW_SIGLIP=google/siglip-base-patch16-224
"""
...
class MLModelConfig:
    ...
    All model names are defined here. Components throughout CoBWeaverClaw
    import these defaults, so changing a value here changes it everywhere.

    Environment variables can override any default:
    - COBWEAVERCLAW_SENTENCE_TRANSFORMER
    - COBWEAVERCLAW_SIGLIP
    - COBWEAVERCLAW_SPACY
    - COBWEAVERCLAW_TECHNIQUE_ROUTER
    ...
    sentence_transformer: str = field(
        default_factory=lambda: os.environ.get("COBWEAVERCLAW_SENTENCE_TRANSFORMER", "all-MiniLM-L6-v2")
    )
    sentence_transformer_dim: int = 384

    # Image Embeddings (SIGLIP)
    siglip: str = field(
        default_factory=lambda: os.environ.get("COBWEAVERCLAW_SIGLIP", "google/siglip-base-patch16-224")
    )

    # Named Entity Recognition (spaCy)
    spacy: str = field(default_factory=lambda: os.environ.get("COBWEAVERCLAW_SPACY", "en_core_web_sm"))

    # Image Technique Router
    technique_router: str = field(
        default_factory=lambda: os.environ.get(
            "COBWEAVERCLAW_TECHNIQUE_ROUTER", "chopratejas/technique-router"
        )
    )
