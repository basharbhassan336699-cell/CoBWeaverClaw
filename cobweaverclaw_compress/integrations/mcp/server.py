"""CoBWeaverClaw MCP integration helpers for compressing tool outputs.

This module currently provides these MCP integration surfaces:

1. CoBWeaverClawMCPCompressor - core compression logic for MCP tool results
2. compress_tool_result() - standalone helper for host applications
3. CoBWeaverClawMCPClientWrapper - client wrapper that compresses tool outputs
4. create_cobweaverclaw_compress_mcp_proxy() - configuration helper for custom proxy setups

The key insight: MCP tool outputs are the PERFECT use case for CoBWeaverClaw.
...
Example - Custom proxy configuration:
    ...
    proxy_config = create_cobweaverclaw_compress_mcp_proxy(
        upstream_servers=[
            ("slack", slack_server),
            ("database", db_server),
...
Example - Middleware (for MCP client libraries):
    ...
    middleware = CoBWeaverClawMCPMiddleware(config)
...
from cobweaverclaw_compress.config import CoBWeaverClawConfig, SmartCrusherConfig
from cobweaverclaw_compress.providers.openai import OpenAIProvider
from cobweaverclaw_compress.transforms.smart_crusher import SmartCrusher
...
class CoBWeaverClawMCPCompressor:
    ...
    def compress(
        ...
        # Remove any CoBWeaverClaw markers for clean output
        compressed_content = re.sub(r"\n<cobweaverclaw_compress:[^>]+>", "", compressed_content)
...
def compress_tool_result(
    ...
    This is the simplest way to use CoBWeaverClaw with MCP in your host application.
    ...
    Args:
        ...
        config: Optional CoBWeaverClaw configuration.
    ...
    Example:
        ...
        ```
    """
    compressor = CoBWeaverClawMCPCompressor(config=config)
...
class CoBWeaverClawMCPClientWrapper:
    ...
    Example:
        ...
        from cobweaverclaw_compress.integrations.mcp import CoBWeaverClawMCPClientWrapper

        # Wrap your MCP client
        base_client = Client(transport)
        client = CoBWeaverClawMCPClientWrapper(base_client)
    ...
    def __init__(
        ...
        """
        self._client = client
        self._compressor = CoBWeaverClawMCPCompressor(config=config)
...
def create_cobweaverclaw_compress_mcp_proxy(
    upstream_servers: list[tuple[str, MCPServer]],
    config: CoBWeaverClawConfig | None = None,
) -> dict[str, Any]:
    """Create configuration for a custom CoBWeaverClaw MCP proxy/server wrapper.

    This returns the compressor and upstream-server mapping needed by an
    application-defined MCP proxy/server wrapper. CoBWeaverClaw does not yet ship
    a ready-to-run ``CoBWeaverClawMCPProxy`` server implementation.
