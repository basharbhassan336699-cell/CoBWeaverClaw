"""LangSmith integration for CoBWeaverClaw compression metrics.

This module provides CoBWeaverClawLangSmithCallbackHandler, a LangChain callback
handler that adds CoBWeaverClaw compression metrics to LangSmith traces.

When used with CoBWeaverClawChatModel, it automatically captures:
...
Example:
    ...
    from cobweaverclaw_compress.integrations import (
    ...
    # Create handler
    handler = CoBWeaverClawLangSmithCallbackHandler()

    # Use with CoBWeaverClawChatModel
    llm = CoBWeaverClawChatModel(
        ChatOpenAI(model="gpt-4o"),
        callbacks=[handler],
    )

    # Traces will include cobweaverclaw_compress.* metadata
...
class CoBWeaverClawLangSmithCallbackHandler(BaseCallbackHandler):
    """Callback handler that adds CoBWeaverClaw metrics to LangSmith traces.

    Integrates with LangSmith to provide visibility into context
    optimization within traces. Metrics appear as metadata with
    the `cobweaverclaw_compress.` prefix.

    Works automatically when:
    1. LANGCHAIN_TRACING_V2=true is set
    2. Used as a callback with CoBWeaverClawChatModel
    ...
    Example:
        ...
        # - cobweaverclaw_compress.tokens_before
        # - cobweaverclaw_compress.tokens_after
        # - cobweaverclaw_compress.tokens_saved
        # - cobweaverclaw_compress.savings_percent
        # - cobweaverclaw_compress.transforms_applied
    ...
    def __init__(
        ...
        """Initialize CoBWeaverClawLangSmithCallbackHandler.
        ...
        Args:
            ...
            auto_update_runs: If True, automatically updates LangSmith
                runs with CoBWeaverClaw metadata. Default True.
        """
    ...
    def set_cobweaverclaw_compress_metrics(
        self,
        run_id: str | UUID,
        tokens_before: int,
        tokens_after: int,
        ...
        """Set CoBWeaverClaw metrics for a run.

        Call this from CoBWeaverClawChatModel after optimization to attach
        ...
        logger.debug(
            f"CoBWeaverClaw metrics set for run {run_id_str}: "
    ...
    def on_llm_end(
        ...
        **kwargs: Any,
        ...
        Attaches pending CoBWeaverClaw metrics to the LangSmith run.
        """
        run_id_str = str(run_id)
    ...
    def _attach_metrics_to_run(self, run_id: str, metrics: PendingMetrics) -> None:
        """Attach CoBWeaverClaw metrics to a LangSmith run.
        ...
        """
        metadata = {
            "cobweaverclaw_compress.tokens_before": metrics.tokens_before,
            "cobweaverclaw_compress.tokens_after": metrics.tokens_after,
            "cobweaverclaw_compress.tokens_saved": metrics.tokens_saved,
            "cobweaverclaw_compress.savings_percent": round(metrics.savings_percent, 2),
            "cobweaverclaw_compress.transforms_applied": metrics.transforms_applied,
            "cobweaverclaw_compress.optimization_timestamp": metrics.timestamp.isoformat(),
        }
        ...
        if self._client and self._auto_update:
            try:
                ...
                logger.debug(f"Updated LangSmith run {run_id} with CoBWeaverClaw metrics")
            except Exception as e:
                logger.debug(f"Could not update LangSmith run: {e}")

    def get_run_metrics(self, run_id: str | UUID) -> dict[str, Any]:
        """Get CoBWeaverClaw metrics for a specific run.

        Args:
            run_id: The run ID.

        Returns:
            Dictionary of cobweaverclaw_compress.* metrics for the run.
        """
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        total_saved = sum(m.get("cobweaverclaw_compress.tokens_saved", 0) for m in self._run_metrics.values())
        savings_percents = [
            m.get("cobweaverclaw_compress.savings_percent", 0) for m in self._run_metrics.values()
        ]
