...
This module provides CoBWeaverClawToolWrapper and wrap_tools_with_cobweaverclaw_compress
...
Example:
    ...
    from cobweaverclaw_compress.integrations import wrap_tools_with_cobweaverclaw_compress
    ...
    # Wrap with CoBWeaverClaw compression
    wrapped_tools = wrap_tools_with_cobweaverclaw_compress(tools)
...
class CoBWeaverClawToolWrapper:
    ...
    Example:
        from langchain.tools import Tool
        from cobweaverclaw_compress.integrations import CoBWeaverClawToolWrapper
        ...
        wrapped = CoBWeaverClawToolWrapper(search_tool)
    ...
    def __init__(
        ...
        """Initialize CoBWeaverClawToolWrapper.
    ...
    def _record_metrics(self, original: str, compressed: str, was_compressed: bool) -> None:
        ...
        if was_compressed and chars_saved > 0:
            logger.info(
                f"CoBWeaverClawToolWrapper[{self.name}]: {chars_before} -> {chars_after} chars "
...
def wrap_tools_with_cobweaverclaw_compress(
    tools: list[BaseTool],
    min_chars_to_compress: int = 1000,
    metrics_collector: ToolMetricsCollector | None = None,
) -> list[StructuredTool]:
    """Wrap multiple LangChain tools with CoBWeaverClaw compression.
    ...
    Example:
        ...
        wrapped = wrap_tools_with_cobweaverclaw_compress(tools)
    ...
    wrapped = []
    for tool in tools:
        wrapper = CoBWeaverClawToolWrapper(
            tool=tool,
