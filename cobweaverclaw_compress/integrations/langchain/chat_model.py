"""LangChain integration for CoBWeaverClaw SDK.
...
Components:
1. CoBWeaverClawChatModel - Wraps any BaseChatModel to apply CoBWeaverClaw transforms
2. CoBWeaverClawCallbackHandler - Tracks metrics and token usage (observability only)
3. CoBWeaverClawRunnable - LCEL-compatible Runnable for chain composition
...
Example:
    ...
    optimized_llm = CoBWeaverClawChatModel(llm)

    # Use normally - CoBWeaverClaw automatically optimizes context
...
from cobweaverclaw_compress import CoBWeaverClawConfig, CoBWeaverClawMode
from cobweaverclaw_compress.providers import OpenAIProvider
from cobweaverclaw_compress.transforms import TransformPipeline

from .providers import get_cobweaverclaw_compress_provider, get_model_name_from_langchain
...
def _check_langchain_available() -> None:
    ...
    if not LANGCHAIN_AVAILABLE:
        raise ImportError(
            ...
            "Install with: pip install cobweaverclaw_compress[langchain] "
...
class CoBWeaverClawChatModel(BaseChatModel):
    """LangChain chat model wrapper that applies CoBWeaverClaw optimizations.

    Wraps any LangChain BaseChatModel and automatically optimizes the context
    before each API call. This is the recommended way to use CoBWeaverClaw with
    LangChain because:
    ...
    Example:
        ...
        # Basic usage
        llm = ChatOpenAI(model="gpt-4o")
        optimized = CoBWeaverClawChatModel(llm)
        response = optimized.invoke([HumanMessage("Hello!")])

        # With custom config
        from cobweaverclaw_compress import CoBWeaverClawConfig, CoBWeaverClawMode
        config = CoBWeaverClawConfig(default_mode=CoBWeaverClawMode.OPTIMIZE)
        optimized = CoBWeaverClawChatModel(llm, config=config)
    ...
    Attributes:
        ...
        cobweaverclaw_compress_client: CoBWeaverClawClient instance for optimization
    ...
    cobweaverclaw_compress_config: Any = Field(default=None, description="CoBWeaverClaw configuration")
    mode: CoBWeaverClawMode = Field(default=CoBWeaverClawMode.OPTIMIZE, description="CoBWeaverClaw mode")
    ...
    def __init__(
        ...
        config: CoBWeaverClawConfig | None = None,
        mode: CoBWeaverClawMode = CoBWeaverClawMode.OPTIMIZE,
        auto_detect_provider: bool = True,
        **kwargs: Any,
    ) -> None:
        """Initialize CoBWeaverClawChatModel.

        Args:
            wrapped_model: Any LangChain BaseChatModel to wrap
            config: CoBWeaverClawConfig for optimization settings
            mode: CoBWeaverClawMode (AUDIT, OPTIMIZE, or SIMULATE)
            auto_detect_provider: Auto-detect provider from wrapped model.
                ...
                CoBWeaverClaw provider for accurate token counting.
        ...
        super().__init__(  # type: ignore[call-arg]
            ...
            cobweaverclaw_compress_config=config or CoBWeaverClawConfig(),
            mode=mode,
    ...
    def _llm_type(self) -> str:
        ...
        return f"cobweaverclaw_compress-{self.wrapped_model._llm_type}"
    ...
    def _identifying_params(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cobweaverclaw_compress_mode": self.mode.value,
        }
    ...
    def pipeline(self) -> TransformPipeline:
        ...
        if self._pipeline is None:
            if self.auto_detect_provider:
                self._provider = get_cobweaverclaw_compress_provider(self.wrapped_model)
                logger.debug(f"Auto-detected provider: {self._provider.__class__.__name__}")
            else:
                self._provider = OpenAIProvider()
            self._pipeline = TransformPipeline(
                config=self.cobweaverclaw_compress_config,
    ...
    def _convert_messages_to_openai(self, messages: list[BaseMessage]) -> list[dict[str, Any]]:
        """Convert LangChain messages to OpenAI format for CoBWeaverClaw."""
        result = []
        for msg in messages:
    ...
    def _optimize_messages(
        ...
        """Apply CoBWeaverClaw optimization to messages."""
        ...
        # Apply CoBWeaverClaw transforms via pipeline
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        """Generate response with CoBWeaverClaw optimization.
        ...
        logger.info(
            f"CoBWeaverClaw optimized: {metrics.tokens_before} -> {metrics.tokens_after} tokens "
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        """Stream response with CoBWeaverClaw optimization."""
        # Optimize messages
        optimized_messages, metrics = self._optimize_messages(messages)

        logger.info(
            f"CoBWeaverClaw optimized (streaming): {metrics.tokens_before} -> "
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        """Async generate response with CoBWeaverClaw optimization.
        ...
        logger.info(
            f"CoBWeaverClaw optimized (async): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
        ...
        # same CoBWeaverClawChatModel instance. (GitHub #1285, review feedback)
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        """Async stream response with CoBWeaverClaw optimization.
        ...
        logger.info(
            f"CoBWeaverClaw optimized (async streaming): {metrics.tokens_before} -> "
            f"{metrics.tokens_after} tokens"
        )
    ...
    def bind_tools(self, tools: Sequence[Any], **kwargs: Any) -> CoBWeaverClawChatModel:
        """Bind tools to the wrapped model."""
        new_wrapped = self.wrapped_model.bind_tools(tools, **kwargs)
        return CoBWeaverClawChatModel(
...
class CoBWeaverClawCallbackHandler(BaseCallbackHandler):
    """LangChain callback handler for CoBWeaverClaw metrics and observability.

    NOTE: Callbacks CANNOT modify messages in LangChain (by design).
    Use CoBWeaverClawChatModel for actual optimization. This handler is for:
    ...
    Example:
        from langchain_openai import ChatOpenAI
        from cobweaverclaw_compress.integrations import CoBWeaverClawCallbackHandler

        handler = CoBWeaverClawCallbackHandler(
            log_level="INFO",
...
class CoBWeaverClawRunnable:
    """LCEL-compatible Runnable for CoBWeaverClaw optimization.

    Use this to add CoBWeaverClaw optimization to any LangChain chain using LCEL.

    Example:
        from langchain_openai import ChatOpenAI
        from langchain_core.prompts import ChatPromptTemplate
        from cobweaverclaw_compress.integrations import CoBWeaverClawRunnable
        ...
        # Add CoBWeaverClaw optimization to chain
        chain = prompt | CoBWeaverClawRunnable() | llm
    ...
    def __init__(
        ...
        """Initialize CoBWeaverClawRunnable.
        ...
        self.config = config or CoBWeaverClawConfig()
        self.mode = mode
...
def optimize_messages(
    ...
    Example:
        ...
        from cobweaverclaw_compress.integrations import optimize_messages
    ...
    config = config or CoBWeaverClawConfig()
