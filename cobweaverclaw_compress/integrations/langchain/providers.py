...
class paths to identify the appropriate CoBWeaverClaw provider.

Example:
    from langchain_anthropic import ChatAnthropic
    from cobweaverclaw_compress.integrations.langchain import get_cobweaverclaw_compress_provider

    model = ChatAnthropic(model="claude-3-5-sonnet-20241022")
    provider = get_cobweaverclaw_compress_provider(model)  # Returns AnthropicProvider
"""
...
def get_cobweaverclaw_compress_provider(model: Any) -> Provider:
    """Get appropriate CoBWeaverClaw Provider instance for a LangChain model.

    This function automatically detects the provider from the model type
    and returns a configured CoBWeaverClaw provider for accurate token counting
    ...
    Returns:
        Configured CoBWeaverClaw Provider instance

    Example:
        >>> from langchain_anthropic import ChatAnthropic
        >>> model = ChatAnthropic(model="claude-3-5-sonnet-20241022")
        >>> provider = get_cobweaverclaw_compress_provider(model)
        >>> provider.name
        'anthropic'
    """
    # Import providers lazily to avoid circular imports
    from cobweaverclaw_compress.providers import (
        AnthropicProvider,
        GoogleProvider,
        OpenAIProvider,
    )
