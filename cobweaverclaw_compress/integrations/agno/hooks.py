"""Agno hooks for CoBWeaverClaw integration.

This module provides pre_hooks and post_hooks that can be used with
Agno agents to apply CoBWeaverClaw optimization at the agent level.
"""
...
@dataclass
class HookMetrics:
    """Metrics collected by CoBWeaverClaw pre-hooks.

    Note: These metrics track request counts and timing, not token savings.
    For actual token optimization metrics, use CoBWeaverClawAgnoModel which
    ...
    # Use CoBWeaverClawAgnoModel for actual token optimization
    tokens_before: int = 0
    tokens_after: int = 0
    tokens_saved: int = 0
...
class CoBWeaverClawPreHook:
    ...
    optimization, use CoBWeaverClawAgnoModel to wrap your model.
    ...
    using CoBWeaverClawAgnoModel.

    Example:
        from agno.agent import Agent
        from agno.models.openai import OpenAIChat
        from cobweaverclaw_compress.integrations.agno import CoBWeaverClawPreHook, CoBWeaverClawAgnoModel

        # For request tracking only
        pre_hook = CoBWeaverClawPreHook()

        # For actual optimization, wrap the model
        model = CoBWeaverClawAgnoModel(OpenAIChat(id="gpt-4o"))
    ...
    def __init__(
        ...
        """Initialize CoBWeaverClawPreHook.

        Args:
            config: CoBWeaverClawConfig for optimization settings (stored for future use)
            mode: CoBWeaverClawMode (stored for future use)
    ...
    def __call__(self, run_input: Any, **kwargs: Any) -> Any:
        ...
        logger.debug(f"CoBWeaverClawPreHook tracking request {request_id}")
        ...
        # Return input unchanged (use CoBWeaverClawAgnoModel for actual optimization)
        return run_input
...
class CoBWeaverClawPostHook:
    ...
    Example:
        from agno.agent import Agent
        from agno.models.openai import OpenAIChat
        from cobweaverclaw_compress.integrations.agno import CoBWeaverClawPostHook

        post_hook = CoBWeaverClawPostHook()
    ...
    def __init__(
        ...
        """Initialize CoBWeaverClawPostHook.
...
def create_cobweaverclaw_compress_hooks(
    config: CoBWeaverClawConfig | None = None,
    mode: CoBWeaverClawMode = CoBWeaverClawMode.OPTIMIZE,
    model: str = "gpt-4o",
    log_level: str = "INFO",
    token_alert_threshold: int | None = None,
) -> tuple[CoBWeaverClawPreHook, CoBWeaverClawPostHook]:
    """Create a pair of CoBWeaverClaw hooks for Agno agents.
    ...
    Example:
        from agno.agent import Agent
        from agno.models.openai import OpenAIChat
        from cobweaverclaw_compress.integrations.agno import create_cobweaverclaw_compress_hooks

        pre_hook, post_hook = create_cobweaverclaw_compress_hooks(
    ...
    """
    pre_hook = CoBWeaverClawPreHook(config=config, mode=mode, model=model)
    post_hook = CoBWeaverClawPostHook(
        log_level=log_level,
