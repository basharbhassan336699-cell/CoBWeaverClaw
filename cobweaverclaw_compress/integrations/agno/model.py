"""Agno model wrapper for CoBWeaverClaw optimization.

This module provides CoBWeaverClawAgnoModel, which wraps any Agno model
to apply CoBWeaverClaw context optimization before API calls.
"""
...
from cobweaverclaw_compress.parser import _coerce_tool_call_to_dict
...
@dataclass
class CoBWeaverClawAgnoModel(Model):  # type: ignore[misc]
    """Agno model wrapper that applies CoBWeaverClaw optimizations.
    ...
    Example:
        ...
        optimized = CoBWeaverClawAgnoModel(wrapped_model=model)
        ...
        optimized = CoBWeaverClawAgnoModel(wrapped_model=model, cobweaverclaw_compress_config=config)

        # Agno reasoning with CoBWeaverClawAgnoModel
        model = OpenAIChat(id="gpt-4o")
        wrapped = CoBWeaverClawAgnoModel(wrapped_model=model)
        agent = Agent(
            model=wrapped,
            reasoning=True,
    ...
    id: str = field(default="cobweaverclaw_compress-wrapper")
    name: str | None = field(default=None)
    provider: str | None = field(default=None)

    # CoBWeaverClawAgnoModel specific fields
    wrapped_model: Any = field(default=None)
    cobweaverclaw_compress_config: CoBWeaverClawConfig | None = field(default=None)
    cobweaverclaw_compress_mode: CoBWeaverClawMode | None = field(default=None)
    ...
    _cobweaverclaw_compress_provider: Any = field(default=None, repr=False, compare=False)
    _lock: threading.Lock = field(default_factory=threading.Lock, repr=False, compare=False)
    _initialized: bool = field(default=False, repr=False, compare=False)

    def __post_init__(self) -> None:
        """Initialize CoBWeaverClawAgnoModel after dataclass construction."""
        ...
        if hasattr(self.wrapped_model, "id"):
            self.id = f"cobweaverclaw_compress:{self.wrapped_model.id}"
        ...
        # Initialize config
        if self.cobweaverclaw_compress_config is None:
            self.cobweaverclaw_compress_config = CoBWeaverClawConfig()

        # Handle deprecated mode parameter
        if self.cobweaverclaw_compress_mode is not None:
            warnings.warn(
                "The 'cobweaverclaw_compress_mode' parameter is deprecated. Use CoBWeaverClawConfig(default_mode=...) instead.",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name in (
            "wrapped_model",
            "cobweaverclaw_compress_config",
            "cobweaverclaw_compress_mode",
    ...
    def underlying_model(self) -> Any:
        ...
        Example:
            wrapped = CoBWeaverClawAgnoModel(wrapped_model=Claude(...))
            ...
            with self._lock:
                ...
                if self._pipeline is None:
                    if self.auto_detect_provider:
                        self._cobweaverclaw_compress_provider = get_cobweaverclaw_compress_provider(self.wrapped_model)
                        logger.debug(
                            f"Auto-detected provider: {self._cobweaverclaw_compress_provider.__class__.__name__}"
                        )
                    else:
                        self._cobweaverclaw_compress_provider = OpenAIProvider()
                    self._pipeline = TransformPipeline(
                        config=self.cobweaverclaw_compress_config,
                        provider=self._cobweaverclaw_compress_provider,
                    )
    ...
    def _convert_messages_to_openai(self, messages: list[Any]) -> list[dict[str, Any]]:
        """Convert Agno messages to OpenAI format for CoBWeaverClaw.
        ...
        for msg in messages:
            ...
            if hasattr(msg, "role") and hasattr(msg, "content"):
                ...
                # CoBWeaverClaw pipeline + Agno's own re-serialization both call
    ...
    def _optimize_messages(self, messages: list[Any]) -> tuple[list[Any], OptimizationMetrics]:
        ...
        if self._has_thinking_blocks(openai_messages):
            logger.info("Skipping CoBWeaverClaw optimization: messages contain extended thinking blocks")
        ...
        model_limit = (
            self._cobweaverclaw_compress_provider.get_context_limit(model) if self._cobweaverclaw_compress_provider else 128000
        )
        ...
        try:
            ...
            logger.warning(
                f"CoBWeaverClaw optimization failed, using original messages: {type(e).__name__}: {e}"
            )
    ...
    def response(self, messages: list[Any], **kwargs: Any) -> Any:  # type: ignore[override]
        ...
        applies CoBWeaverClaw optimization before delegating to wrapped_model.invoke().
    ...
    # These delegate to the wrapped model after applying CoBWeaverClaw optimization
    # =========================================================================
    ...
    def invoke(self, messages: list[Any], **kwargs: Any) -> Any:
        ...
        logger.info(
            f"CoBWeaverClaw optimized (invoke): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
    ...
    async def ainvoke(self, messages: list[Any], **kwargs: Any) -> Any:
        ...
        logger.info(
            f"CoBWeaverClaw optimized (ainvoke): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
    ...
    def invoke_stream(self, messages: list[Any], **kwargs: Any) -> Iterator[Any]:
        ...
        logger.info(
            f"CoBWeaverClaw optimized (invoke_stream): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
    ...
    async def ainvoke_stream(self, messages: list[Any], **kwargs: Any) -> AsyncIterator[Any]:
        ...
        logger.info(
            f"CoBWeaverClaw optimized (ainvoke_stream): {metrics.tokens_before} -> {metrics.tokens_after} tokens "
            f"({metrics.savings_percent:.1f}% saved)"
        )
...
def optimize_messages(
    ...
    Example:
        from cobweaverclaw_compress.integrations.agno import optimize_messages
