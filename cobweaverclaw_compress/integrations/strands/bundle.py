"""CoBWeaverClawBundle — single-helper MCP wiring for a Strands Agent.

The cleanest production setup for Strands is the same kit
``cobweaverclaw_compress wrap claude`` installs for Claude Code, restated as
Strands-native primitives:

* **CoBWeaverClaw MCP** (``cobweaverclaw_compress mcp serve``) — exposes
  ``cobweaverclaw_compress_retrieve`` / ``cobweaverclaw_compress_compress`` / ``cobweaverclaw_compress_stats``
  via stdio. The proxy emits ``Retrieve original: hash=...`` markers
  in compressed content; the LLM calls ``cobweaverclaw_compress_retrieve`` when it
...
* **CoBWeaverClawHookProvider** — the RTK-equivalent for Strands.
...
.. code-block:: python
    ...
    from cobweaverclaw_compress.integrations.strands import CoBWeaverClawBundle
    ...
    bundle = CoBWeaverClawBundle(proxy_url="http://127.0.0.1:8787")
    agent = Agent(
        model=model,
...
``cobweaverclaw_compress proxy``. This keeps the bundle stateless and lets the
...
from cobweaverclaw_compress import CoBWeaverClawConfig
from cobweaverclaw_compress.mcp_registry.install import (
    DEFAULT_PROXY_URL,
    build_cobweaverclaw_compress_spec,
    build_serena_spec,
...
#: code-aware agent loop (the same context ``cobweaverclaw_compress wrap claude`` uses).
...
def _make_cobweaverclaw_compress_client(proxy_url: str) -> MCPClient:
    return _client_for(build_cobweaverclaw_compress_spec(proxy_url))
...
@dataclass
class CoBWeaverClawBundle:
    """Single helper that hands a Strands Agent every CoBWeaverClaw integration.

    Attributes:
        proxy_url: HTTP URL the CoBWeaverClaw MCP server should contact for
            retrieval. Default :data:`DEFAULT_PROXY_URL`
            (``http://127.0.0.1:8787``).
        serena_context: Serena context label. Default ``"ide-assistant"``.
        enable_cobweaverclaw_compress_mcp: Include the CoBWeaverClaw MCP server. Default True.
        ...
        enable_hooks: Include :class:`CoBWeaverClawHookProvider` for in-place
            tool-output compression (the RTK-equivalent for Strands).
            Default True.
        config: Optional :class:`CoBWeaverClawConfig` passed to
            :class:`CoBWeaverClawHookProvider`. Default uses framework
            defaults.
    ...
    enable_cobweaverclaw_compress_mcp: bool = True
    ...
    # (CoBWeaverClawHookProvider) is an optimisation for memory/network when
    ...
    _cobweaverclaw_compress_mcp: MCPClient | None = field(default=None, init=False, repr=False, compare=False)
    ...
    _hook: CoBWeaverClawHookProvider | None = field(default=None, init=False, repr=False, compare=False)

    def __post_init__(self) -> None:
        if self.enable_cobweaverclaw_compress_mcp:
            self._cobweaverclaw_compress_mcp = _make_cobweaverclaw_compress_client(self.proxy_url)
            logger.info(
                "CoBWeaverClawBundle: CoBWeaverClaw MCP client constructed (proxy_url=%s)",
                self.proxy_url,
            )
        if self.enable_tokensave_mcp:
            self._tokensave_mcp = _make_tokensave_client()
            logger.info("CoBWeaverClawBundle: tokensave MCP client constructed (primary)")
        if self.enable_serena_mcp:
            self._serena_mcp = _make_serena_client(self.serena_context)
            logger.info(
                "CoBWeaverClawBundle: Serena MCP client constructed (backup, context=%s)",
                self.serena_context,
            )
        if self.enable_hooks:
            self._hook = CoBWeaverClawHookProvider(config=self.config)
            logger.info("CoBWeaverClawBundle: CoBWeaverClawHookProvider attached")
    ...
    def tools(self) -> list[Any]:
        ...
        """
        out: list[Any] = []
        if self._cobweaverclaw_compress_mcp is not None:
            out.append(self._cobweaverclaw_compress_mcp)
    ...
    @property
    def cobweaverclaw_compress_mcp(self) -> MCPClient | None:
        """Direct handle to the CoBWeaverClaw MCPClient (for advanced callers)."""
        return self._cobweaverclaw_compress_mcp
