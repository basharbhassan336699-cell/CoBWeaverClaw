"""Strands SDK hook provider for CoBWeaverClaw tool output compression.

This module provides CoBWeaverClawHookProvider, which implements Strands' HookProvider
interface to intercept tool outputs and compress them using CoBWeaverClaw's SmartCrusher.

Example:
    from strands import Agent
    from cobweaverclaw_compress.integrations.strands import CoBWeaverClawHookProvider

    # Create the hook provider
    hook_provider = CoBWeaverClawHookProvider(
...
from cobweaverclaw_compress.transforms.smart_crusher import SmartCrusher, SmartCrusherConfig
...
@dataclass
class CoBWeaverClawHookProvider(HookProvider):  # type: ignore[misc]
    """Strands HookProvider that compresses tool outputs using CoBWeaverClaw.

    This hook provider intercepts tool call results via AfterToolCallEvent
    and applies CoBWeaverClaw's SmartCrusher to compress large outputs, reducing
    ...
    Example:
        ...
        hook = CoBWeaverClawHookProvider(min_tokens_to_compress=50)
    ...
    config: CoBWeaverClawConfig | None = field(default=None)
    ...
    def __post_init__(self) -> None:
        ...
        if self.config is None:
            self.config = CoBWeaverClawConfig()

        self._initialized = True
        logger.debug(
            "CoBWeaverClawHookProvider initialized: compress=%s, min_tokens=%d, preserve_errors=%s",
    ...
    def crusher(self) -> SmartCrusher:
        ...
        if self._crusher is None:
            with self._lock:
                ...
                if self._crusher is None:
                    # Use config from CoBWeaverClawConfig if available
    ...
    def register_hooks(self, registry: HookRegistry, **kwargs: Any) -> None:
        ...
        logger.info(
            "CoBWeaverClawHookProvider registered: compressing tool outputs >= %d tokens",
            self.min_tokens_to_compress,
        )
    ...
    def reset(self) -> None:
        ...
        logger.debug("CoBWeaverClawHookProvider metrics reset")
