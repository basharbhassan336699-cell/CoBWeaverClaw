"""Strands SDK model wrapper for CoBWeaverClaw optimization.

This module provides CoBWeaverClawStrandsModel, which wraps any Strands model
...
from cobweaverclaw_compress import CoBWeaverClawConfig  # noqa: E402
from cobweaverclaw_compress.providers import OpenAIProvider  # noqa: E402
from cobweaverclaw_compress.transforms import TransformPipeline  # noqa: E402

from .providers import get_cobweaverclaw_compress_provider, get_model_name_from_strands  # noqa: E402
...
class CoBWeaverClawStrandsModel(Model):  # type: ignore[misc]
    """Strands model wrapper that applies CoBWeaverClaw optimizations.
    ...
    Example:
        ...
        optimized = CoBWeaverClawStrandsModel(wrapped_model=model)
        ...
        config = CoBWeaverClawConfig()
        optimized = CoBWeaverClawStrandsModel(wrapped_model=model, config=config)
    ...
    def __init__(
        ...
        """Initialize CoBWeaverClawStrandsModel.

        Args:
            wrapped_model: The Strands model to wrap (e.g., BedrockModel, OpenAIModel)
            config: Optional CoBWeaverClawConfig for optimization settings
            auto_detect_provider: Whether to auto-detect the CoBWeaverClaw provider
        ...
        self.cobweaverclaw_compress_config = config or CoBWeaverClawConfig()
        ...
        self._cobweaverclaw_compress_provider: Any = None
    ...
    def _convert_messages_to_openai(self, messages: list[Any]) -> list[dict[str, Any]]:
        """Convert Strands messages to OpenAI format for CoBWeaverClaw.
    ...
    async def stream(
        ...
        **kwargs: Any,
        ...
        logger.info(
            f"CoBWeaverClaw optimized (stream): {metrics.tokens_before} -> "
    ...
    async def structured_output(
        ...
        **kwargs: Any,
        ...
        """Generate structured output with CoBWeaverClaw optimization.
        ...
        logger.info(
            f"CoBWeaverClaw optimized (structured_output): {metrics.tokens_before} -> "
            f"{metrics.tokens_after} tokens ({metrics.savings_percent:.1f}% saved)"
        )
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if name in (
            ...
            "_total_tokens_saved",
            "_pipeline",
            "_cobweaverclaw_compress_provider",
            "_lock",
            "pipeline",
            "total_tokens_saved",
            "metrics_history",
...
def optimize_messages(
    ...
    Example:
        from cobweaverclaw_compress.integrations.strands import optimize_messages
