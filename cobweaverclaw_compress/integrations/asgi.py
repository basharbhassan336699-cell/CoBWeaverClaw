"""ASGI Middleware — add CoBWeaverClaw compression to any Python proxy.
...
Local mode (compression runs in-process):
    ...
    from cobweaverclaw_compress.integrations.asgi import CompressionMiddleware
    app.add_middleware(CompressionMiddleware)

Cloud mode (managed CCR, TOIN, analytics via CoBWeaverClaw Cloud):
...
_DEFAULT_CLOUD_URL = "https://api.cobweaverclaw_compresslabs.ai"
...
class CompressionMiddleware:
    ...
    Two modes:
    - Local (default): Compresses in-process using cobweaverclaw_compress.compress().
    - Cloud (api_key set): Calls CoBWeaverClaw Cloud API for managed compression
      with org-scoped CCR, TOIN learning, and analytics dashboards.

    Response headers include compression metrics:
    - x-cobweaverclaw_compress-tokens-before: original token count
    - x-cobweaverclaw_compress-tokens-after: compressed token count
    - x-cobweaverclaw_compress-tokens-saved: tokens removed
    - x-cobweaverclaw_compress-compressed: "true" if compression occurred
    """
    ...
    def __init__(
        ...
        # Cloud mode: if api_key is set, compress via CoBWeaverClaw Cloud API
        self._api_key = api_key or os.environ.get("COBWEAVERCLAW_API_KEY", "").strip() or None
        self._api_url = (
            api_url or os.environ.get("COBWEAVERCLAW_API_URL", "").strip() or _DEFAULT_CLOUD_URL
        ).rstrip("/")
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            if messages:
                ...
                if result and result.get("tokens_saved", 0) > 0 and "messages" in result:
                    ...
                    logger.info(
                        "CoBWeaverClaw%s: %d→%d tokens (saved %d, %.0f%%)",
            ...
            logger.debug("CoBWeaverClaw middleware: skipping non-JSON request: %s", e)
        ...
        async def metrics_send(message: MutableMapping[str, Any]) -> None:
            if message["type"] == "http.response.start" and tokens_saved > 0:
                ...
                headers.append((b"x-cobweaverclaw_compress-compressed", b"true"))
                headers.append((b"x-cobweaverclaw_compress-tokens-before", str(tokens_before).encode()))
                headers.append((b"x-cobweaverclaw_compress-tokens-after", str(tokens_after).encode()))
                headers.append((b"x-cobweaverclaw_compress-tokens-saved", str(tokens_saved).encode()))
    ...
    def _local_compress(self, messages: list[dict], model: str) -> dict[str, Any] | None:
        """Compress locally using cobweaverclaw_compress.compress()."""
    ...
    async def _cloud_compress(self, messages: list[dict], model: str) -> dict[str, Any] | None:
        """Compress via CoBWeaverClaw Cloud API (managed CCR, TOIN, analytics)."""
        if self._client is None:
            try:
                import httpx
            except ImportError as e:
                raise ImportError(
                    "httpx is required for CoBWeaverClaw Cloud mode: pip install httpx"
                ) from e
        ...
        resp = await client.post(
            ...
            headers={
                "X-CoBWeaverClaw-Key": self._api_key,
        ...
        if resp.status_code != 200:
            logger.warning("CoBWeaverClaw Cloud API error: %d %s", resp.status_code, resp.text[:200])
            return None
