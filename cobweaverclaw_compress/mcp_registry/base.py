...
The MCP protocol itself is universal, and the cobweaverclaw_compress MCP server
(``cobweaverclaw_compress mcp serve``) is a single stdio binary that any compliant client
...
The orchestrator in :mod:`cobweaverclaw_compress.mcp_registry.install` calls a fleet of
registrars to install cobweaverclaw_compress across every detected agent.
"""
