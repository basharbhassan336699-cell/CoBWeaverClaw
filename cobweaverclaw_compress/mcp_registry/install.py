"""Top-level orchestration: register CoBWeaverClaw MCP across detected agents."""

from __future__ import annotations

from collections.abc import Iterable

from cobweaverclaw_compress.install.runtime import resolve_cobweaverclaw_compress_command
...
def build_cobweaverclaw_compress_spec(proxy_url: str = DEFAULT_PROXY_URL) -> ServerSpec:
    """Construct the canonical :class:`ServerSpec` for the cobweaverclaw_compress server.
    ...
    if proxy_url and proxy_url != DEFAULT_PROXY_URL:
        env["COBWEAVERCLAW_PROXY_URL"] = proxy_url
    command = resolve_cobweaverclaw_compress_command()
    return ServerSpec(
...
def build_serena_spec(context: str) -> ServerSpec:
    ...
    startup. CoBWeaverClaw installs Serena by default, so without this flag every
...
def install_everywhere(
    ...
    *,
    ...
    """Install the cobweaverclaw_compress MCP server into every detected agent.
