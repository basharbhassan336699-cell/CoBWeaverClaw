"""CoBWeaverClaw-owned MCP install ledger.

The ledger tracks MCP servers that CoBWeaverClaw registered on the user's behalf
when the target agent config cannot carry CoBWeaverClaw-specific ownership markers.
It lets unwrap remove only entries still matching the spec CoBWeaverClaw installed,
...
def ledger_path() -> Path:
    """Return the CoBWeaverClaw MCP install ledger path."""
...
def record_install(agent: str, spec: ServerSpec, *, path: Path | None = None) -> None:
    """Record that CoBWeaverClaw installed ``spec`` for ``agent``."""
...
def cobweaverclaw_compress_installed_matching(
    agent: str,
    current_spec: ServerSpec | None,
    *,
    path: Path | None = None,
) -> bool:
    """Return True when the ledger says CoBWeaverClaw installed ``current_spec``."""
