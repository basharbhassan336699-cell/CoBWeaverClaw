...
changes, cache clears, stats resets) to the dedicated ``cobweaverclaw_compress.audit``
...
``COBWEAVERCLAW_STATELESS`` (no filesystem writes) and respects whatever log sink
...
audit_logger = logging.getLogger("cobweaverclaw_compress.audit")
...
def record_admin_action(
    *,
    ...
    try:
        event: dict[str, Any] = {
            "event": "cobweaverclaw_compress_admin_audit",
            "action": action,
