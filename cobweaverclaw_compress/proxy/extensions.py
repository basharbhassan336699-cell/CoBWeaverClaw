...
External packages hook into the CoBWeaverClaw proxy at startup by declaring an
entry point in the ``cobweaverclaw_compress.proxy_extension`` group in their ``pyproject.toml``:

    [project.entry-points."cobweaverclaw_compress.proxy_extension"]
...
Enabling extensions:
  ...
  * CLI:  ``cobweaverclaw_compress proxy --proxy-extension shield_enterprise,mypkg``
  * Env:  ``COBWEAVERCLAW_PROXY_EXTENSIONS=shield_enterprise,mypkg``
...
ENTRY_POINT_GROUP = "cobweaverclaw_compress.proxy_extension"
ENV_VAR = "COBWEAVERCLAW_PROXY_EXTENSIONS"
...
def _resolve_enabled(enabled: Iterable[str] | None) -> set[str]:
    ...
    Precedence: explicit ``enabled`` argument > ``COBWEAVERCLAW_PROXY_EXTENSIONS``
