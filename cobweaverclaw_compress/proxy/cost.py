"""Cost tracking and budget management for the CoBWeaverClaw proxy.
...
from cobweaverclaw_compress.proxy.modes import PROXY_MODE_CACHE

if TYPE_CHECKING:
    from cobweaverclaw_compress.proxy.prometheus_metrics import PrometheusMetrics
...
def header_safe_transforms(transforms: list[str]) -> list[str]:
    ...
    ``x-cobweaverclaw_compress-transforms`` is built as ``",".join(transforms_applied)``, so a
...
def build_prefix_cache_stats(
    ...
    for provider, pc in metrics.cache_by_provider.items():
        ...
        # client (e.g. Claude Code) pays regardless of CoBWeaverClaw. We track it
    ...
    return {
        ...
        "attribution": (
            ...
            "CoBWeaverClaw reports cache stats as observed from API responses. "
            "CacheAligner and prefix freeze improve cache hit rates by stabilizing "
            "the message prefix, but baseline caching happens without CoBWeaverClaw. "
...
def _aggregate_mcp_events() -> dict[str, int]:
    """Aggregate compression / retrieval events written by CoBWeaverClaw MCP
    server instances to the cross-process shared events file.

    The CoBWeaverClaw MCP server (``cobweaverclaw_compress mcp serve``) records every
    ``cobweaverclaw_compress_compress`` and ``cobweaverclaw_compress_retrieve`` invocation to a
    file-locked shared log (see :func:`cobweaverclaw_compress.ccr.mcp_server._append_shared_event`).
    ...
    Keys: ``compressions`` (count of cobweaverclaw_compress_compress calls),
    ``tokens_removed`` (sum of input_tokens-output_tokens across
    compress events), ``retrievals`` (count of cobweaverclaw_compress_retrieve
    calls — the load-bearing over-compression signal).
    """
    zero = {"compressions": 0, "tokens_removed": 0, "retrievals": 0}
    try:
        from cobweaverclaw_compress.ccr.mcp_server import _read_shared_events
    except ImportError:
        return zero
...
def build_session_summary(
    ...
    "is CoBWeaverClaw working?" at a glance.
    """
    ...
    cost_with = cost_stats.get("cost_with_cobweaverclaw_compress_usd", 0.0)
    compression_savings = cost_stats.get("savings_usd", 0.0)
    ...
    summary: dict[str, Any] = {
        ...
        "cost": {
            "without_cobweaverclaw_compress_usd": round(cost_without, 2),
            "with_cobweaverclaw_compress_usd": round(cost_with, 2),
    ...
    # MCP-side compression: events written by `cobweaverclaw_compress mcp serve`
    ...
    if proxy.config.mode == PROXY_MODE_CACHE and uncompressed_reasons["prefix_frozen"] > 10:
        summary["tip"] = (
            "Most requests are prefix-frozen. Set COBWEAVERCLAW_MODE=token "
...
class CostTracker:
    ...
    def estimate_cost(
        ...
        try:
            from cobweaverclaw_compress.pricing.litellm_pricing import resolve_litellm_model
    ...
    def record_tokens(
        ...
        Args:
            model: Model name.
            tokens_saved: Tokens removed by compression (CoBWeaverClaw's count).
            tokens_sent: Compressed message tokens sent (CoBWeaverClaw's count).
    ...
    def stats(self) -> dict:
        ...
        # data is unavailable.
        cost_with_cobweaverclaw_compress = 0.0
        ...
        for model in self._tokens_saved_by_model:
            ...
            if prices:
                ...
                if cr + cw + uncached > 0:
                    ...
                    billed_tokens = sent
                cost_with_cobweaverclaw_compress += model_cost
        ...
        return {
            ...
            "total_input_cost_usd": round(cost_with_cobweaverclaw_compress, 4),
            ...
            "per_model": per_model,
            "cost_with_cobweaverclaw_compress_usd": round(cost_with_cobweaverclaw_compress, 4),
            "savings_usd": round(savings_usd, 4),
            # Budget config passthrough — surfaces in /stats["cost"] so
            # `cobweaverclaw_compress doctor` can report whether a budget is set.
