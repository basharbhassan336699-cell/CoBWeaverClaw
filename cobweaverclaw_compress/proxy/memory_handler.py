...
This module provides memory capabilities for the CoBWeaverClaw proxy:
...
from cobweaverclaw_compress.memory import qdrant_env
...
if TYPE_CHECKING:
    from cobweaverclaw_compress.memory.backends.local import LocalBackend
...
# Memory tool names for detection (CoBWeaverClaw's custom tools)
MEMORY_TOOL_NAMES = {
    "memory_save",
    "memory_search",
    "memory_update",
...
class MemoryConfig:
    ...
    Qdrant connection fields default to values read from ``COBWEAVERCLAW_QDRANT_*``
    environment variables (see :mod:`cobweaverclaw_compress.memory.qdrant_env`). Passing an
    explicit value to the constructor always wins over the environment.
    """

    enabled: bool = False
    backend: Literal["local", "qdrant-neo4j"] = "local"
    db_path: str = "cobweaverclaw_compress_memory.db"
    ...
    native_memory_dir: str = ""  # Directory for native memory files (default: ~/.cobweaverclaw_compress/memories)
    # Qdrant+Neo4j config (Qdrant defaults resolve from COBWEAVERCLAW_QDRANT_* env vars)
    ...
    # Memory Bridge (bidirectional markdown <-> CoBWeaverClaw sync)
...
class MemoryHandler:
    ...
    Supports two modes:
    - Custom tools: CoBWeaverClaw's memory_save, memory_search, etc. (default)
    ...
    def _init_native_memory_dir(self) -> None:
        ...
        if self.config.native_memory_dir:
            ...
            # Default: workspace memories directory (respects COBWEAVERCLAW_WORKSPACE_DIR)
    ...
    async def _init_backend_locked(self) -> None:
        ...
        if self.config.backend == "local":
            from cobweaverclaw_compress.memory.backends.local import LocalBackend, LocalBackendConfig
            ...
            # Opt-in GPU offload: COBWEAVERCLAW_EMBEDDER_RUNTIME=pytorch_mps routes embedding
            ...
            if os.environ.get("COBWEAVERCLAW_EMBEDDER_RUNTIME", "").strip().lower() == "pytorch_mps":
                try:
                    ...
                    if torch.backends.mps.is_available():
                        ...
                        logger.info(
                            "Memory: COBWEAVERCLAW_EMBEDDER_RUNTIME=pytorch_mps → "
                            "torch embedder on Apple GPU (MPS)"
                        )
                    else:
                        logger.warning(
                            "Memory: COBWEAVERCLAW_EMBEDDER_RUNTIME=pytorch_mps requested but "
            ...
            try:
                from cobweaverclaw_compress.memory.backends.direct_mem0 import (
                    DirectMem0Adapter,
                    Mem0Config,
                )
                ...
                logger.error(
                    ...
                    "Install with: pip install 'cobweaverclaw_compress-ai[memory-stack]'"
                )
                raise
        else:
    ...
    async def _init_and_import_bridge(self) -> None:
        ...
        if self._bridge is not None:
            return
        try:
            from cobweaverclaw_compress.memory.bridge import MemoryBridge
            from cobweaverclaw_compress.memory.bridge_config import BridgeConfig, MarkdownFormat
    ...
    def _get_memory_tools(self) -> list[dict[str, Any]]:
        ...
        if self._memory_tools is None:
            from cobweaverclaw_compress.memory.tools import get_memory_tools_optimized
        ...
        *,
        ...
        Args:
            user_id: User identifier for memory scoping (the base user
                id, derived from ``x-cobweaverclaw_compress-user-id`` upstream).
            ...
            ranker: Optional :class:`~cobweaverclaw_compress.proxy.memory_ranker.MemoryRanker`
        ...
        """
        from cobweaverclaw_compress.proxy.memory_injection import MemoryInjectionBudget
        ...
        # but no x-cobweaverclaw_compress-project-id / x-cobweaverclaw_compress-cwd / system-prompt
        ...
        try:
            ...
            if ranker is not None:
                from cobweaverclaw_compress.proxy.memory_ranker import MemoryCandidate
    ...
    def _append_to_latest_user_tail(
        ...
        *,
        ...
        if provider == "anthropic":
            ...
            # cobweaverclaw_compress.proxy.handlers.anthropic which imports MemoryHandler.
        ...
        if provider == "openai":
            from cobweaverclaw_compress.proxy.helpers import append_text_to_latest_user_chat_message
