"""Data models for the CoBWeaverClaw proxy.
...
from cobweaverclaw_compress.providers.registry import ProviderApiOverrides
...
class RequestLog:
    ...
    # See ``cobweaverclaw_compress.proxy.helpers.compute_turn_id`` for the derivation. None
...
class ProxyConfig:
    ...
    # CLI: --disable-kompress; env: COBWEAVERCLAW_DISABLE_KOMPRESS=1.
    ...
    # CLI: --disable-kompress-fallback; env: COBWEAVERCLAW_DISABLE_KOMPRESS_FALLBACK=1.
    ...
    # (and -openai); env: COBWEAVERCLAW_DISABLE_KOMPRESS_ANTHROPIC / _OPENAI
    # (1 = disable, 0 = enable).
    ...
    # CLI: --force-kompress-all; env: COBWEAVERCLAW_FORCE_KOMPRESS_ALL=1.
    force_kompress_all: bool = False

    lossless: bool = False  # CLI: --lossless; env: COBWEAVERCLAW_LOSSLESS=1. No-CCR mode: compress without any retrieval marker.
    ...
    # CLI: --compress-user-messages; env: COBWEAVERCLAW_COMPRESS_USER_MESSAGES=1.
    compress_user_messages: bool = False
    # Named savings policy shared across Claude/Codex/Cursor proxy handlers.
    # CLI/env: COBWEAVERCLAW_SAVINGS_PROFILE=agent-90.
    ...
    # CLI: --exclude-tools <name1,name2>; env: COBWEAVERCLAW_EXCLUDE_TOOLS=<name1,name2>
    ...
    # CLI: --protect-tool-results <name1,name2>; env: COBWEAVERCLAW_PROTECT_TOOL_RESULTS=<name1,name2>
    ...
    # env: COBWEAVERCLAW_READ_MATURATION=1
    ...
    # env: COBWEAVERCLAW_READ_MATURATION_QUIESCE_TURNS / _MAX_HOLD_TURNS / _MIN_SIZE_BYTES.
    ...
    # to enable from the `cobweaverclaw_compress.proxy_extension` group, or `["*"]` for
    # wildcard. Empty/None means no extensions run, even if installed.
    # CLI: --proxy-extension <name1,name2>; env: COBWEAVERCLAW_PROXY_EXTENSIONS.
    ...
    memory_db_path: str = ""  # Empty = auto: {cwd}/.cobweaverclaw_compress/memory.db
    ...
    # x-cobweaverclaw_compress-user-id only. ``global`` keeps the pre-fix single-DB
    ...
    # Qdrant connection (defaults resolve from COBWEAVERCLAW_QDRANT_* env vars)
    ...
    license_cloud_url: str = "https://app.cobweaverclaw_compresslabs.ai"
    ...
    # Env: COBWEAVERCLAW_PERIODIC_TOIN_STATS=0.
    ...
    # ``X-CoBWeaverClaw-Proxy-Token`` header). Loopback callers are exempt. Closes the
    # gap where a container bound to 0.0.0.0 exposes unauthenticated /v1/* routes
    # to the pod network. Env: COBWEAVERCLAW_PROXY_TOKEN.
    ...
    # downloads) for fully offline / regulated deployments. Env: COBWEAVERCLAW_OFFLINE=1.
    offline: bool = False
    ...
    # ``COBWEAVERCLAW_COMPRESSION_MAX_WORKERS``.
    #
    ...
    # ``CoBWeaverClawProxy._run_compression_in_executor``.
