"""Streaming handler mixin for CoBWeaverClawProxy.
...
from cobweaverclaw_compress.copilot_auth import apply_copilot_api_auth
...
class StreamingMixin:
    """Mixin providing streaming response methods for CoBWeaverClawProxy."""
    ...
    def _queue_mid_turn_message(self, session_key: str, body: dict) -> dict:
        ...
        return {"status": 202, "event": "cobweaverclaw_compress_queued"}
    ...
    def _parse_sse_usage(self, chunk: bytes, provider: str) -> dict[str, int] | None:
        ...
        """
        from cobweaverclaw_compress.proxy.helpers import parse_sse_events_from_byte_buffer
    ...
    def _record_ccr_feedback_from_response(
        ...
        """Extract cobweaverclaw_compress_retrieve tool calls from a response and record feedback.
        ...
        for block in content:
            ...
            if block.get("type") != "tool_use":
                continue
            if block.get("name") != "cobweaverclaw_compress_retrieve":
                continue
    ...
    def _record_ccr_feedback_from_openai_sse(self, full_sse_data: str, request_id: str) -> None:
        """Record cobweaverclaw_compress_retrieve feedback from OpenAI Chat Completions SSE.
        ...
        ``cobweaverclaw_compress_retrieve`` invocation as a no-op store call for the
        ...
        for slot in tool_calls.values():
            if slot["name"] != "cobweaverclaw_compress_retrieve":
                continue
            try:
        ...
        *,
        ...
        async def generate():
            ...
            try:
                ...
                # CCR Feedback: Record cobweaverclaw_compress_retrieve tool calls for TOIN learning.
                ...
                if pending_messages:
                    pending_event = json.dumps(
                        {"type": "cobweaverclaw_compress_pending_messages", "messages": pending_messages}
                    )
                    yield f"event: cobweaverclaw_compress_pending_messages\ndata: {pending_event}\n\n".encode()
        ...
        async def generate():
            try:
                ...
                async for event in self.anthropic_backend.stream_message(body, headers):
                    ...
                    # the token count CoBWeaverClaw actually sent upstream
    ...
    async def _stream_openai_via_backend(
        ...
        from cobweaverclaw_compress.proxy.handlers.openai import _infer_openai_cache_write_tokens
        from cobweaverclaw_compress.proxy.outcome import RequestOutcome
        ...
        async def generate():
            ...
            try:
                ...
                # CCR Feedback: record cobweaverclaw_compress_retrieve tool calls so
