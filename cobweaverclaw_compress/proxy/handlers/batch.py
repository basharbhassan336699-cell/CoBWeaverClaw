"""Batch handler mixin for CoBWeaverClawProxy.
...
class BatchHandlerMixin:
    """Mixin providing batch API handler methods for CoBWeaverClawProxy."""
    ...
    async def handle_google_batch_create(
        ...
        _pre_strip_count_gb = sum(1 for k in headers if k.lower().startswith("x-cobweaverclaw_compress-"))
    ...
    async def _google_batch_passthrough(
        ...
        _pre_strip_count_gpt = sum(1 for k in headers if k.lower().startswith("x-cobweaverclaw_compress-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
        ...
        # has been synthesized by CoBWeaverClaw and is canonically serialized.
        ...
        from cobweaverclaw_compress.proxy.helpers import get_python_forwarder_mode
    ...
    async def handle_google_batch_passthrough(
        ...
        _pre_strip_count_gp = sum(1 for k in headers if k.lower().startswith("x-cobweaverclaw_compress-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_google_batch_results(
        ...
        _pre_strip_count_gbr = sum(1 for k in headers if k.lower().startswith("x-cobweaverclaw_compress-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_batch_create(self, request: Request) -> Response:
        ...
        _pre_strip_count_oacc = sum(1 for k in headers if k.lower().startswith("x-cobweaverclaw_compress-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
        ...
        try:
            ...
            compression_metadata = {
                **metadata,
                "cobweaverclaw_compress_compressed": "true",
                "cobweaverclaw_compress_original_file_id": input_file_id,
                "cobweaverclaw_compress_total_requests": str(stats["total_requests"]),
                "cobweaverclaw_compress_tokens_saved": str(stats["total_tokens_saved"]),
                "cobweaverclaw_compress_original_tokens": str(stats["total_original_tokens"]),
                "cobweaverclaw_compress_compressed_tokens": str(stats["total_compressed_tokens"]),
                "cobweaverclaw_compress_savings_percent": f"{stats['savings_percent']:.1f}",
            }
            ...
            # batch_body is synthesized by CoBWeaverClaw (compressed file_id +
            ...
            response_headers.pop("content-encoding", None)
            response_headers.pop("content-length", None)
            response_headers["x-cobweaverclaw_compress-tokens-saved"] = str(stats["total_tokens_saved"])
            response_headers["x-cobweaverclaw_compress-savings-percent"] = f"{stats['savings_percent']:.1f}"
    ...
    async def _batch_passthrough(self, request: Request, body: dict) -> Response:
        ...
        headers.pop("content-length", None)
        # PR-A5 (P5-49): strip internal x-cobweaverclaw_compress-* before forwarding upstream.
        _pre_strip_count_obp = sum(1 for k in headers if k.lower().startswith("x-cobweaverclaw_compress-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
