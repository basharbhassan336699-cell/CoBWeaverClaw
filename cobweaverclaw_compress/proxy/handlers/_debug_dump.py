...
def _debug_dump_mode(config: Any) -> str:
    ...
    - "redacted" : structure, roles, and lengths only — content elided
                   (``COBWEAVERCLAW_DEBUG_DUMP=1``/``true``/``on``/``redacted``)
    - "full"     : everything including content (``COBWEAVERCLAW_DEBUG_DUMP=full``)
    """
    if getattr(config, "stateless", False):
        return "off"
    raw = os.environ.get("COBWEAVERCLAW_DEBUG_DUMP", "").strip().lower()
