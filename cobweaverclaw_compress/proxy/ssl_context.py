"""SSL context builder for the CoBWeaverClaw upstream httpx client.
...
Strict-mode toggle (``COBWEAVERCLAW_TLS_STRICT``):
    ...
    Setting ``COBWEAVERCLAW_TLS_STRICT=0`` clears *only* ``VERIFY_X509_STRICT`` from
    every TLS context CoBWeaverClaw controls (the httpx upstream client AND the
...
TLS_STRICT_ENV = "COBWEAVERCLAW_TLS_STRICT"
...
def tls_strict_disabled() -> bool:
    """True when ``COBWEAVERCLAW_TLS_STRICT`` opts out of OpenSSL strict mode.
...
def _relax_x509_strict_for_custom_ca(ctx: ssl.SSLContext, *, path: str) -> ssl.SSLContext:
    ...
    strict flag is relaxed here regardless of ``COBWEAVERCLAW_TLS_STRICT`` (the
...
def _default_strict_relaxed_context() -> ssl.SSLContext:
    ...
    the OS/default trust store) but ``COBWEAVERCLAW_TLS_STRICT=0`` asks us to
...
def build_httpx_verify() -> ssl.SSLContext | bool:
    ...
    2. No bundle, but ``COBWEAVERCLAW_TLS_STRICT=0`` → the default trust store with
...
def apply_global_tls_relaxation() -> bool:
    ...
    When ``COBWEAVERCLAW_TLS_STRICT=0`` this monkeypatches
    ...
    if getattr(_u3ssl.create_urllib3_context, "_cobweaverclaw_compress_strict_relaxed", False):
        return True
    ...
    _relaxed_create_urllib3_context._cobweaverclaw_compress_strict_relaxed = True  # type: ignore[attr-defined]
