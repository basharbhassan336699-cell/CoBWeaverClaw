"""Top-level helper functions and constants for the CoBWeaverClaw proxy.
...
from cobweaverclaw_compress._subprocess import run
...
_CODEX_WIRE_DEBUG_ENV = "COBWEAVERCLAW_CODEX_WIRE_DEBUG"
_CODEX_WIRE_DEBUG_DIR_ENV = "COBWEAVERCLAW_CODEX_WIRE_DEBUG_DIR"
...
#     forwards untouched.
#
# Configurable via COBWEAVERCLAW_MEMORY_INJECTION_MODE env var. There is no
...
_MEMORY_INJECTION_MODE_ENV = "COBWEAVERCLAW_MEMORY_INJECTION_MODE"
...
_PYTHON_FORWARDER_MODE_ENV = "COBWEAVERCLAW_PROXY_PYTHON_FORWARDER_MODE"
...
def extract_tags(headers: Any) -> dict[str, str]:
    """Extract ``x-cobweaverclaw_compress-*`` tags from inbound headers.
    ...
    ``x-cobweaverclaw_compress-`` prefix stripped.
    """
    return {
        k.lower().replace("x-cobweaverclaw_compress-", ""): v
        for k, v in headers.items()
        if k.lower().startswith("x-cobweaverclaw_compress-")
    }


def _cobweaverclaw_compress_bypass_enabled(headers: Any) -> bool:
    """Return True when inbound headers request full CoBWeaverClaw passthrough.
    ...
    try:
        bypass = str(headers.get("x-cobweaverclaw_compress-bypass", "")).strip().lower() == "true"
        passthrough = str(headers.get("x-cobweaverclaw_compress-mode", "")).strip().lower() == "passthrough"
...
_RTK_GAIN_SCOPE_ENV = "COBWEAVERCLAW_RTK_GAIN_SCOPE"
...
RTK_STATS_CACHE_TTL_SECONDS = float(os.environ.get("COBWEAVERCLAW_CONTEXT_TOOL_STATS_TTL_SECONDS", "60"))
...
# COBWEAVERCLAW_SSE_BUFFER_MAX_BYTES. Guards against pathological huge events
...
_SSE_EVENT_MAX_BYTES_ENV = "COBWEAVERCLAW_SSE_BUFFER_MAX_BYTES"
...
# Configurable via COBWEAVERCLAW_PROXY_BODY_TOO_LARGE_STATUS for operators who need
# to override (no expected production use; documentation knob).
_BODY_TOO_LARGE_STATUS_ENV = "COBWEAVERCLAW_PROXY_BODY_TOO_LARGE_STATUS"
...
# COBWEAVERCLAW_COMPRESSION_TIMEOUT_SECONDS env var for slow CPUs or long Claude Code
# conversations (GH #946). Falls back to 30 on an unparseable value.
try:
    COMPRESSION_TIMEOUT_SECONDS = float(
        os.environ.get("COBWEAVERCLAW_COMPRESSION_TIMEOUT_SECONDS", "30")
    )
except ValueError:
...
# COBWEAVERCLAW_EAGER_PRELOAD_TIMEOUT_SECONDS. Falls back to 120 on an unparseable value.
try:
    EAGER_PRELOAD_TIMEOUT_SECONDS = float(
        os.environ.get("COBWEAVERCLAW_EAGER_PRELOAD_TIMEOUT_SECONDS", "120")
    )
except ValueError:
...
# CoBWeaverClaw thought was large enough to compress, the legacy behaviour was to
...
# upstream (which CoBWeaverClaw's earlier successful compressions shrunk), then
...
# ``COBWEAVERCLAW_WS_FAIL_OPEN_ON_COMPRESSION_FAILURE=1``. The oversize threshold
# below which transient errors still fall through to passthrough is
# configurable via ``COBWEAVERCLAW_WS_COMPRESSION_FAIL_THRESHOLD_BYTES``
# (default 256 KiB ≈ 64K tokens).
WS_COMPRESSION_FAIL_OPEN_ENV = "COBWEAVERCLAW_WS_FAIL_OPEN_ON_COMPRESSION_FAILURE"
WS_COMPRESSION_OVERSIZE_BYTES_ENV = "COBWEAVERCLAW_WS_COMPRESSION_FAIL_THRESHOLD_BYTES"
...
def decide_compression_failure_action(
    ...
    * Codex client compression timeout → forward. Codex currently treats
      ...
      standalone rather than through ``cobweaverclaw_compress wrap codex``.
    * exception is :class:`asyncio.TimeoutError` → refuse (the compression
      stage hit its own timeout, which only fires on frames CoBWeaverClaw
...
def _get_image_compressor():
    ...
    try:
        from cobweaverclaw_compress.image import ImageCompressor
...
# Always-on file logging to the workspace logs directory for `cobweaverclaw_compress perf` analysis.
# Resolved lazily so COBWEAVERCLAW_WORKSPACE_DIR env-var changes are honored.


def _cobweaverclaw_compress_log_dir() -> Path:
    return _paths.log_dir()


def _setup_file_logging() -> None:
    """Add a RotatingFileHandler to the cobweaverclaw_compress root logger.

    Writes to ~/.cobweaverclaw_compress/logs/proxy.log with automatic rotation:
    - Rotates at 10 MB
    ...
    try:
        log_dir = _cobweaverclaw_compress_log_dir()
        ...
        # Attach to the cobweaverclaw_compress root logger so all sub-loggers are captured.
        # Disable propagation to root to avoid duplicate writes when
        # wrap.py redirects stderr to the same log file.
        cobweaverclaw_compress_logger = logging.getLogger("cobweaverclaw_compress")
        cobweaverclaw_compress_logger.setLevel(logging.INFO)
        if not any(isinstance(h, RotatingFileHandler) for h in cobweaverclaw_compress_logger.handlers):
            cobweaverclaw_compress_logger.addHandler(handler)
        cobweaverclaw_compress_logger.propagate = False
    except OSError:
...
def _context_tool_summary_payload(
    *,
    ...
    CoBWeaverClaw computes session values by subtracting a startup baseline, so
...
def _get_context_tool_stats() -> dict[str, Any] | None:
    """Get context-tool savings for the current CoBWeaverClaw proxy session.
...
# ---------------------------------------------------------------------------
#
# `x-cobweaverclaw_compress-*` request headers (e.g. ``x-cobweaverclaw_compress-bypass``,
# ``x-cobweaverclaw_compress-mode``, ``x-cobweaverclaw_compress-user-id``, ``x-cobweaverclaw_compress-stack``,
# ``x-cobweaverclaw_compress-base-url``) are internal control flags consumed by the
...
# ``x-cobweaverclaw_compress-*``, memory ``x-cobweaverclaw_compress-user-id`` lookup) keep using
# the original dict / ``request.headers``. The stripped copy is what
# every upstream-bound forwarder receives.
#
# Note: response-side ``X-CoBWeaverClaw-*`` injection (e.g.
# ``x-cobweaverclaw_compress-tokens-saved``) is unrelated — the proxy is allowed to
# tell its client about its own work. This helper only filters
# request-side headers.

_INTERNAL_HEADER_PREFIX = "x-cobweaverclaw_compress-"

# Operator opt-in env var. ``enabled`` (default) strips internal
# ``x-cobweaverclaw_compress-*`` headers from every upstream-bound forwarder.
...
_STRIP_INTERNAL_HEADERS_ENV = "COBWEAVERCLAW_STRIP_INTERNAL_HEADERS"
...
def _strip_internal_headers(headers: dict[str, str]) -> dict[str, str]:
    """Return a copy of ``headers`` with internal ``x-cobweaverclaw_compress-*`` keys stripped.

    Used at every upstream call site to prevent fingerprinting / leakage of
    internal flags like ``x-cobweaverclaw_compress-bypass``, ``x-cobweaverclaw_compress-mode``,
    ``x-cobweaverclaw_compress-user-id``, ``x-cobweaverclaw_compress-stack``, ``x-cobweaverclaw_compress-base-url``.
    Case-insensitive on the prefix. Returns a NEW dict; never mutates the
    caller's mapping. Pure function. No regex.

    When the operator opt-in ``COBWEAVERCLAW_STRIP_INTERNAL_HEADERS=disabled``
...
#      client value already contained a CoBWeaverClaw-required token.
#
...
#     then CoBWeaverClaw-required tokens (in the order passed). Dedupe is
...
#
# Operator opt-in `COBWEAVERCLAW_BETA_HEADER_STICKY=disabled` short-circuits
...
_BETA_HEADER_STICKY_ENV = "COBWEAVERCLAW_BETA_HEADER_STICKY"
BetaHeaderStickyMode = Literal["enabled", "disabled"]
_BETA_HEADER_STICKY_DEFAULT: BetaHeaderStickyMode = "enabled"

_BETA_TRACKER_MAX_SESSIONS_ENV = "COBWEAVERCLAW_BETA_TRACKER_MAX_SESSIONS"
...
def _merge_beta_tokens(client_value: str | None, cobweaverclaw_compress_required: list[str]) -> str:
    ...
    * CoBWeaverClaw-required tokens append in the order given, skipping any
    ...
    * Returns ``""`` when both inputs are empty.
    ...
    for token in _split_beta_tokens(client_value):
        ...
        out.append(token)
    for token in cobweaverclaw_compress_required:
        if not token:
            continue
        token = token.strip()
        if not token:
...
def merge_anthropic_beta(client_value: str | None, cobweaverclaw_compress_required: list[str]) -> str:
    """Merge client `anthropic-beta` value with CoBWeaverClaw-required tokens.

    See `_merge_beta_tokens` for full semantics. Order is deterministic:
    client tokens first (in their original order), then cobweaverclaw_compress tokens
    ...
    Returns ``""`` when both inputs are empty.
    """
    return _merge_beta_tokens(client_value, cobweaverclaw_compress_required)


def merge_openai_beta(client_value: str | None, cobweaverclaw_compress_required: list[str]) -> str:
    """Merge client `OpenAI-Beta` value with CoBWeaverClaw-required tokens.
...
class SessionBetaTracker:
    ...
    def record_and_get_sticky_betas(
        ...
        When `COBWEAVERCLAW_BETA_HEADER_STICKY=disabled` returns the client
...
def get_session_beta_tracker() -> SessionBetaTracker:
    ...
    Lazily constructed so the env-var bound (`COBWEAVERCLAW_BETA_TRACKER_MAX_SESSIONS`)
...
def log_beta_header_merge(
    *,
    ...
    cobweaverclaw_compress_added: list[str],
    request_id: str | None,
) -> None:
    """Structured log for every cache-affecting beta-header merge.

    `cobweaverclaw_compress_added` is a list of public, documented beta tokens
    ...
    CoBWeaverClaw logs. Emitting counts only makes the decision auditable.
    """
    logger.info(
        "event=beta_header_merge provider=%s session_id=%s "
        "client_betas=%d sticky_betas=%d cobweaverclaw_compress_added=%s request_id=%s",
        provider,
        session_id or "",
        client_betas_count,
        sticky_betas_count,
        ",".join(cobweaverclaw_compress_added) if cobweaverclaw_compress_added else "",
        request_id or "",
    )
...
#
# Operator opt-in `COBWEAVERCLAW_TOOL_INJECTION_STICKY=disabled` short-
...
_TOOL_INJECTION_STICKY_ENV = "COBWEAVERCLAW_TOOL_INJECTION_STICKY"
ToolInjectionStickyMode = Literal["enabled", "disabled"]
_TOOL_INJECTION_STICKY_DEFAULT: ToolInjectionStickyMode = "enabled"

_TOOL_TRACKER_MAX_SESSIONS_ENV = "COBWEAVERCLAW_TOOL_TRACKER_MAX_SESSIONS"
...
def serialize_tool_definition_canonical(tool_definition: dict[str, Any]) -> bytes:
    ...
    ``cobweaverclaw_compress/proxy/memory_handler.py`` and
    ``cobweaverclaw_compress/proxy/memory_tool_adapter.py`` already do.
...
def get_session_tool_tracker() -> SessionToolTracker:
    ...
    (`COBWEAVERCLAW_TOOL_TRACKER_MAX_SESSIONS`) is honored at first use.
...
def apply_session_sticky_memory_tools(
    *,
    ...
    Logic (guide §6.3 #2):
      ...
      * If ``COBWEAVERCLAW_TOOL_INJECTION_STICKY=disabled``: bypass tracker,
...
# `cobweaverclaw_compress_retrieve` tool stays registered in `body["tools"]` for every
...
class SessionCcrTracker:
    ...
    Two pieces of state per session:
      ...
      * ``golden_tool_bytes``: canonical serialization of the
        ``cobweaverclaw_compress_retrieve`` tool definition recorded the first time
...
def has_new_ccr_markers(
    *,
    ...
    if not previous_forwarded_messages:
        ...
        return True
    from cobweaverclaw_compress.ccr.tool_injection import CCRToolInjector
...
def should_inject_ccr_tool(
    *,
    ...
    """Decide whether the ``cobweaverclaw_compress_retrieve`` tool must be injected this turn.
...
def apply_session_sticky_ccr_tool(
    *,
    ...
    """
    from cobweaverclaw_compress.ccr.tool_injection import CCR_TOOL_NAME, create_ccr_tool_definition
...
# ``cobweaverclaw_compress wrap claude``). For users who run ``claude`` manually we cannot
...
def format_tool_search_disabled_hint(tools: list[Any]) -> str:
    ...
    return (
        ...
        "or launch via `cobweaverclaw_compress wrap claude` (which sets it automatically). "
        "See https://github.com/chopratejas/cobweaverclaw_compress/issues/746"
    )
