...
OpenAI chat, Gemini) never gated on ``x-cobweaverclaw_compress-bypass``, so
...
Precedence (highest first):
  ...
  4. ``mode_disabled`` — operator COBWEAVERCLAW_MEMORY_INJECTION_MODE=disabled
  5. ``mode_tool`` — operator COBWEAVERCLAW_MEMORY_INJECTION_MODE=tool
...
from cobweaverclaw_compress.proxy.helpers import _cobweaverclaw_compress_bypass_enabled
...
class MemoryDecision:
    ...
    #   * "bypass_header"  — user set x-cobweaverclaw_compress-bypass/x-cobweaverclaw_compress-mode
    #   * "no_handler"     — no memory backend configured on the proxy
    #   * "no_user_id"     — per-request user_id missing
    #   * "mode_disabled"  — operator COBWEAVERCLAW_MEMORY_INJECTION_MODE=disabled
    #   * "mode_tool"      — operator COBWEAVERCLAW_MEMORY_INJECTION_MODE=tool
    ...
    def decide(
        ...
        *,
        ...
        headers
            ...
            Bypass detected via ``_cobweaverclaw_compress_bypass_enabled``.
        memory_handler
            The proxy's memory handler instance, or ``None`` if no
            memory backend is configured. Presence only is checked —
            no methods called.
        memory_user_id
            Per-request user_id from ``x-cobweaverclaw_compress-user-id`` header (or
        ...
        mode_name
            ...
            ``COBWEAVERCLAW_MEMORY_INJECTION_MODE``.
        """
        bypass = _cobweaverclaw_compress_bypass_enabled(headers)
