...
``cobweaverclaw_compress wrap`` launches agents with an ``X-CoBWeaverClaw-Project`` header
...
from cobweaverclaw_compress.proxy.savings_tracker import sanitize_project_name

PROJECT_HEADER = "x-cobweaverclaw_compress-project"
PROJECT_PATH_PREFIX = "/p/"

_current_project: ContextVar[str | None] = ContextVar("cobweaverclaw_compress_current_project", default=None)
...
def classify_project(headers: Mapping[str, Any] | Any) -> str | None:
    ...
    if get is None:
        return None
    value = get(PROJECT_HEADER) or get("X-CoBWeaverClaw-Project")
...
def with_project_prefix(base_url: str, project: str | None) -> str:
    ...
    ``cobweaverclaw_compress wrap`` for clients that cannot send custom headers.
