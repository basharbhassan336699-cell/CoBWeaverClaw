...
:meth:`CoBWeaverClawProxy._record_request_outcome`), which owns the four
...
class RequestOutcome:
    ...
    # Response-cache hit (CoBWeaverClaw's own semantic cache served the
    ...
    #     via :func:`cobweaverclaw_compress.proxy.auth_mode.classify_client`. The
    ...
    def cache_hit(self) -> bool:
        ...
        was served from CoBWeaverClaw's own response cache.
...
async def emit_request_outcome(handler: Any, outcome: RequestOutcome) -> None:
    ...
    Owns the four downstream effects in canonical order:
      ...
      4. structured PERF log line — consumed by ``cobweaverclaw_compress perf``

    Takes the handler as a free argument rather than ``self`` so this
    function is callable from:
    * ``CoBWeaverClawProxy._record_request_outcome`` (production)
    ...
    * any provider handler mixin
    ...
    """
    from cobweaverclaw_compress.proxy.cost import _summarize_transforms
    from cobweaverclaw_compress.proxy.models import RequestLog
    from cobweaverclaw_compress.proxy.project_context import get_current_project
    ...
    # HTTP middleware / WS accept captured from ``X-CoBWeaverClaw-Project``.
    ...
    #    line unchanged, and gives ``cobweaverclaw_compress perf --client X``
