...
Direct port of ``crates/cobweaverclaw_compress-core/src/auth_mode.rs``. The two
...
class AuthMode(str, enum.Enum):
    """Three auth-mode classes CoBWeaverClaw routes compression policy through.
    ...
    #: ``accept-encoding``, never strip; never inject ``X-CoBWeaverClaw-*``;
...
# short normalized client name. The dashboard / `cobweaverclaw_compress perf` use
...
CLIENT_UA_MAP: tuple[tuple[str, str], ...] = (
    ...
    # explicitly (see `cobweaverclaw_compress/integrations/strands/README.md`).
...
def classify_client(headers: Mapping[str, Any] | Any, *, default: str | None = None) -> str | None:
    ...
    1. **``X-Client`` header** (explicit override) — clients that
       know they're talking to CoBWeaverClaw can self-identify with a
