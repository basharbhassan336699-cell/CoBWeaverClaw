...
This is the input-side analog of :class:`cobweaverclaw_compress.proxy.outcome.RequestOutcome`.
...
``x-cobweaverclaw_compress-bypass: true`` requests were silently ignored on every
...
class CompressionDecision:
    ...
    #   * ``"bypass_header"``      — user set x-cobweaverclaw_compress-bypass or
    #                                x-cobweaverclaw_compress-mode=passthrough
    ...
    def decide(
        ...
        *,
        ...
        headers
            ...
            Both ``x-cobweaverclaw_compress-bypass: true`` and
            ``x-cobweaverclaw_compress-mode: passthrough`` trigger bypass — semantics
            mirrored from :func:`cobweaverclaw_compress.proxy.helpers._cobweaverclaw_compress_bypass_enabled`.
        config
            ``CoBWeaverClawConfig``-shaped object; only ``optimize: bool``
            is read.
        usage_reporter
