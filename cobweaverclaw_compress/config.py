"""Configuration models for CoBWeaverClaw SDK."""
...
from cobweaverclaw_compress.models.config import ML_MODEL_DEFAULTS


class CoBWeaverClawMode(str, Enum):
    """Operating modes for CoBWeaverClaw."""
...
class RelevanceScorerConfig:
    ...
    For full hybrid support, install: pip install cobweaverclaw_compress[relevance]
...
# COBWEAVERCLAW_PROTECT_TOOL_RESULTS=Bash or --protect-tool-results Bash.
...
class ReadMaturationConfig:
    ...
    Motivation (measured by `cobweaverclaw_compress audit-reads`): the median Read stays
...
class CacheOptimizerConfig:
    ...
    auto_detect_provider: bool = True  # Auto-detect from CoBWeaverClawClient provider
...
class CCRConfig:
    ...
    4. RETRIEVE: If LLM needs more, it calls cobweaverclaw_compress_retrieve(hash) to get the full original back
    ...
    # with Rust DEFAULT_TTL (crates/cobweaverclaw_compress-core/src/ccr/mod.rs) and
    ...
    inject_tool: bool = True  # Inject cobweaverclaw_compress_retrieve tool into tools array
...
class PrefixFreezeConfig:
    ...
    CoBWeaverClaw from invalidating the provider's prefix cache (which would replace
...
@dataclass
class CoBWeaverClawConfig:
    """Main configuration for CoBWeaverClawClient."""

    store_url: str = "sqlite:///cobweaverclaw_compress.db"
    default_mode: CoBWeaverClawMode = CoBWeaverClawMode.AUDIT
    ...
    # Env var COBWEAVERCLAW_INTERCEPT_ENABLED=1 also enables (for CLI `--intercept-tool-results`).
...
class DiffArtifact:
    ...
    Opt-in via CoBWeaverClawConfig.generate_diff_artifact = True.
