...
# Load the UTF-8-forcing subprocess wrapper WITHOUT importing the `cobweaverclaw_compress`
# package. The release workflow runs this file as a bare script
# (`python cobweaverclaw_compress/release_version.py`), where `sys.path[0]` is `cobweaverclaw_compress/`
# rather than the repo root, so `from cobweaverclaw_compress._subprocess import run` fails
# with `ModuleNotFoundError: No module named 'cobweaverclaw_compress'` — and even when it
# resolves, it drags in `cobweaverclaw_compress/__init__.py` (the Rust `_core` import), which
...
try:  # normal package context (tests import `cobweaverclaw_compress.release_version`)
    ...
    _spec = _ilu.spec_from_file_location(
        "_cobweaverclaw_compress_subprocess", Path(__file__).resolve().parent / "_subprocess.py"
    )
