"""One-function compression API for CoBWeaverClaw.

The simplest way to use CoBWeaverClaw — no proxy, no config, just compress:

    from cobweaverclaw_compress import compress
...
class CompressConfig:
    ...
    Pass to ``compress()`` or any integration that uses cobweaverclaw_compress.
...
def compress(
    ...
    **kwargs: Any,
    ...
    """Compress messages using CoBWeaverClaw's full compression pipeline.

    This is the simplest way to use CoBWeaverClaw. No proxy, no config needed.
...
def compress_spreadsheet(
    ...
    **kwargs: Any,
    ...
    (``pip install cobweaverclaw_compress-ai[spreadsheet]``).
    ...
    """
    from cobweaverclaw_compress.transforms.spreadsheet_ingest import load_spreadsheet
...
def _get_pipeline() -> Any:
    ...
    with _pipeline_lock:
        ...
        logger.debug("CoBWeaverClaw compression pipeline initialized")
        return _pipeline
