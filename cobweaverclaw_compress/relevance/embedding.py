"""Embedding-based relevance scorer for CoBWeaverClaw SDK.
...
Install with: pip install cobweaverclaw_compress[relevance]
...
# Set COBWEAVERCLAW_HF_PIN=off to bypass (mirrors cobweaverclaw_compress.onnx_runtime pinning).
...
def _pinned_revision(model_name: str) -> str | None:
    ...
    Returns ``None`` for custom models or when ``COBWEAVERCLAW_HF_PIN=off``.
    """
    if os.environ.get("COBWEAVERCLAW_HF_PIN", "").strip().lower() in ("off", "0", "false", "no"):
        return None
...
class EmbeddingScorer(RelevanceScorer):
    ...
    Note:
        Requires fastembed: pip install cobweaverclaw_compress[relevance]
    """
    ...
    def _get_model(self) -> TextEmbedding:
        ...
        if not self.is_available():
            raise RuntimeError(
                "EmbeddingScorer requires fastembed. Install with: pip install cobweaverclaw_compress[relevance]"
            )
