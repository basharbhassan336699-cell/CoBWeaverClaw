"""Plugin registry for cobweaverclaw_compress learn.

Discovers built-in plugins from cobweaverclaw_compress.learn.plugins.* and external
plugins registered via the ``cobweaverclaw_compress.learn_plugin`` entry point group.

Follows the same pattern as cobweaverclaw_compress.storage_backend (storage/__init__.py).
"""
...
def _discover() -> dict[str, LearnPlugin]:
    ...
    # 1. Built-in: scan cobweaverclaw_compress.learn.plugins.* submodules
    from cobweaverclaw_compress.learn import plugins as plugins_pkg

    for _, mod_name, _ in pkgutil.iter_modules(plugins_pkg.__path__):
        try:
            mod = importlib.import_module(f"cobweaverclaw_compress.learn.plugins.{mod_name}")
    ...
    # 2. External: entry_points(group="cobweaverclaw_compress.learn_plugin")
    try:
        from importlib.metadata import entry_points

        for ep in entry_points(group="cobweaverclaw_compress.learn_plugin"):
            try:
                obj = ep.load()
