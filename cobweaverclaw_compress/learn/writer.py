...
# Marker delimiters for CoBWeaverClaw-managed sections
_MARKER_START = "<!-- cobweaverclaw_compress:learn:start -->"
_MARKER_END = "<!-- cobweaverclaw_compress:learn:end -->"
...
def _build_section(recommendations: list[Recommendation]) -> str:
    ...
    lines = [
        _MARKER_START,
        "## CoBWeaverClaw Learned Patterns",
        f"*Auto-generated by `cobweaverclaw_compress learn` on {now} — do not edit manually*",
        "",
    ]
...
def extract_marker_block(file_content: str) -> str | None:
    """Return the raw text of the cobweaverclaw_compress:learn marker block, or None.
...
def _strip_marker_block(content: str) -> str:
    """Remove the cobweaverclaw_compress:learn marker block from text, tidying blank lines.
...
class ClaudeCodeWriter(ContextWriter):
    ...
    def write(
        ...
        if context_recs:
            ...
            # cobweaverclaw_compress versions into the new target, then strip it from CLAUDE.md
    ...
    def _migrate_legacy_block(
        ...
        """Move a stale cobweaverclaw_compress block out of CLAUDE.md into the new target.
        ...
        if cleaned:
            ...
            result.warnings.append(
                f"Moved CoBWeaverClaw learnings out of {legacy_path} into {target_path}: "
            ...
            # CLAUDE.md held nothing but the CoBWeaverClaw block — remove the husk.
            result.warnings.append(
                f"Removed {legacy_path} (it contained only CoBWeaverClaw learnings) and "
