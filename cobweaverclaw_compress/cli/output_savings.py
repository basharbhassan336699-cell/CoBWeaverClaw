...
def output_savings() -> None:
    ...
    emitted unshaped. This reports the honest estimate:
      ...
      * "measured" — from an A/B holdout (set COBWEAVERCLAW_OUTPUT_HOLDOUT>0), the
        unbiased difference between unshaped and shaped arms.
      * "estimated" — synthetic control: shaped output vs. the per-stratum
        baseline learned by `cobweaverclaw_compress learn --verbosity`.
    ...
    if not path.exists():
        ...
        click.echo("Run `cobweaverclaw_compress learn --verbosity --apply` to seed the baseline,")
        click.echo("then enable the shaper (COBWEAVERCLAW_OUTPUT_SHAPER=1) and send traffic.")
        return
    ...
    if est.kind == "estimated":
        click.echo(
            ...
            "\n  set COBWEAVERCLAW_OUTPUT_HOLDOUT=0.1 to leave 10% of conversations"
