"""CLI commands for CoBWeaverClaw Learn — offline failure learning."""
...
External plugins register via 'cobweaverclaw_compress.learn_plugin' entry point.
...
def learn(
    ...
    be installed via pip (entry point: cobweaverclaw_compress.learn_plugin).

    \b
    Examples:
        cobweaverclaw_compress learn                        # Auto-detect agent & model
        cobweaverclaw_compress learn --apply                # Write recommendations
        cobweaverclaw_compress learn --model gpt-4o         # Use GPT-4o for analysis
        cobweaverclaw_compress learn --all                  # Analyze all projects
        cobweaverclaw_compress learn --agent codex --all    # Analyze all Codex sessions
        cobweaverclaw_compress learn --target CLAUDE.md     # Write to the team-shared file
    """
    import os
    ...
    for agent_name, plugin in agent_configs:
        ...
        if analyze_all:
            ...
            if not targets and len(agent_configs) == 1:
                ...
                click.echo("Try: cobweaverclaw_compress learn --all  or  cobweaverclaw_compress learn --project <path>")
        ...
        for proj in targets:
            ...
            for file_path, content in result.content_by_file.items():
                ...
                for line in content.split("\n"):
                    if line.startswith("<!-- cobweaverclaw_compress"):
                        continue
                    click.echo(f"  {line}")
                click.echo(f"  {'─' * 50}")
...
def _activate_output_shaper(port: int | None = None) -> tuple[str, int]:
    ...
    ``COBWEAVERCLAW_OUTPUT_SHAPER`` is enabled in the proxy that serves traffic.
    ...
    resolved_port = port if port is not None else int(_os.environ.get("COBWEAVERCLAW_PORT", "8787"))
    request = urllib.request.Request(
        f"http://127.0.0.1:{resolved_port}/admin/runtime-env",
        data=_json.dumps({"COBWEAVERCLAW_OUTPUT_SHAPER": "1"}).encode("utf-8"),
        method="POST",
    ...
    if apply:
        ...
        if status == "live":
            click.echo(
                ...
                f"level {best_profile.level} is live now (while COBWEAVERCLAW_VERBOSITY_LEVEL is unset)."
            )
            click.echo(
                "    To keep it on across restarts: export COBWEAVERCLAW_OUTPUT_SHAPER=1 "
                "before `cobweaverclaw_compress wrap ...` (wrap pushes it to the proxy)."
            )
        else:
            click.echo(
            ...
            click.echo(
                "    Enable it: export COBWEAVERCLAW_OUTPUT_SHAPER=1 then `cobweaverclaw_compress wrap ...` "
                "(or start `cobweaverclaw_compress proxy` with it set). The learned level is then used "
                "automatically while COBWEAVERCLAW_VERBOSITY_LEVEL is unset."
            )
    else:
