...
def capture_group() -> None:
    """Capture and compare network traffic for CoBWeaverClaw investigations."""
...
@click.option(
    ...
    "cobweaverclaw_compress_path",
    required=True,
    type=click.Path(exists=True, dir_okay=False, path_type=Path),
    help="JSONL capture from the CoBWeaverClaw-proxied Claude Code lane.",
)
@click.option(
    "--output",
    "markdown_output",
...
def network_diff(
    direct_path: Path,
    cobweaverclaw_compress_path: Path,
    markdown_output: Path | None,
    json_output: Path | None,
    pair_by: str,
) -> None:
    """Compare direct and CoBWeaverClaw MITM capture JSONL files."""

    from cobweaverclaw_compress.capture.network_diff import (
        compare_captures,
        load_capture_file,
    ...
    cobweaverclaw_compress = load_capture_file(cobweaverclaw_compress_path, fallback_lane="cobweaverclaw_compress")
    diff = compare_captures(direct, cobweaverclaw_compress, pair_by=pair_by)
