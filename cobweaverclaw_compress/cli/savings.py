...
Reads the append-only savings ledger (``~/.cobweaverclaw_compress/savings_events.jsonl``,
...
def savings(as_json: bool, days: int, reset: bool) -> None:
    ...
    if calls == 0:
        ...
        click.echo(
            "Compress via the CoBWeaverClaw MCP tool or route traffic through the "
            "proxy, then re-run `cobweaverclaw_compress savings`."
        )
