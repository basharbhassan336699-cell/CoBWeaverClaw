...
def _default_db_path() -> str:
    ...
    (``./.cobweaverclaw_compress/memory.db``); otherwise fall back to the global workspace
    store (``~/.cobweaverclaw_compress/memory.db``). The previous bare ``cobweaverclaw_compress_memory.db``
    default pointed at neither, so ``cobweaverclaw_compress memory list`` silently read an
    empty/legacy DB.
    """
    project_db = Path.cwd() / ".cobweaverclaw_compress" / "memory.db"
...
def db_path_option(fn: Any) -> Any:
    ...
    return click.option(
        ...
        "(./.cobweaverclaw_compress/memory.db) if present, else the global store "
        "(~/.cobweaverclaw_compress/memory.db).",
...
def memory(ctx: click.Context) -> None:
    """Manage memories stored in CoBWeaverClaw.

    \b
    Examples:
        cobweaverclaw_compress memory list                     List all stored memories
        cobweaverclaw_compress memory list --limit 10          List the 10 most recent memories
        cobweaverclaw_compress memory list --scope USER        List only USER-level memories
        cobweaverclaw_compress memory list --since 7d          List memories from the last 7 days
        cobweaverclaw_compress memory show <id>                Show full details of a memory
        cobweaverclaw_compress memory stats                    Show memory statistics
        cobweaverclaw_compress memory edit <id> --content ...  Edit a memory's content
        cobweaverclaw_compress memory delete <id>              Delete a memory
        cobweaverclaw_compress memory prune --older-than 30d   Delete memories older than 30 days
        cobweaverclaw_compress memory purge --confirm          Delete ALL memories
        cobweaverclaw_compress memory export --output file.json  Export all memories to JSON
        cobweaverclaw_compress memory import file.json         Import memories from JSON
    """
    pass
...
def list_memories(
    ...
    \b
    Examples:
        cobweaverclaw_compress memory list                  List most recent memories
        cobweaverclaw_compress memory list --limit 10       Show only 10 memories
        cobweaverclaw_compress memory list --scope USER     Show only USER-level memories
        cobweaverclaw_compress memory list --since 7d       Show memories from the last 7 days
        cobweaverclaw_compress memory list --search "api"   Search for memories containing "api"
    """
...
def show_memory(
    ...
    \b
    Examples:
        cobweaverclaw_compress memory show abc123            Show memory details
        cobweaverclaw_compress memory show abc123 --json     Output as raw JSON
    """
    store = get_store(db_path)
    ...
    try:
        ...
        if old_count > 0:
            console.print(
                ...
                f"Consider running: cobweaverclaw_compress memory prune --older-than 30d[/dim]"
            )
...
def edit_memory(
    ...
    \b
    Examples:
        cobweaverclaw_compress memory edit abc123 --content "Updated content"
        cobweaverclaw_compress memory edit abc123 --importance 0.8
        cobweaverclaw_compress memory edit abc123 -c "New content" -i 0.9
    """
...
def delete_memories(
    ...
    \b
    Examples:
        cobweaverclaw_compress memory delete abc123           Delete one memory
        cobweaverclaw_compress memory delete abc123 def456    Delete multiple memories
        cobweaverclaw_compress memory delete abc123 --force   Skip confirmation
    """
    store = get_store(db_path)
...
def prune_memories(
    ...
    Examples:
        cobweaverclaw_compress memory prune --older-than 30d              Delete memories older than 30 days
        cobweaverclaw_compress memory prune --scope TURN                  Delete all TURN-level memories
        cobweaverclaw_compress memory prune --low-importance 0.3          Delete low-importance memories
        cobweaverclaw_compress memory prune --older-than 7d --scope TURN  Combine filters
        cobweaverclaw_compress memory prune --older-than 30d --dry-run    Preview what would be deleted
    """
...
def purge_memories(ctx: click.Context, db_path: str, confirm_flag: bool) -> None:
    ...
    \b
    Example:
        cobweaverclaw_compress memory purge --confirm
    """
    if not confirm_flag:
...
def export_memories(ctx: click.Context, db_path: str, output: str | None) -> None:
    ...
    \b
    Examples:
        cobweaverclaw_compress memory export                       Output to stdout
        cobweaverclaw_compress memory export --output backup.json  Save to file
        cobweaverclaw_compress memory export -o backup.json        Save to file (short form)
    """
    store = get_store(db_path)
...
def import_memories(ctx: click.Context, db_path: str, file: str, force: bool) -> None:
    ...
    \b
    Examples:
        cobweaverclaw_compress memory import backup.json          Import from file
        cobweaverclaw_compress memory import backup.json --force  Skip confirmation
    """
    file_path = Path(file)
