...
from cobweaverclaw_compress.providers.registry import resolve_api_overrides, resolve_api_targets
from cobweaverclaw_compress.proxy.modes import PROXY_MODE_TOKEN, normalize_proxy_mode
...
# Corporate TLS-inspection support (issue #1308). When COBWEAVERCLAW_TLS_STRICT=0,
...
try:  # pragma: no cover - exercised via integration, not unit-importable cheaply
    from cobweaverclaw_compress.proxy.ssl_context import apply_global_tls_relaxation as _apply_tls_relax
...
@click.option(
    ...
    envvar="COBWEAVERCLAW_PORT",
    help="Proxy port (default: 8787, env: COBWEAVERCLAW_PORT)",
)
...
def dashboard(port: int, no_open: bool) -> None:
    """Open the CoBWeaverClaw savings dashboard in your browser.

    Requires a running proxy (start one with `cobweaverclaw_compress proxy` or `cobweaverclaw_compress wrap ...`).
    """
    import webbrowser
...
@click.option(
    "--host",
    default="127.0.0.1",
    envvar="COBWEAVERCLAW_HOST",
    help="Host to bind to (default: 127.0.0.1, env: COBWEAVERCLAW_HOST)",
)
@click.option(
    "--port",
    "-p",
    ...
    type=click.IntRange(1, 65535),
    envvar="COBWEAVERCLAW_PORT",
    help="Port to bind to (default: 8787, env: COBWEAVERCLAW_PORT)",
)
@click.option(
    "--workers",
    default=1,
    type=click.IntRange(min=1),
    envvar="COBWEAVERCLAW_WORKERS",
    help="Number of Uvicorn worker processes (default: 1, env: COBWEAVERCLAW_WORKERS)",
)
@click.option(
    "--limit-concurrency",
    default=1000,
    type=click.IntRange(min=1),
    envvar="COBWEAVERCLAW_LIMIT_CONCURRENCY",
    help=(
        "Maximum concurrent connections before Uvicorn returns 503 "
        "(default: 1000, env: COBWEAVERCLAW_LIMIT_CONCURRENCY)"
    ),
)
@click.option(
    "--max-connections",
    default=500,
    type=click.IntRange(min=1),
    envvar="COBWEAVERCLAW_MAX_CONNECTIONS",
    help="Maximum upstream HTTP connections (default: 500, env: COBWEAVERCLAW_MAX_CONNECTIONS)",
)
@click.option(
    "--max-keepalive",
    ...
    envvar="COBWEAVERCLAW_MAX_KEEPALIVE",
    help="Maximum upstream keep-alive connections (default: 100, env: COBWEAVERCLAW_MAX_KEEPALIVE)",
)
@click.option(
    "--http2/--no-http2",
    "http2",
    default=True,
    envvar="COBWEAVERCLAW_HTTP2",
    help=(
        "Use HTTP/2 to upstream providers (default: on, env: COBWEAVERCLAW_HTTP2). "
...
@click.option(
    "--http-proxy",
    default=None,
    envvar="COBWEAVERCLAW_HTTP_PROXY",
    help=(
        "HTTP proxy URL for upstream provider requests only "
        "(HTTPS uses CONNECT; env: COBWEAVERCLAW_HTTP_PROXY)."
    ),
)
@click.option(
    "--keepalive-expiry",
    ...
    envvar="COBWEAVERCLAW_KEEPALIVE_EXPIRY",
    help="Seconds an idle upstream keep-alive connection is kept open (default: 90, env: COBWEAVERCLAW_KEEPALIVE_EXPIRY)",
)
@click.option(
    "--mode",
    default=None,
    ...
    help=(
        ...
        "Legacy aliases (token_mode, token_savings, token_cobweaverclaw_compress, cache_mode, "
        "cost_savings) are still accepted. Env: COBWEAVERCLAW_MODE."
    ),
)
@click.option(
    "--target-ratio",
    ...
    default=None,
    show_default=True,
    envvar="COBWEAVERCLAW_TARGET_RATIO",
    help=(
        ...
        "Env: COBWEAVERCLAW_TARGET_RATIO."
    ),
)
@click.option(
...
@click.option(
    ...
    default=None,
    type=click.IntRange(min=1),
    envvar="COBWEAVERCLAW_RPM",
    help="Max requests per minute. Env: COBWEAVERCLAW_RPM. Default: 60.",
)
@click.option(
    "--tpm",
    default=None,
    type=click.IntRange(min=1),
    envvar="COBWEAVERCLAW_TPM",
    help="Max tokens per minute. Env: COBWEAVERCLAW_TPM. Default: 100000.",
)
@click.option(
    "--no-ccr",
    is_flag=True,
    envvar="COBWEAVERCLAW_NO_CCR",
    help=(
        "Disable CCR entirely: no retrieval markers in compressed content AND no "
        "cobweaverclaw_compress_retrieve tool injected. Lossy compression with no recovery path "
        "(maximum savings; also right for streaming / non-MCP clients that can't "
        "resolve an injected tool). Env: COBWEAVERCLAW_NO_CCR."
    ),
)
@click.option(
    "--lossless",
    is_flag=True,
    envvar="COBWEAVERCLAW_LOSSLESS",
    help=(
        ...
        "retrieval marker, so no MCP retrieve tool is needed. Env: COBWEAVERCLAW_LOSSLESS=1."
    ),
)
@click.option(
    "--no-ccr-proactive-expansion",
    is_flag=True,
    envvar="COBWEAVERCLAW_NO_CCR_PROACTIVE_EXPANSION",
    help=(
        "Disable proactive expansion of previously compressed content. "
        "Env: COBWEAVERCLAW_NO_CCR_PROACTIVE_EXPANSION."
    ),
)
@click.option(
    "--proxy-extension",
    "proxy_extension",
    multiple=True,
    envvar="COBWEAVERCLAW_PROXY_EXTENSIONS",
    help=(
        ...
        "every discovered extension. Env: COBWEAVERCLAW_PROXY_EXTENSIONS."
    ),
)
@click.option(
    "--no-subscription-tracking",
    is_flag=True,
    envvar="COBWEAVERCLAW_NO_SUBSCRIPTION_TRACKING",
    help=(
        "Disable the Anthropic Claude Code subscription usage poller "
        "(GET /api/oauth/usage). Env: COBWEAVERCLAW_NO_SUBSCRIPTION_TRACKING."
    ),
)
@click.option(
    ...
    default=None,
    envvar="COBWEAVERCLAW_SUBSCRIPTION_POLL_INTERVAL",
    help=(
        ...
        "Env: COBWEAVERCLAW_SUBSCRIPTION_POLL_INTERVAL."
    ),
)
@click.option(
    "--retry-max-attempts",
...
@click.option(
    ...
    type=int,
    default=None,
    envvar="COBWEAVERCLAW_REQUEST_TIMEOUT",
    help=(
        "Request timeout in seconds (default: 300). "
        "Useful for slow providers (eg local). "
        "Env: COBWEAVERCLAW_REQUEST_TIMEOUT."
    ),
)
@click.option(
    ...
    default=None,
    envvar="COBWEAVERCLAW_CONNECT_TIMEOUT_SECONDS",
    help=(
        "Upstream connection timeout in seconds (1–300, default: 10). "
        "Env: COBWEAVERCLAW_CONNECT_TIMEOUT_SECONDS."
    ),
)
@click.option(
    ...
    type=click.IntRange(min=1),
    default=None,
    envvar="COBWEAVERCLAW_ANTHROPIC_BUFFERED_REQUEST_TIMEOUT_SECONDS",
    help=(
        "Buffered Anthropic read timeout in seconds for non-streaming "
        "message and batch paths (default: 600). "
        "Env: COBWEAVERCLAW_ANTHROPIC_BUFFERED_REQUEST_TIMEOUT_SECONDS."
    ),
)
@click.option(
    ...
    type=int,
    default=None,
    envvar="COBWEAVERCLAW_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY",
    help=(
        ...
        "Env: COBWEAVERCLAW_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY."
    ),
)
@click.option(
    ...
    type=float,
    default=None,
    envvar="COBWEAVERCLAW_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS",
    help=(
        ...
        "Env: COBWEAVERCLAW_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS."
    ),
)
@click.option(
    ...
    type=float,
    default=None,
    envvar="COBWEAVERCLAW_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS",
    help=(
        "Fail-open timeout for Anthropic memory-context lookup while the request "
        "still holds a pre-upstream slot. "
        "Default: 2.0 seconds. "
        "Env: COBWEAVERCLAW_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS."
    ),
)
@click.option(
    ...
    type=int,
    default=None,
    envvar="COBWEAVERCLAW_COMPRESSION_MAX_WORKERS",
    help=(
        ...
        "Env: COBWEAVERCLAW_COMPRESSION_MAX_WORKERS."
    ),
)
@click.option(
    "--log-file",
    default=None,
    envvar="COBWEAVERCLAW_LOG_FILE",
    help=(
        ...
        "Disabled in --stateless mode. Env: COBWEAVERCLAW_LOG_FILE."
    ),
)
@click.option(
    "--log-messages",
    is_flag=True,
    envvar="COBWEAVERCLAW_LOG_MESSAGES",
    help=(
        ...
        "Env: COBWEAVERCLAW_LOG_MESSAGES."
    ),
)
@click.option(
    "--codex-wire-debug",
...
@click.option(
    ...
    help=(
        ...
        "~/.cobweaverclaw_compress/logs/codex_wire or workspace .cobweaverclaw_compress/logs/codex_wire)."
    ),
)
@click.option(
    "--budget",
    ...
    default=None,
    envvar="COBWEAVERCLAW_BUDGET",
    help=(
        "Budget limit in USD per --budget-period. Requests are rejected with 429 "
        "once the limit is reached. Env: COBWEAVERCLAW_BUDGET."
    ),
)
@click.option(
    "--budget-period",
    ...
    default="daily",
    envvar="COBWEAVERCLAW_BUDGET_PERIOD",
    help=(
        ...
        "Env: COBWEAVERCLAW_BUDGET_PERIOD."
    ),
)
# Code-aware compression (AST-based, requires `pip install cobweaverclaw_compress-ai[code]`).
# Pair of flags so users can override the env-var default in either direction.
# We resolve COBWEAVERCLAW_CODE_AWARE_ENABLED in the body (not via Click's envvar=),
...
@click.option(
    ...
    help=(
        ...
        "tree-sitter dependency: pip install cobweaverclaw_compress-ai[code]. "
        "Default: disabled. Env: COBWEAVERCLAW_CODE_AWARE_ENABLED=1 to enable."
    ),
)
@click.option(
    "--disable-kompress",
    is_flag=True,
    envvar="COBWEAVERCLAW_DISABLE_KOMPRESS",
    help=(
        "Disable Kompress ML compression while keeping structural compression enabled. "
        "Env: COBWEAVERCLAW_DISABLE_KOMPRESS=1."
    ),
)
@click.option(
    "--disable-kompress-fallback",
    is_flag=True,
    envvar="COBWEAVERCLAW_DISABLE_KOMPRESS_FALLBACK",
    help=(
        ...
        "Env: COBWEAVERCLAW_DISABLE_KOMPRESS_FALLBACK=1."
    ),
)
@click.option(
    ...
    default=None,
    envvar="COBWEAVERCLAW_DISABLE_KOMPRESS_ANTHROPIC",
    help=(
        "Disable (or --enable-) Kompress for the Anthropic pipeline only, overriding "
        "--disable-kompress. Env: COBWEAVERCLAW_DISABLE_KOMPRESS_ANTHROPIC=1."
    ),
)
@click.option(
    ...
    default=None,
    envvar="COBWEAVERCLAW_DISABLE_KOMPRESS_OPENAI",
    help=(
        "Disable (or --enable-) Kompress for the OpenAI/Codex pipeline only, overriding "
        "--disable-kompress. Env: COBWEAVERCLAW_DISABLE_KOMPRESS_OPENAI=1."
    ),
)
...
@click.option(
    "--read-maturation",
    is_flag=True,
    envvar="COBWEAVERCLAW_READ_MATURATION",
    help=(
        ...
        "file quiesces (env: COBWEAVERCLAW_READ_MATURATION=1)"
    ),
)
@click.option(
    ...
    default=5,
    show_default=True,
    envvar="COBWEAVERCLAW_READ_MATURATION_QUIESCE_TURNS",
...
@click.option(
    ...
    default=25,
    show_default=True,
    envvar="COBWEAVERCLAW_READ_MATURATION_MAX_HOLD_TURNS",
...
@click.option(
    ...
    default=2048,
    show_default=True,
    envvar="COBWEAVERCLAW_READ_MATURATION_MIN_SIZE_BYTES",
...
@click.option(
    ...
    help=(
        ...
        "x-cobweaverclaw_compress-user-id and/or x-cobweaverclaw_compress-project-id / x-cobweaverclaw_compress-cwd request headers."
    ),
)
@click.option(
    "--memory-db-path",
    default="",
    envvar="COBWEAVERCLAW_MEMORY_DB_PATH",
    help=(
        "Path to the legacy single-file memory DB (used in --memory-storage=global, "
        "and as the seed for the project-mode storage root). "
        "Default: {cwd}/.cobweaverclaw_compress/memory.db. Env: COBWEAVERCLAW_MEMORY_DB_PATH."
    ),
)
@click.option(
    "--memory-storage",
    ...
    help=(
        ...
        "no cross-project bleed. user: one DB per x-cobweaverclaw_compress-user-id. global: a single "
...
@click.option(
    "--memory-project-root",
    default="",
    envvar="COBWEAVERCLAW_MEMORY_PROJECT_ROOT",
    help=(
        ...
        "workspace. Takes effect after the x-cobweaverclaw_compress-project-id and x-cobweaverclaw_compress-cwd "
        "headers. Env: COBWEAVERCLAW_MEMORY_PROJECT_ROOT."
    ),
)
@click.option(
    "--no-memory-tools",
    is_flag=True,
    envvar="COBWEAVERCLAW_NO_MEMORY_TOOLS",
    help=(
        "Disable automatic injection of memory_save/memory_search tools into requests. "
        "Env: COBWEAVERCLAW_NO_MEMORY_TOOLS."
    ),
)
@click.option(
    "--no-memory-context",
    is_flag=True,
    envvar="COBWEAVERCLAW_NO_MEMORY_CONTEXT",
    help=(
        "Disable automatic injection of relevant past memories into the system prompt. "
        "Env: COBWEAVERCLAW_NO_MEMORY_CONTEXT."
    ),
)
@click.option(
    "--memory-top-k",
...
@click.option(
    ...
    help=(
        ...
        "Also reads COBWEAVERCLAW_QDRANT_URL."
    ),
)
@click.option(
    "--memory-qdrant-host",
    ...
    help=(
        ...
        "(default: localhost, also reads COBWEAVERCLAW_QDRANT_HOST)"
    ),
)
@click.option(
    "--memory-qdrant-port",
    ...
    default=None,
    help=(
        "Qdrant port for the qdrant-neo4j backend (default: 6333, also reads COBWEAVERCLAW_QDRANT_PORT)"
    ),
)
@click.option(
    "--memory-qdrant-api-key",
    default=None,
    help=("API key for hosted Qdrant (e.g. Qdrant Cloud). Also reads COBWEAVERCLAW_QDRANT_API_KEY."),
)
...
@click.option(
    ...
    type=click.IntRange(min=1),
    default=None,
    envvar="COBWEAVERCLAW_MIN_EVIDENCE",
    help=(
        ...
        "cost of slower learning. Default: 5. (env: COBWEAVERCLAW_MIN_EVIDENCE)"
    ),
)
...
@click.option(
    "--region",
    default="us-west-2",
    envvar="COBWEAVERCLAW_REGION",
    help="Cloud region for Bedrock/Vertex/etc (default: us-west-2). Env: COBWEAVERCLAW_REGION.",
)
@click.option(
    "--bedrock-region",
    default=None,
...
@click.option(
    "--telemetry",
    is_flag=True,
    help="Opt in to anonymous usage telemetry — off by default (env: COBWEAVERCLAW_TELEMETRY=on)",
)
@click.option(
    "--no-telemetry",
    is_flag=True,
    help="Force anonymous usage telemetry off (already the default; env: COBWEAVERCLAW_TELEMETRY=off)",
)
@click.option(
    "--stateless",
    is_flag=True,
    ...
    "(env: COBWEAVERCLAW_STATELESS=true)",
)
@click.option(
    ...
    "(env: COBWEAVERCLAW_EMBEDDING_SERVER=true)",
)
@click.option(
    "--embedding-server-socket",
    default=None,
    help="Unix socket path for the embedding server sidecar. "
    "Default: /tmp/cobweaverclaw_compress-embed-{port}.sock. "
    "(env: COBWEAVERCLAW_EMBEDDING_SERVER_SOCKET)",
)
@click.pass_context
def proxy(
    ctx: click.Context,
    ...
    \b
    Examples:
        cobweaverclaw_compress proxy                    Start proxy on port 8787
        cobweaverclaw_compress proxy --port 8080        Start proxy on port 8080
        cobweaverclaw_compress proxy --no-optimize      Passthrough mode (no optimization)
    ...
    try:
        from cobweaverclaw_compress.proxy.server import (
            ProxyConfig,
            _parse_csv_tools,
        ...
        click.secho(
            "Error: Proxy dependencies not installed. Run: pip install cobweaverclaw_compress-ai[proxy]",
            fg="red",
            err=True,
        )
    ...
    if intercept_tool_results:
        from cobweaverclaw_compress.binaries import ensure_tools
        ...
        if missing:
            ...
            click.secho(
                ...
                f"be installed: {missing}. Run `cobweaverclaw_compress tools doctor` to diagnose, "
                "or omit the flag to start the proxy without interceptors.",
                fg="red",
                err=True,
            )
            sys.exit(1)
        os.environ["COBWEAVERCLAW_INTERCEPT_ENABLED"] = "1"
    ...
    effective_anyllm_provider = os.environ.get("COBWEAVERCLAW_ANYLLM_PROVIDER") or anyllm_provider

    # Resolve mode: CLI flag > env var > default
    effective_mode: str = normalize_proxy_mode(
        mode or os.environ.get("COBWEAVERCLAW_MODE") or PROXY_MODE_TOKEN
    )

    # Stateless mode: CLI flag or env var
    is_stateless = stateless or os.environ.get("COBWEAVERCLAW_STATELESS", "").lower() in (
        "true",
        "1",
        "yes",
        "on",
    ...
    if telemetry:
        os.environ["COBWEAVERCLAW_TELEMETRY"] = "on"
    if no_telemetry:
        os.environ["COBWEAVERCLAW_TELEMETRY"] = "off"

    if codex_wire_debug or codex_wire_debug_dir:
        os.environ["COBWEAVERCLAW_CODEX_WIRE_DEBUG"] = "1"
        os.environ["COBWEAVERCLAW_CODEX_WIRE_DEBUG_DIR"] = codex_wire_debug_dir or str(
            _paths.codex_wire_debug_dir()
        )

    # Stateless mode: suppress TOIN filesystem persistence
    if is_stateless:
        os.environ["COBWEAVERCLAW_TOIN_BACKEND"] = "none"

    # License key for managed/enterprise deployments (optional)
    license_key = os.environ.get("COBWEAVERCLAW_LICENSE_KEY")
    ...
    # COBWEAVERCLAW_QDRANT_* env vars. Explicit CLI values win over env.
    ...
    config = ProxyConfig(
        ...
        compress_user_messages=_get_env_bool("COBWEAVERCLAW_COMPRESS_USER_MESSAGES", False),
        min_tokens_to_crush=_get_env_int_optional("COBWEAVERCLAW_MIN_TOKENS") or 500,
        max_items_after_crush=_get_env_int_optional("COBWEAVERCLAW_MAX_ITEMS") or 50,
        ...
        smart_crusher_with_compaction=_get_env_bool_optional("COBWEAVERCLAW_SMART_CRUSHER_COMPACTION"),
        savings_profile=os.environ.get("COBWEAVERCLAW_SAVINGS_PROFILE") or None,
        target_ratio=target_ratio,
        compress_system_messages=_get_env_bool_optional("COBWEAVERCLAW_COMPRESS_SYSTEM_MESSAGES"),
        protect_recent=_get_env_int_optional("COBWEAVERCLAW_PROTECT_RECENT"),
        protect_analysis_context=_get_env_bool_optional("COBWEAVERCLAW_PROTECT_ANALYSIS_CONTEXT"),
        ...
        # `--proxy-extension a,b --proxy-extension c` and `COBWEAVERCLAW_PROXY_EXTENSIONS=a,b,c`
        ...
        or os.environ.get("COBWEAVERCLAW_LOG_MESSAGES", "").lower() in ("true", "1", "yes", "on"),
        ...
        # 2. Otherwise read COBWEAVERCLAW_CODE_AWARE_ENABLED (truthy = on).
        ...
        code_aware_enabled=(
            ...
            else os.environ.get("COBWEAVERCLAW_CODE_AWARE_ENABLED", "").strip().lower()
            in ("true", "1", "yes", "on")
        ),
        ...
        **qdrant_overrides,
        ...
        traffic_learning_agent_type=os.environ.get("COBWEAVERCLAW_AGENT_TYPE", "unknown"),
        ...
        # path. ``None`` -> CoBWeaverClawProxy computes ``max(2, min(8,
...
"""
        ...
        # LiteLLM backend
        from cobweaverclaw_compress.backends.litellm import get_provider_config
...
IMPORTANT for {provider_config.display_name} users:
  ...
  2. Set a dummy Anthropic key: ANTHROPIC_API_KEY="sk-ant-dummy"
     (CoBWeaverClaw ignores this - it uses your {provider_config.display_name} credentials)
...
Memory (Multi-Provider):
  ...
  - Storage mode: {config.memory_storage_mode} (per-project DB by default — set x-cobweaverclaw_compress-project-id / x-cobweaverclaw_compress-cwd to override)
...
"""
    ...
    if is_telemetry_enabled():
        telemetry_line = (
            ...
            "                Disable: COBWEAVERCLAW_TELEMETRY=off or cobweaverclaw_compress proxy --no-telemetry"
        )
    else:
        telemetry_line = (
            "  Telemetry:    DISABLED (opt in: COBWEAVERCLAW_TELEMETRY=on or cobweaverclaw_compress proxy --telemetry)"
        )

    # Discover proxy extensions (third-party packages registered via the
    # `cobweaverclaw_compress.proxy_extension` entry-point group). Surfaced in the banner
    ...
    try:
        from cobweaverclaw_compress.proxy.extensions import discover as _discover_extensions
    ...
    if not _ext_available:
        ...
        extensions_line = (
            ...
            f"(opt-in: --proxy-extension <name> or COBWEAVERCLAW_PROXY_EXTENSIONS=<n>)"
        )
    ...
    _auth_on = bool(config.proxy_token or os.environ.get("COBWEAVERCLAW_PROXY_TOKEN"))
    if config.offline:
    ...
    from cobweaverclaw_compress.proxy.server import _get_code_aware_banner_status
    ...
    _embed_socket = os.environ.get("COBWEAVERCLAW_EMBEDDING_SERVER_SOCKET") or (
        embedding_server and (embedding_server_socket or f"/tmp/cobweaverclaw_compress-embed-{port}.sock")
    )
...
""")
    ...
    try:
        from cobweaverclaw_compress.update_check import format_update_notice
    ...
    if embedding_server:
        _embed_socket = embedding_server_socket or f"/tmp/cobweaverclaw_compress-embed-{config.port}.sock"
        # Pass socket path to all worker processes via environment variable
        os.environ["COBWEAVERCLAW_EMBEDDING_SERVER_SOCKET"] = _embed_socket
        ...
        async def _start_embed_watchdog() -> Any:
            ...
            from cobweaverclaw_compress.memory.adapters.watchdog import EmbeddingServerWatchdog
        ...
        try:
            ...
            os.environ.pop("COBWEAVERCLAW_EMBEDDING_SERVER_SOCKET", None)
    ...
    try:
        ...
        # a richer one above. Direct `python -m cobweaverclaw_compress.proxy.server` keeps
