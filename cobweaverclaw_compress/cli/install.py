...
from cobweaverclaw_compress.install.health import probe_json, probe_ready
from cobweaverclaw_compress.install.models import (
    ConfigScope,
    DeploymentManifest,
    InstallPreset,
...
from cobweaverclaw_compress.install.providers import apply_mutations, revert_mutations
...
from cobweaverclaw_compress.install.state import (
    ManifestError,
    delete_manifest,
    load_manifest,
    save_manifest,
)
from cobweaverclaw_compress.install.supervisors import (
    install_supervisor,
    remove_supervisor,
    start_supervisor,
    stop_supervisor,
...
@main.group()
def install() -> None:
    """Install and manage persistent CoBWeaverClaw deployments."""
...
def _reject_task_lifecycle(manifest: DeploymentManifest, action: str) -> None:
    if manifest.supervisor_kind == SupervisorKind.TASK.value:
        raise click.ClickException(
            ...
            f"`cobweaverclaw_compress install {action}` is not supported for task deployments."
        )
...
@click.option(
    ...
    show_default=True,
    help="Runtime used to execute CoBWeaverClaw for service/task modes.",
)
@click.option(
    "--scope",
...
@click.option(
    "--image",
    default="ghcr.io/chopratejas/cobweaverclaw_compress:latest",
    show_default=True,
...
def install_apply(
    ...
    """Install a persistent CoBWeaverClaw deployment."""
