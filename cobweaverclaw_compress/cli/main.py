"""Main CLI entry point for CoBWeaverClaw."""
...
@click.version_option(get_version(), "--version", "-v", prog_name="cobweaverclaw_compress")
@click.pass_context
def main(ctx: click.Context) -> None:
    ...
    \b
    Examples:
        cobweaverclaw_compress proxy              Start the optimization proxy
        cobweaverclaw_compress memory list        List stored memories
        cobweaverclaw_compress memory stats       Show memory statistics
        cobweaverclaw_compress update             Update CoBWeaverClaw to the latest release
    """
    ...
    if ctx.invoked_subcommand != "update":
        try:
            from cobweaverclaw_compress.update_check import maybe_check_async
