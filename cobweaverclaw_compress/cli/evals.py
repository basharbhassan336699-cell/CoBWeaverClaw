...
def evals() -> None:
    ...
    \b
    Examples:
        cobweaverclaw_compress evals memory       Run LoCoMo memory evaluation
        cobweaverclaw_compress evals memory-v2    Run V2 evaluation with LLM-controlled tools
        cobweaverclaw_compress evals adversarial  Compression-robustness adversarial grid
        cobweaverclaw_compress evals probes       Retention probes over recorded sessions
    """
    pass
...
def memory_eval(
    ...
    \b
    Examples:
        cobweaverclaw_compress evals memory -n 3
        cobweaverclaw_compress evals memory --answer-model gpt-4o --llm-judge
    """
    _run_memory_eval(
...
def memory_eval_v2(
    ...
    \b
    Examples:
        cobweaverclaw_compress evals memory-v2 -n 3
        cobweaverclaw_compress evals memory-v2 --answer-model gpt-4o --save-model gpt-4o-mini
    """
    _run_memory_eval_v2(
        n_conversations=n_conversations,
        categories=categories,
...
def memory_eval_compat(
    ...
    """Deprecated: Use 'cobweaverclaw_compress evals memory' instead."""
    click.echo("Note: 'memory-eval' is deprecated. Use 'cobweaverclaw_compress evals memory'", err=True)
    _run_memory_eval(
        n_conversations=n_conversations,
        categories=categories,
        include_adversarial=include_adversarial,
...
def memory_eval_v2_compat(
    ...
    """Deprecated: Use 'cobweaverclaw_compress evals memory-v2' instead."""
    click.echo("Note: 'memory-eval-v2' is deprecated. Use 'cobweaverclaw_compress evals memory-v2'", err=True)
    _run_memory_eval_v2(
        n_conversations=n_conversations,
        categories=categories,
        include_adversarial=include_adversarial,
...
def _run_memory_eval(
    *,
    ...
    try:
        from cobweaverclaw_compress.evals.memory import (
            LoCoMoEvaluator,
            MemoryEvalConfig,
        ...
        from cobweaverclaw_compress.memory import MemoryConfig
    except ImportError as e:
        click.echo("Error: Memory eval dependencies not installed.")
        click.echo("Run: pip install cobweaverclaw_compress[memory,evals]")
...
## Answer (be concise - just the facts):"""
    ...
    click.echo(f"""
╔═══════════════════════════════════════════════════════════════════════╗
║                    COBWEAVERCLAW MEMORY EVALUATION                          ║
║                         LoCoMo Benchmark                               ║
╚═══════════════════════════════════════════════════════════════════════╝
...
def _run_memory_eval_v2(
    *,
    ...
    click.echo(f"""
╔═══════════════════════════════════════════════════════════════════════╗
║                   COBWEAVERCLAW MEMORY EVALUATION V2                        ║
...
@click.option(
    ...
    help="Directory of JSONL recordings written via COBWEAVERCLAW_PROBE_RECORD_DIR.",
)
@click.option(
    "--json-output",
...
def probes(recordings_dir: Path, json_output: Path | None) -> None:
    ...
    COBWEAVERCLAW_PROBE_RECORD_DIR set. Recordings contain full conversation
    content in plaintext and stay on this machine.
    """
    import json as json_module

    from cobweaverclaw_compress.evals.session_probes import render_report, run_probes
...
def adversarial(json_output: Path | None) -> None:
    ...
    from cobweaverclaw_compress.evals.adversarial_grid import render_report, run_adversarial_grid
