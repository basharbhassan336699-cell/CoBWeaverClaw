"""`cobweaverclaw_compress update` — self-update across the supported install methods.

Detects how CoBWeaverClaw was installed and runs the matching upgrade command, or
...
from cobweaverclaw_compress.update_check import (
    PACKAGE_NAME,
...
class InstallMethod:
    """How CoBWeaverClaw is installed and how (or whether) to upgrade it."""
...
def _is_source_checkout() -> bool:
    try:
        from cobweaverclaw_compress._version import _source_root
...
def _in_docker() -> bool:
    try:
        ...
        return Path("/.dockerenv").exists() or bool(
            os.environ.get("COBWEAVERCLAW_IN_DOCKER", "").strip()
        )
    except Exception:
        return False
...
def _package_location() -> str | None:
    """Normalized base directory the cobweaverclaw_compress-ai distribution is installed in."""
    try:
        from importlib.metadata import distribution
...
def _managed_env_guidance() -> str:
    if sys.platform == "darwin":
        hint = "`brew upgrade cobweaverclaw_compress-ai` (if installed via Homebrew), or reinstall with pipx"
    elif sys.platform.startswith("win"):
        hint = "reinstall with pipx (`pipx install cobweaverclaw_compress-ai`) or use a virtualenv"
    else:
        hint = "use your distro package manager, or reinstall with pipx / a virtualenv"
    return (
        "CoBWeaverClaw is installed in an externally-managed system Python (PEP 668). "
...
def detect_install_method(extras: str | None = None) -> InstallMethod:
    ...
    if _in_docker():
        return InstallMethod(
            ...
            can_self_update=False,
            guidance=(
                "Running inside a container — pull a newer CoBWeaverClaw image instead of self-updating."
            ),
        )
...
def update(check_only: bool, assume_yes: bool, allow_pre: bool, extras: str | None) -> None:
    """Update CoBWeaverClaw to the latest release.
    ...
    click.echo("Checking PyPI for the latest CoBWeaverClaw release...")
    ...
    if current:
        ...
        try:
            if Version(latest) <= Version(current):
                click.echo(f"CoBWeaverClaw is up to date ({current}).")
                return
        except InvalidVersion:
            pass
        click.echo(f"Update available: {current} → {latest}")
    else:
        click.echo(f"Latest CoBWeaverClaw release: {latest}")
    ...
    click.echo(f"CoBWeaverClaw upgraded to {latest}.")
    click.echo("Restart any running `cobweaverclaw_compress proxy` to pick up the new version.")
