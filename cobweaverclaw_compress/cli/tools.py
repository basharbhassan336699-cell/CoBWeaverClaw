...
Exposes:
    ...
    cobweaverclaw_compress sg …           ->  ast-grep (from the ast-grep-cli PyPI wheel)
    cobweaverclaw_compress diff A B …     ->  difftastic
    cobweaverclaw_compress loc [PATH] …   ->  scc
    cobweaverclaw_compress tools install  ->  pre-fetch all bundled binaries
    cobweaverclaw_compress tools doctor   ->  print a status table
    cobweaverclaw_compress tools list     ->  show the registry
...
tool without any CoBWeaverClaw-specific protocol.
"""
...
from cobweaverclaw_compress import binaries
...
def _exec_tool(tool: str, argv: Sequence[str]) -> None:
    try:
        ...
        click.secho(
            f"error: {e}\nHint: run `cobweaverclaw_compress tools install` on a networked machine, "
    ...
    # the parent `cobweaverclaw_compress` process, not these thin passthroughs).
    cmd = [str(path), *argv]
    if not _is_windows():
    ...
    for name in selected:
        ...
        if binaries._is_pypi_tool(name):  # noqa: SLF001
            ...
            if on_path:
                ...
                click.secho(
                    f"{name}: not on PATH — `pip install cobweaverclaw_compress-ai` should provide it",
                    fg="yellow",
                )
                exit_code = 1
            continue
