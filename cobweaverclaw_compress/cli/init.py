...
from cobweaverclaw_compress.install.models import ConfigScope, InstallPreset, RuntimeKind, SupervisorKind
from cobweaverclaw_compress.install.paths import claude_settings_path, codex_config_path, validate_profile_name
from cobweaverclaw_compress.install.planner import build_manifest
from cobweaverclaw_compress.install.providers import _apply_unix_env_scope, _apply_windows_env_scope
from cobweaverclaw_compress.install.runtime import (
    acquire_runtime_start_lock,
    resolve_cobweaverclaw_compress_command,
    runtime_status,
...
from cobweaverclaw_compress.install.state import ManifestError, load_manifest, save_manifest
from cobweaverclaw_compress.install.supervisors import start_supervisor
from cobweaverclaw_compress.providers.claude import TOOL_SEARCH_DEFAULT, TOOL_SEARCH_ENV
from cobweaverclaw_compress.providers.codex.install import codex_uses_chatgpt_auth
from cobweaverclaw_compress.providers.codex.threads import retag_to_cobweaverclaw_compress

from .main import main

logger = logging.getLogger(__name__)

_VERBOSE_HANDLER_ATTR = "_cobweaverclaw_compress_init_verbose_handler"

_GLOBAL_PROFILE = "init-user"
_CLAUDE_HOOK_MARKER = "cobweaverclaw_compress-init-claude"
_COPILOT_HOOK_MARKER = "cobweaverclaw_compress-init-copilot"
_CODEX_HOOK_MARKER = "cobweaverclaw_compress-init-codex"
_CODEX_PROVIDER_MARKER_START = "# --- CoBWeaverClaw init provider ---"
_CODEX_PROVIDER_MARKER_END = "# --- end CoBWeaverClaw init provider ---"
_CODEX_FEATURE_MARKER_START = "# --- CoBWeaverClaw init features ---"
_CODEX_FEATURE_MARKER_END = "# --- end CoBWeaverClaw init features ---"
...
def _hook_command(*parts: str) -> str:
    return _command_string([*resolve_cobweaverclaw_compress_command(), "init", "hook", "ensure", *parts])
...
def _enable_verbose_logging() -> None:
    ...
    mutate stdout; all verbose output goes to stderr so ``cobweaverclaw_compress init``
    ...
    handler.setFormatter(logging.Formatter("[cobweaverclaw_compress init] %(message)s"))
...
def _strip_codex_init_block(content: str) -> str:
    """Remove all CoBWeaverClaw init-managed blocks and orphan keys from a Codex config.toml string."""
    import re
    ...
    # Strip any orphaned [model_providers.cobweaverclaw_compress] table that is recognisably ours.
...
def _ensure_codex_provider(path: Path, port: int) -> None:
    ...
    # native threads vanish once we switch to "cobweaverclaw_compress". Retag them to match the
    ...
    # handled by `cobweaverclaw_compress unwrap codex`.
    retag_to_cobweaverclaw_compress(path.parent)
...
def _ensure_runtime_manifest(
    *,
    ...
    manifest = build_manifest(
        ...
        image="ghcr.io/chopratejas/cobweaverclaw_compress:latest",
    )
...
def _marketplace_source() -> str:
    override = os.environ.get("COBWEAVERCLAW_MARKETPLACE_SOURCE")
    if override:
        return override
    repo_root = Path(__file__).resolve().parents[2]
    if (repo_root / ".claude-plugin" / "marketplace.json").exists():
        return str(repo_root)
    return "chopratejas/cobweaverclaw_compress"
...
def _install_claude_marketplace(scope: str) -> None:
    ...
    _run_checked(
        [claude_bin, "plugin", "install", "cobweaverclaw_compress@cobweaverclaw_compress-marketplace", "--scope", scope],
...
def _install_copilot_marketplace() -> None:
    ...
    _run_checked(
        [copilot_bin, "plugin", "install", "cobweaverclaw_compress@cobweaverclaw_compress-marketplace"],
...
def _format_empty_detection_error(global_scope: bool) -> str:
    ...
    lines: list[str] = [
        ...
        "",
        "CoBWeaverClaw probed the following agents via shutil.which():",
    ]
    ...
    lines.extend(
        [
            ...
            "cobweaverclaw_compress init just needs to know which agent to target.",
    ...
    for name, _path in probes:
        ...
        lines.append(f"  cobweaverclaw_compress init{flag} {name}")

    lines.extend(
        [
            "",
            "Tip: run `cobweaverclaw_compress init --help` to see all options.",
        ]
    )
    return "\n".join(lines)
...
def _init_claude(*, global_scope: bool, profile: str, port: int) -> None:
    ...
    click.echo("Restart Claude Code to activate CoBWeaverClaw hooks and provider routing.")
...
def _init_copilot(*, global_scope: bool, profile: str, port: int, backend: str) -> None:
    ...
    click.echo("Restart Copilot CLI to activate CoBWeaverClaw hooks and provider routing.")
...
def _init_codex(*, global_scope: bool, profile: str, port: int) -> None:
    ...
    click.echo("Restart Codex to activate CoBWeaverClaw configuration.")
...
def _init_openclaw(*, global_scope: bool, port: int) -> None:
    ...
    command = [*resolve_cobweaverclaw_compress_command(), "wrap", "openclaw", "--proxy-port", str(port)]
...
def _run_init_targets(
    *,
    ...
    # Register the cobweaverclaw_compress MCP server with every targeted agent that has
    ...
    _install_cobweaverclaw_compress_mcp_for_targets(targets=targets, port=port)


def _install_cobweaverclaw_compress_mcp_for_targets(*, targets: list[str], port: int) -> None:
    """Install the cobweaverclaw_compress MCP server into each detected target agent."""
    from cobweaverclaw_compress.mcp_registry import format_results, install_everywhere
...
def init(
    ...
    """Install durable CoBWeaverClaw integrations for supported agents."""
    if verbose:
...
def init_openclaw(ctx: click.Context) -> None:
    """Install the durable OpenClaw CoBWeaverClaw plugin."""
    _run_init_targets(
        targets=["openclaw"],
        global_scope=bool(_ctx_value(ctx, "global_scope")),
        port=int(_ctx_value(ctx, "port") or 8787),
