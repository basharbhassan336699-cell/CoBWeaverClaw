...
def perf(hours: float, raw: bool, output_format: str) -> None:
    ...
    \b
    Reads logs from ~/.cobweaverclaw_compress/logs/proxy.log and shows:
    ...
    \b
    Examples:
        cobweaverclaw_compress perf                      Analyze last 7 days
        cobweaverclaw_compress perf --hours 24           Analyze last 24 hours
        cobweaverclaw_compress perf --raw                Show raw parsed records
        cobweaverclaw_compress perf --format json        Aggregated report as JSON
        cobweaverclaw_compress perf --format csv --hours 24 > last-24h.csv
        cobweaverclaw_compress perf --format json --raw  Raw records as a JSON array
    """
    from cobweaverclaw_compress.perf.analyzer import (
        PERF_RECORD_FIELDS,
        build_perf_summary,
        format_report,
        parse_log_files,
    ...
    if raw:
        ...
        if not report.perf_records:
            click.echo("No PERF records found. Run the proxy first: cobweaverclaw_compress proxy")
    else:
