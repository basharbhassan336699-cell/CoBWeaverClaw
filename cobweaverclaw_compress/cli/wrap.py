"""Wrap CLI commands to run through CoBWeaverClaw proxy.

Usage:
    cobweaverclaw_compress wrap claude                    # Start proxy + context tool + claude
    cobweaverclaw_compress wrap copilot -- --model ...    # Start proxy + launch GitHub Copilot CLI
    cobweaverclaw_compress wrap codex                     # Start proxy + OpenAI Codex CLI
    cobweaverclaw_compress wrap aider                     # Start proxy + aider
    cobweaverclaw_compress wrap vibe                      # Start proxy + Mistral Vibe
    cobweaverclaw_compress wrap cursor                    # Start proxy + print Cursor config instructions
    cobweaverclaw_compress wrap openclaw                  # Install + configure OpenClaw plugin
    cobweaverclaw_compress wrap claude --no-context-tool  # Without CLI context-tool setup
    cobweaverclaw_compress wrap claude --port 9999        # Custom proxy port
    cobweaverclaw_compress wrap claude -- --model opus    # Pass args to claude
"""
...
from cobweaverclaw_compress._subprocess import pid_alive, run
...
from cobweaverclaw_compress import fsutil
from cobweaverclaw_compress._version import __version__ as _COBWEAVERCLAW_VERSION
from cobweaverclaw_compress.agent_savings import (
    apply_agent_savings_env_defaults,
)
from cobweaverclaw_compress.copilot_auth import (
    has_oauth_auth,
    resolve_client_bearer_token,
    resolve_copilot_api_url,
    resolve_subscription_bearer_token_details,
)
from cobweaverclaw_compress.providers.aider import build_launch_env as _build_aider_launch_env
from cobweaverclaw_compress.providers.claude import (
...
from cobweaverclaw_compress.providers.codex import build_launch_env as _build_codex_launch_env
from cobweaverclaw_compress.providers.codex.install import codex_uses_chatgpt_auth
from cobweaverclaw_compress.providers.codex.threads import retag_to_cobweaverclaw_compress, retag_to_native
from cobweaverclaw_compress.providers.copilot import (
...
from cobweaverclaw_compress.providers.cursor import render_setup_lines as _render_cursor_setup_lines
from cobweaverclaw_compress.providers.mistral_vibe import build_launch_env as _build_mistral_vibe_launch_env
from cobweaverclaw_compress.providers.openclaw import (
...
from cobweaverclaw_compress.providers.opencode import build_launch_env as _build_opencode_launch_env
from cobweaverclaw_compress.providers.opencode.config import (
    ...
    strip_opencode_cobweaverclaw_compress_blocks,
)
from cobweaverclaw_compress.proxy.project_context import with_project_prefix as _with_project_prefix
...
_CONTEXT_TOOL_ENV = "COBWEAVERCLAW_CONTEXT_TOOL"
...
_WRAP_PROXY_TIMEOUT_ENV = "COBWEAVERCLAW_WRAP_PROXY_TIMEOUT"
...
def _selected_context_tool() -> str:
    ...
    ``COBWEAVERCLAW_CONTEXT_TOOL=lean-ctx`` to let lean-ctx configure the supported
...
def _wrap_agent_savings_profile(agent_type: str) -> str | None:
    ...
    if agent_type not in _AGENT_SAVINGS_WRAP_AGENTS:
        return None
    return os.environ.get("COBWEAVERCLAW_SAVINGS_PROFILE") or None
...
def _print_telemetry_notice() -> None:
    ...
    Respects the COBWEAVERCLAW_TELEMETRY and COBWEAVERCLAW_TELEMETRY_WARN feature flags.
    Does nothing when telemetry or warnings are disabled.
    """
    from cobweaverclaw_compress.telemetry.beacon import format_telemetry_notice
...
def _check_proxy(port: int) -> bool:
    """Check if CoBWeaverClaw proxy is running on given port."""
    try:
...
def _format_unbindable_port_error(port: int, error: OSError, agent_type: str) -> str:
    ...
    command = "cobweaverclaw_compress proxy"
    if agent_type != "unknown":
        command = f"cobweaverclaw_compress wrap {agent_type}"
...
def _get_log_path() -> Path:
    ...
    from cobweaverclaw_compress import paths as _paths
...
def _start_proxy(
    ...
    *,
    ...
    """Start CoBWeaverClaw proxy as a background subprocess.

    Stdout and stderr are written to a dedicated sibling file, usually
    `~/.cobweaverclaw_compress/logs/proxy-stdio.log`, to avoid pipe deadlock risk without
    competing with the rotating `proxy.log` runtime log.
    """
    cmd = [sys.executable, "-m", "cobweaverclaw_compress.cli", "proxy", "--port", str(port)]

    # Forward COBWEAVERCLAW_MODE env var so the proxy respects the user's mode choice
    cobweaverclaw_compress_mode = os.environ.get("COBWEAVERCLAW_MODE")
    if cobweaverclaw_compress_mode:
        cmd.extend(["--mode", cobweaverclaw_compress_mode])
    ...
    _backend = backend or os.environ.get("COBWEAVERCLAW_BACKEND")
    if _backend:
        cmd.extend(["--backend", _backend])

    _anyllm = anyllm_provider or os.environ.get("COBWEAVERCLAW_ANYLLM_PROVIDER")
    if _anyllm:
        cmd.extend(["--anyllm-provider", _anyllm])

    _region = region or os.environ.get("COBWEAVERCLAW_REGION")
    if _region:
    ...
    if os.environ.get("CLAUDE_CODE_USE_VERTEX") or os.environ.get("ANTHROPIC_VERTEX_PROJECT_ID"):
        proxy_env.setdefault("COBWEAVERCLAW_HTTP2", "false")
    # Tell the proxy which agent is being wrapped (for traffic learning output)
    if agent_type != "unknown":
        proxy_env["COBWEAVERCLAW_AGENT_TYPE"] = agent_type
        proxy_env.setdefault("COBWEAVERCLAW_STACK", f"wrap_{agent_type}")
    ...
    # other ``cobweaverclaw_compress wrap`` instance routed through the same port.
...
def _setup_rtk(verbose: bool = False) -> Path | None:
    ...
    from cobweaverclaw_compress.rtk import get_rtk_path
    from cobweaverclaw_compress.rtk.installer import ensure_rtk, register_claude_hooks
...
def _patch_rtk_hook_absolute_path(rtk_path: Path, hook_script_path: Path | None = None) -> bool:
    ...
    to use the absolute path to the RTK binary CoBWeaverClaw manages.

    ``rtk init --global --auto-patch`` writes ``~/.claude/hooks/rtk-rewrite.sh``
    with a bare ``rtk`` command that depends on PATH lookup. Since
    ``~/.cobweaverclaw_compress/bin`` (where CoBWeaverClaw installs rtk) is not automatically
    ...
    # "/Users/Alice Smith/.cobweaverclaw_compress/bin/rtk") must be quoted, or the
...
def _setup_lean_ctx_agent(agent: str, verbose: bool = False) -> Path | None:
    ...
    from cobweaverclaw_compress.lean_ctx import get_lean_ctx_path
    from cobweaverclaw_compress.lean_ctx.installer import ensure_lean_ctx
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="cobweaverclaw_compress-lean-ctx-") as setup_cwd:
...
# Hook-command markers CoBWeaverClaw manages in Claude settings.json. unwrap drops
# any hook entry whose command contains one of these.
_COBWEAVERCLAW_HOOK_MARKERS = ("rtk-rewrite", "cobweaverclaw_compress-init-claude")

# Env vars CoBWeaverClaw's init/wrap inject into Claude settings.json; unwrap removes
# them. ENABLE_TOOL_SEARCH keeps Claude Code's tool deferral on behind the proxy
# (GH #746), paired with init/wrap setting it.
_COBWEAVERCLAW_ENV_KEYS = ("ANTHROPIC_BASE_URL", "ENABLE_TOOL_SEARCH")


def _remove_claude_rtk_hooks(settings_path: Path | None = None) -> bool:
    """Remove CoBWeaverClaw-managed entries from Claude settings.json.

    Reverses what ``cobweaverclaw_compress init claude`` and ``rtk init --auto-patch`` add:
      * PreToolUse / SessionStart hooks whose command contains a CoBWeaverClaw marker
        (``rtk-rewrite`` or ``cobweaverclaw_compress-init-claude``), and
                ...
                retained_hooks = [
                    ...
                    if not (
                        ...
                        and any(
                            ...
                            for marker in _COBWEAVERCLAW_HOOK_MARKERS
                        )
                    )
                ]
    ...
    if isinstance(env, dict):
        removed_keys = [k for k in _COBWEAVERCLAW_ENV_KEYS if env.pop(k, None) is not None]
        if removed_keys:
            changed = True
            if env:
                payload["env"] = env
...
def _vertex_target_api_url_from_claude_env(proxy_url: str) -> str | None:
    ...
    from cobweaverclaw_compress.providers.registry import DEFAULT_VERTEX_API_URL
...
def _wrap_marker_path(settings_path: Path) -> Path:
    ...
    Kept out of settings.local.json itself so CoBWeaverClaw's own bookkeeping never
    shows up as a stray key inside a file Claude Code's config loader parses.
    """
    return settings_path.parent / ".cobweaverclaw_compress_wrap_marker.json"
...
def _check_and_clear_stale_wrap_marker(settings_path: Path, *, key: str) -> str | None:
    ...
    click.echo(
        f"cobweaverclaw_compress: clearing stale {key} left by crashed wrap session (pid {marker.get('pid')})",
        err=True,
    )
...
def _write_claude_wrap_base_url(
    ...
    *,
    ...
    initial launch — routes through the CoBWeaverClaw proxy without touching the
...
def _setup_cobweaverclaw_compress_mcp(
    registrar: Any, port: int, *, verbose: bool = False, force: bool = False
) -> None:
    """Register the cobweaverclaw_compress MCP server with the given agent (idempotent).
    ...
    nothing — the agent has no ``cobweaverclaw_compress_retrieve`` tool to call.
    ...
    """
    from cobweaverclaw_compress.mcp_registry import build_cobweaverclaw_compress_spec, format_result
    ...
    spec = build_cobweaverclaw_compress_spec(proxy_url)
    ...
    line = format_result(
        ...
        verbose=verbose,
        overwrite_hint=f"cobweaverclaw_compress mcp install --proxy-url {proxy_url} --force",
...
def _setup_serena_mcp(
    ...
    A prior ``cobweaverclaw_compress wrap`` may have persisted a Serena entry built from an
    ...
    config is one CoBWeaverClaw installed, force-update it to the current spec. A
    user-managed Serena (absent from our ledger) is left untouched and the
    mismatch is reported as before.
    """
    from cobweaverclaw_compress.mcp_registry import build_serena_spec, format_result
    from cobweaverclaw_compress.mcp_registry.base import RegisterStatus
    from cobweaverclaw_compress.mcp_registry.ledger import cobweaverclaw_compress_installed_matching, record_install
    ...
    # Migrate a stale CoBWeaverClaw-installed entry. register_server won't overwrite
    # a differing spec without force, so an older CoBWeaverClaw Serena entry would
    # otherwise persist across re-wraps. Force-update it only when the ledger
    # proves CoBWeaverClaw installed the entry that's currently on disk — never a
    # user-managed Serena.
    if (
        result.status == RegisterStatus.MISMATCH
        and not force
        and cobweaverclaw_compress_installed_matching(registrar.name, registrar.get_server("serena"))
    ):
    ...
    line = format_result(
        ...
        label="Serena MCP",
        verbose=verbose,
        overwrite_hint="update or remove the existing serena MCP entry, then rerun cobweaverclaw_compress wrap",
        restart_hint=f"restart {registrar.display_name} if it was already running",
    )
    if line is not None:
        click.echo(line)
...
def _remove_cobweaverclaw_compress_installed_serena_mcp(registrar: Any) -> str:
    """Remove Serena MCP only if the ledger proves CoBWeaverClaw installed it."""
    from cobweaverclaw_compress.mcp_registry.ledger import clear_install, cobweaverclaw_compress_installed_matching

    current = registrar.get_server("serena")
    if not cobweaverclaw_compress_installed_matching(registrar.name, current):
        return "not_cobweaverclaw_compress_owned"
...
def _disable_serena_mcp(
    ...
    """Actively disable a CoBWeaverClaw-installed Serena entry, not merely skip it.

    Serena used to be registered by default, so a prior ``cobweaverclaw_compress wrap``
    ...
    run leaves that stale entry in place — so this removes the entry CoBWeaverClaw
    ...
    status = _remove_cobweaverclaw_compress_installed_serena_mcp(registrar)
    if status == "removed":
    ...
    elif status == "not_cobweaverclaw_compress_owned":
        click.echo(
            "  Serena MCP is present but user-managed — leaving it in place "
            "(--no-serena only removes entries CoBWeaverClaw installed)"
        )
    else:  # "failed"
        click.echo(
...
def _ensure_tokensave_binary(verbose: bool = False) -> Path | None:
    ...
    """
    from cobweaverclaw_compress.graph.tokensave_installer import ensure_tokensave, get_tokensave_path
...
def _setup_tokensave_mcp(registrar: Any, *, verbose: bool = False, force: bool = False) -> bool:
    ...
    CoBWeaverClaw-installed ``tokensave`` entry is force-updated to the current
    spec, while a user-managed entry is left untouched.
    """
    from cobweaverclaw_compress.mcp_registry import build_tokensave_spec, format_result
    from cobweaverclaw_compress.mcp_registry.base import RegisterStatus
    from cobweaverclaw_compress.mcp_registry.ledger import cobweaverclaw_compress_installed_matching, record_install
    ...
    # Migrate a stale CoBWeaverClaw-installed entry (e.g. an older binary path or
    # pinned version), mirroring the Serena migration path. Only force-update
    # when the ledger proves CoBWeaverClaw installed the entry on disk.
    if (
        result.status == RegisterStatus.MISMATCH
        and not force
        and cobweaverclaw_compress_installed_matching(registrar.name, registrar.get_server("tokensave"))
    ):
        result = registrar.register_server(spec, force=True)
        if result.status == RegisterStatus.REGISTERED:
    ...
    line = format_result(
        ...
        label="tokensave MCP",
        verbose=verbose,
        overwrite_hint="update or remove the existing tokensave MCP entry, then rerun cobweaverclaw_compress wrap",
        restart_hint=f"restart {registrar.display_name} if it was already running",
    )
    if line is not None:
        click.echo(line)
...
def _remove_cobweaverclaw_compress_installed_tokensave_mcp(registrar: Any) -> str:
    """Remove the tokensave MCP entry only if the ledger proves CoBWeaverClaw installed it."""
    from cobweaverclaw_compress.mcp_registry.ledger import clear_install, cobweaverclaw_compress_installed_matching
...
def _disable_tokensave_mcp(registrar: Any, *, verbose: bool = False) -> None:
    """Make ``--no-tokensave`` actively remove a CoBWeaverClaw-installed tokensave entry."""
    if not registrar.detect():
        if verbose:
            click.echo(f"  tokensave MCP: {registrar.display_name} not detected — skipping")
        return
    ...
    status = _remove_cobweaverclaw_compress_installed_tokensave_mcp(registrar)
    if status == "removed":
        ...
        click.echo(
            ...
            "(--no-tokensave only removes entries CoBWeaverClaw installed)"
        )
    else:  # "failed"
        click.echo(
...
def _setup_coding_compressor(registrar: Any, *, serena_context: str, **kwargs: Any) -> None:
    ...
    * tokensave is set up by default; on success it becomes the primary
      compressor and any CoBWeaverClaw-installed Serena entry is removed.
...
def _setup_code_graph(verbose: bool = False) -> bool:
    ...
    tokensave is CoBWeaverClaw's primary code-graph compressor and is normally
    ...
    server is no longer installed, and ``cobweaverclaw_compress unwrap`` still cleans up any
    legacy ``codebase-memory-mcp`` entry a prior wrap left behind.
    """
    from cobweaverclaw_compress.mcp_registry import ClaudeRegistrar
...
<!-- cobweaverclaw_compress:rtk-instructions -->
...
_RTK_MARKER = "<!-- cobweaverclaw_compress:rtk-instructions -->"

# Memory MCP markers
_MEMORY_MCP_MARKER = "# --- CoBWeaverClaw memory MCP (auto-injected) ---"
_MEMORY_MCP_END = "# --- end CoBWeaverClaw memory ---"
_MEMORY_AGENTS_MARKER = "<!-- cobweaverclaw_compress:memory-instructions -->"

# Codex config injection markers
_CODEX_TOP_LEVEL_MARKER = "# --- CoBWeaverClaw proxy (auto-injected by cobweaverclaw_compress wrap codex) ---"
_CODEX_END_MARKER = "# --- end CoBWeaverClaw ---"
_CODEX_MCP_MARKER = "# --- CoBWeaverClaw MCP server ---"
_CODEX_MCP_END = "# --- end CoBWeaverClaw MCP server ---"
# File name used for the pre-wrap snapshot of the Codex config file.  The
# snapshot lets `cobweaverclaw_compress unwrap codex` restore the exact prior state, even
# if the user had their own `model_provider` / `[model_providers.*]` config
# before running wrap.
_CODEX_CONFIG_BACKUP_SUFFIX = ".cobweaverclaw_compress-backup"
...
def _strip_codex_cobweaverclaw_compress_blocks(
    content: str,
    *,
    remove_mcp: bool = False,
    remove_named_mcp: bool = True,
) -> str:
    """Remove all CoBWeaverClaw-managed blocks from a Codex ``config.toml`` string.
    ...
    if remove_mcp:
        # Remove CoBWeaverClaw-managed MCP blocks written by `wrap codex`.
        content = _remove_marker_span(content, _CODEX_MCP_MARKER, _CODEX_MCP_END)
        if remove_named_mcp:
            content = re.sub(
                r"(?ms)^# --- CoBWeaverClaw MCP server: [^\n]+ ---\n.*?"
                r"^# --- end CoBWeaverClaw MCP server: [^\n]+ ---\n?",
                "",
                content,
            )
    ...
    content = re.sub(r'(?m)^[ \t]*model_provider[ \t]*=[ \t]*"cobweaverclaw_compress"[ \t]*\r?\n', "", content)
    content = re.sub(
    ...
    # Strip any orphaned `[model_providers.cobweaverclaw_compress]` table with the fields we
    # write.  We only remove it if the table is recognisably ours (base_url
    # mentions localhost and a CoBWeaverClaw proxy port).  This protects users who
    # happen to have a differently configured `cobweaverclaw_compress` provider.
    orphan_cobweaverclaw_compress_table = re.compile(
        r"(?ms)^\[model_providers\.cobweaverclaw_compress\][^\[]*?"
        r'base_url[ \t]*=[ \t]*"http://127\.0\.0\.1:\d+/v1"[^\[]*?'
        r"(?=^\[|\Z)"
    )
    content = orphan_cobweaverclaw_compress_table.sub("", content)

    return content.lstrip("\n").rstrip() + "\n" if content.strip() else ""


# Top-level bare keys we redirect to cobweaverclaw_compress values when the user already
...
def _strip_existing_codex_cobweaverclaw_compress_provider_table(content: str) -> str:
    """Remove a pre-existing ``[model_providers.cobweaverclaw_compress]`` table before wrap."""
    if "[model_providers.cobweaverclaw_compress]" not in content:
        return content
...
def _redirect_existing_top_level_keys(content: str, port: int) -> str:
    ...
    which would break ``codex`` startup after ``cobweaverclaw_compress wrap codex`` runs
    ...
    the value with the cobweaverclaw_compress one and append ``# was: <original-value>``
    ...
    pre-wrap file can be restored byte-for-byte on ``cobweaverclaw_compress unwrap
    codex``.
    ...
    def _make_replacer(current_key: str, current_port: int) -> Callable[[re.Match[str]], str]:
        def _replace(match: re.Match[str]) -> str:
            ...
            if current_key == "model_provider":
                new_value = "cobweaverclaw_compress"
...
def _codex_config_has_cobweaverclaw_compress_markers(content: str) -> bool:
...
def _snapshot_codex_config_if_unwrapped(config_file: Path, backup_file: Path) -> None:
    ...
    Called as the first step of every CoBWeaverClaw injection into Codex's
    ``config.toml``.  Guarantees that ``cobweaverclaw_compress unwrap codex`` can restore the
    ...
    * If the config already contains any CoBWeaverClaw-managed Codex marker, a wrap
    ...
    try:
        ...
        return
    if _codex_config_has_cobweaverclaw_compress_markers(content):
        return
...
def _ensure_rtk_binary(verbose: bool = False) -> Path | None:
    ...
    from cobweaverclaw_compress.rtk import get_rtk_path
    from cobweaverclaw_compress.rtk.installer import ensure_rtk
...
# case-insensitively by cobweaverclaw_compress.proxy.project_context.PROJECT_HEADER).
_PROJECT_HEADER_NAME = "X-CoBWeaverClaw-Project"


def _project_name_from_cwd() -> str | None:
    """Project label for X-CoBWeaverClaw-Project: basename of the launch directory.
...
def _apply_project_header_env(env: dict[str, str]) -> None:
    """Inject X-CoBWeaverClaw-Project into ``ANTHROPIC_CUSTOM_HEADERS``.
    ...
    CoBWeaverClaw proxy uses the X-CoBWeaverClaw-Project header for per-project
    savings attribution.  An existing user-supplied x-cobweaverclaw_compress-project
...
def _inject_codex_provider_config(port: int) -> None:
    """Inject a CoBWeaverClaw model provider into Codex's config.toml.

    Two keys need to be in effect for the proxy to route all traffic:

    * ``model_provider = "cobweaverclaw_compress"`` — selects the custom provider for
    ...
    * ``openai_base_url = "http://127.0.0.1:{port}/v1"`` — overrides the
    ...
    in place to the cobweaverclaw_compress values (with the previous value kept in a
    ...
    ``config.toml.cobweaverclaw_compress-backup`` so ``cobweaverclaw_compress unwrap codex``
    ...
    provider_section = (
        ...
        "[model_providers.cobweaverclaw_compress]\n"
        'name = "OpenAI via CoBWeaverClaw proxy"\n'
        f'base_url = "http://127.0.0.1:{port}/v1"\n'
        f"supports_websockets = true\n"
        f"{requires_openai_auth}"
        # Per-project savings: Codex sends the header only when the mapped
        # env var (COBWEAVERCLAW_PROJECT, set by `cobweaverclaw_compress wrap codex`) exists at
        # Codex runtime.  Inline table keeps the key inside this section so
        # _strip_codex_cobweaverclaw_compress_blocks removes it with the rest of the block.
        f'env_http_headers = {{ "{_PROJECT_HEADER_NAME}" = "COBWEAVERCLAW_PROJECT" }}\n'
        f"{_CODEX_END_MARKER}\n"
    )

    # The two redirectable keys and their cobweaverclaw_compress target values.
    _REDIRECT_TARGETS = {
        "model_provider": "cobweaverclaw_compress",
    ...
    try:
        ...
        if config_file.exists():
            content = _read_text(config_file)
            # Remove any prior CoBWeaverClaw-managed blocks before re-injecting so
            # the operation is idempotent and supports port changes.
            content = _strip_codex_cobweaverclaw_compress_blocks(content)
            content = _strip_existing_codex_cobweaverclaw_compress_provider_table(content)
            ...
            # to the cobweaverclaw_compress values; for keys the user has not declared,
        ...
        click.echo(f"  Codex config: injected CoBWeaverClaw provider (WS + HTTP) into {config_file}")
        # Pull existing native threads into the cobweaverclaw_compress-provider menu so Codex's
        # history list stays whole once it routes through CoBWeaverClaw. Best-effort.
        retag_to_cobweaverclaw_compress(_codex_home_dir())
    except Exception as e:
...
def _restore_codex_provider_config() -> tuple[str, Path]:
    ...
    * ``"cleaned"``  — no backup existed, but the CoBWeaverClaw-managed block was
      found and stripped out (preserving surrounding user content).
    * ``"removed"``  — the config file only contained CoBWeaverClaw-managed
      content (created by wrap) and has been deleted.
    * ``"noop"``     — nothing to undo; no CoBWeaverClaw marker and no backup.
    """
    config_file, backup_file = _codex_config_paths()
    ...
    if config_file.exists():
        ...
        if _codex_config_has_cobweaverclaw_compress_markers(original):
            ...
            cleaned = _strip_codex_cobweaverclaw_compress_blocks(
                original,
                remove_mcp=True,
                remove_named_mcp=remove_named_mcp,
            )
            if not cleaned.strip():
                # Nothing left but CoBWeaverClaw content — remove the file entirely
...
def _emit_wrap_interrupted(agent: str, marker_path: Path | None) -> None:
    ...
    re-running the same ``cobweaverclaw_compress wrap <agent>`` command is idempotent and
    safe.
    """
    if marker_path is not None:
        click.echo(
            f"\n  Wrap was interrupted; marker file at {marker_path} is on "
            f"disk. Rerun `cobweaverclaw_compress wrap {agent}` to retry — it's idempotent."
        )
    else:
        click.echo(
            f"\n  Wrap was interrupted before any on-disk changes. Rerun "
            f"`cobweaverclaw_compress wrap {agent}` to retry — it's idempotent."
        )
...
def _print_wrap_banner(agent: str) -> None:
    """Print a centered ``COBWEAVERCLAW WRAP: <AGENT>`` banner.
    ...
    """
    title = f"COBWEAVERCLAW WRAP: {agent.upper()}"
...
def _remove_rtk_instructions(file_path: Path) -> bool:
    """Remove CoBWeaverClaw's marker-fenced rtk guidance from an instruction file."""
    if not file_path.exists():
        return False

    content = _read_text(file_path)
    end_marker = "<!-- /cobweaverclaw_compress:rtk-instructions -->"
...
def _inject_memory_mcp_config(user_id: str) -> None:
    """Register cobweaverclaw_compress memory as an MCP server in Codex's config.toml.
    ...
    mcp_section = (
        f"\n{_MEMORY_MCP_MARKER}\n"
        f"[mcp_servers.cobweaverclaw_compress_memory]\n"
        f'command = "{python_bin}"\n'
        f'args = ["-m", "cobweaverclaw_compress.memory.mcp_server", "--user", "{user_id}"]\n'
...
def _inject_memory_agents_md(file_path: Path) -> bool:
    ...
    memory_block = (
        ...
        "## Memory\n\n"
        "Use the `cobweaverclaw_compress_memory` MCP server for persistent cross-session knowledge.\n\n"
...
def _inject_continue_rtk_systemmessage(config_file: Path, verbose: bool = False) -> bool:
    ...
    * each ``models[i].systemMessage`` where ``models[i]`` is a dict
    ...
    The RTK marker (``<!-- cobweaverclaw_compress:rtk-instructions -->``) is the idempotency
...
def _proxy_health_config(payload: dict[str, Any] | None) -> dict[str, Any] | None:
    """Extract the config block from a CoBWeaverClaw /health payload."""
    if payload is None:
        return None
...
def _agent_savings_config_mismatches(
    ...
    checks: tuple[tuple[str, str, str, str], ...] = (
        ("COBWEAVERCLAW_SAVINGS_PROFILE", "savings_profile", "savings-profile", "str"),
        ("COBWEAVERCLAW_TARGET_RATIO", "target_ratio", "target-ratio", "float"),
        (
            "COBWEAVERCLAW_COMPRESS_USER_MESSAGES",
            "compress_user_messages",
            "compress-user-messages",
            "bool",
        ),
        (
            "COBWEAVERCLAW_COMPRESS_SYSTEM_MESSAGES",
            "compress_system_messages",
            "compress-system-messages",
            "bool",
        ),
        ("COBWEAVERCLAW_PROTECT_RECENT", "protect_recent", "protect-recent", "int"),
        (
            "COBWEAVERCLAW_PROTECT_ANALYSIS_CONTEXT",
            "protect_analysis_context",
            "protect-analysis-context",
            "bool",
        ),
        ("COBWEAVERCLAW_MIN_TOKENS", "min_tokens_to_crush", "min-tokens", "int"),
        ("COBWEAVERCLAW_MAX_ITEMS", "max_items_after_crush", "max-items", "int"),
        (
            "COBWEAVERCLAW_SMART_CRUSHER_COMPACTION",
            "smart_crusher_with_compaction",
            "smart-crusher-compaction",
            "bool",
        ),
        ("COBWEAVERCLAW_ACCURACY_GUARD", "accuracy_guard", "accuracy-guard", "str"),
    )
...
def _proxy_needs_version_restart(payload: dict[str, Any] | None) -> bool:
    """Return True when a running CoBWeaverClaw proxy uses a different package version."""
    running_version = _proxy_version(payload)
    return (
        running_version is not None
        and running_version != "unknown"
        and _COBWEAVERCLAW_VERSION != "unknown"
        and running_version != _COBWEAVERCLAW_VERSION
    )
...
def _stop_local_proxy_for_unwrap(port: int) -> str:
    """Stop a local CoBWeaverClaw proxy for durable unwrap commands.

    Returns a status string:
      * ``"stopped"``: a CoBWeaverClaw proxy was identified and stopped.
      * ``"not_running"``: nothing is listening on the requested port.
      * ``"unidentified"``: something is listening, but it did not expose
        CoBWeaverClaw's health/config payload, so we did not kill it.
      * ``"no_pid"``: the service looked like CoBWeaverClaw but did not expose a PID.
...
def _echo_unwrap_proxy_stop_status(status: str, port: int) -> None:
    ...
    if status == "stopped":
        click.echo(f"  Stopped local CoBWeaverClaw proxy on port {port}.")
    elif status == "not_running":
        click.echo(f"  No local CoBWeaverClaw proxy detected on port {port}.")
    elif status == "unidentified":
        click.echo(
            f"  Warning: port {port} is in use, but it did not look like CoBWeaverClaw; left it running."
        )
    elif status == "no_pid":
        click.echo(
            f"  Warning: CoBWeaverClaw proxy on port {port} did not expose a PID; left it running."
        )
    else:
        click.echo(f"  Warning: failed to stop CoBWeaverClaw proxy on port {port}; stop it manually.")


def _find_persistent_manifest(port: int) -> Any:
    """Return a matching persistent deployment manifest for the requested port."""
    from cobweaverclaw_compress.install.state import list_manifests
...
def _recover_persistent_proxy(port: int) -> bool:
    ...
    from cobweaverclaw_compress.install.health import probe_ready
    from cobweaverclaw_compress.install.models import InstallPreset, SupervisorKind
    from cobweaverclaw_compress.install.runtime import start_detached_agent, start_persistent_docker, wait_ready
    from cobweaverclaw_compress.install.supervisors import start_supervisor
...
def _restart_persistent_proxy(manifest: Any, port: int) -> bool:
    ...
    from cobweaverclaw_compress.install.models import InstallPreset, SupervisorKind
    from cobweaverclaw_compress.install.runtime import (
    ...
    click.echo(
        ...
        f"with CoBWeaverClaw {_COBWEAVERCLAW_VERSION}..."
    )
    try:
        if manifest.preset == InstallPreset.PERSISTENT_DOCKER.value:
...
def _should_use_copilot_oauth(
    *,
    ...
    effective_backend = backend or os.environ.get("COBWEAVERCLAW_BACKEND")
...
def _push_runtime_env(port: int, no_proxy: bool) -> None:
    ...
    if no_proxy:
        return
    from cobweaverclaw_compress.proxy import runtime_env as _rt
    ...
    if not no_proxy:
            ...
            if probe_ready(manifest.health_url):
                ...
                if helpers._proxy_needs_version_restart(health_payload):
                    ...
                    if active_sessions > 0 or other_wrappers:
                        ...
                        click.echo(
                            f"  Proxy on port {port} is running CoBWeaverClaw {running_version}; "
                            f"current CLI is {_COBWEAVERCLAW_VERSION}."
                        )
                        click.echo(
                    ...
                    raise click.ClickException(
                        ...
                        f"is running stale CoBWeaverClaw {running_version} and could not be restarted."
                    )
        ...
        if helpers._check_proxy(port):
            ...
            if helpers._proxy_needs_version_restart(health_payload):
                ...
                click.echo(
                    f"  Proxy on port {port} is running CoBWeaverClaw {running_version}; "
                    f"restarting with {_COBWEAVERCLAW_VERSION}..."
                )
        ...
        click.echo(f"  Starting CoBWeaverClaw proxy on port {port}...")
        try:
            proc = cast(
...
def _make_cleanup(proxy_proc_holder: list, port: int = 8787) -> Any:
    ...
    Only kills the proxy when no other live cobweaverclaw_compress-wrapped clients remain,
...
def _launch_tool(
    ...
    *,
    ...
    try:
        click.echo()
        padded = f"COBWEAVERCLAW WRAP: {tool_label}".center(47)
        click.echo("  ╔═══════════════════════════════════════════════╗")
        click.echo(f"  ║{padded}║")
        click.echo("  ╚═══════════════════════════════════════════════╝")
        click.echo()
        ...
        click.echo()
        click.echo(f"  Launching {tool_label} (API routed through CoBWeaverClaw)...")
...
def _build_openclaw_plugin_entry(
    *,
    ...
    """Merge managed CoBWeaverClaw plugin settings with any existing entry payload."""
...
def _write_openclaw_plugin_entry(openclaw_bin: str, entry: dict[str, object]) -> None:
    """Persist the CoBWeaverClaw plugin config entry."""
    _run_checked(
        [
            openclaw_bin,
            "config",
            "set",
            "plugins.entries.cobweaverclaw_compress",
            json.dumps(entry, separators=(",", ":")),
            "--strict-json",
        ],
        action="openclaw config set plugins.entries.cobweaverclaw_compress",
    )
...
def _copy_openclaw_plugin_into_extensions(
    *,
    ...
    target_dir = extensions_dir / "cobweaverclaw_compress"
...
@main.group()
def wrap() -> None:
    """Wrap CLI tools to run through CoBWeaverClaw.

    \b
    Starts a CoBWeaverClaw proxy, configures the environment, and launches
    the target tool so all API calls route through CoBWeaverClaw automatically.

    \b
    Supported tools (one Click subcommand per tool):
        cobweaverclaw_compress wrap claude              # Claude Code (Anthropic)
        cobweaverclaw_compress wrap codex               # OpenAI Codex CLI
        cobweaverclaw_compress wrap copilot -- --model claude-sonnet-4-20250514
        cobweaverclaw_compress wrap aider               # Aider
        cobweaverclaw_compress wrap vibe                # Mistral Vibe
        cobweaverclaw_compress wrap cursor              # Cursor (prints config instructions)
        cobweaverclaw_compress wrap cline               # Cline (VS Code; prints config instructions)
        cobweaverclaw_compress wrap continue            # Continue (VS Code/JetBrains; injects systemMessage)
        cobweaverclaw_compress wrap goose               # Goose (Block) CLI
        cobweaverclaw_compress wrap openhands           # OpenHands CLI
        cobweaverclaw_compress wrap openclaw            # OpenClaw plugin bootstrap
        cobweaverclaw_compress wrap opencode            # OpenCode CLI

    \b
    `wrap` vs `proxy`:
        - `cobweaverclaw_compress wrap <tool>` — convenience: starts the proxy for you,
          sets the right env vars, and launches the wrapped CLI.
        - `cobweaverclaw_compress proxy` — just the proxy. Use this with any
...
@main.group()
def unwrap() -> None:
    """Undo durable CoBWeaverClaw wrapping for supported tools."""
...
@click.option(
    "--no-mcp",
    is_flag=True,
    help="Skip cobweaverclaw_compress MCP server registration (compression markers will be unactionable)",
)
@click.option(
    "--no-tokensave",
    is_flag=True,
...
@click.option(
    ...
    "(env: COBWEAVERCLAW_BACKEND). For Vertex, prefer CLAUDE_CODE_USE_VERTEX=1 (native, "
    "keeps your GCP auth) over a litellm backend.",
)
@click.option(
    "--region",
    default=None,
    help="Cloud region for Vertex/Bedrock backends (env: COBWEAVERCLAW_REGION).",
)
@click.option(
    "--1m",
    "context_1m",
...
def claude(
    ...
    """Launch Claude Code through CoBWeaverClaw proxy.

    \b
    Sets ANTHROPIC_BASE_URL to route all Anthropic API calls through CoBWeaverClaw.
    All unknown flags are passed through to claude (e.g. --resume, --model).

    \b
    Examples:
        cobweaverclaw_compress wrap claude                    # Start everything
        cobweaverclaw_compress wrap claude --memory           # With persistent memory
        cobweaverclaw_compress wrap claude --resume <id>      # Resume a session
        cobweaverclaw_compress wrap claude -- -p              # Claude in print mode
        cobweaverclaw_compress wrap claude                    # tokensave code graph (primary)
        cobweaverclaw_compress wrap claude --no-tokensave     # Skip tokensave; fall back to Serena
        cobweaverclaw_compress wrap claude --serena           # Also register the Serena backup
        cobweaverclaw_compress wrap claude --no-context-tool  # Skip CLI context-tool setup
        cobweaverclaw_compress wrap claude --no-mcp           # Skip MCP retrieve tool registration
        cobweaverclaw_compress wrap claude --no-serena        # Never register the Serena backup
        cobweaverclaw_compress wrap claude --1m               # Preserve the 1M context window
    """
    if prepare_only:
        if not no_rtk:
            if _selected_context_tool() == _CONTEXT_TOOL_LEAN_CTX:
    ...
    # Memory sync BEFORE proxy startup — sync cobweaverclaw_compress DB ↔ Claude's files
    if memory:
        try:
            mem_dir = Path.cwd() / ".cobweaverclaw_compress"
            ...
            sync_result = run(
                [
                    sys.executable,
                    "-m",
                    "cobweaverclaw_compress.memory.sync",
                    "--db",
                    _sync_db,
                    "--user",
                    _sync_user,
    ...
    try:
        click.echo()
        click.echo("  ╔═══════════════════════════════════════════════╗")
        click.echo("  ║            COBWEAVERCLAW WRAP: CLAUDE              ║")
        click.echo("  ╚═══════════════════════════════════════════════╝")
        click.echo()
        ...
        # CoBWeaverClaw and the proxy compresses the request, then forwards to the
        ...
        # by CoBWeaverClaw. This is the turnkey Vertex compression path.
        ...
        if not no_mcp:
            ...
            _setup_cobweaverclaw_compress_mcp(ClaudeRegistrar(), port, verbose=verbose)
        elif verbose:
        ...
        click.echo()
        click.echo("  Launching Claude Code (API routed through CoBWeaverClaw)...")
        if use_vertex:
            click.echo(
        ...
        if use_vertex:
            ...
            # we only redirect its Vertex endpoint to CoBWeaverClaw.
        ...
        # daemon's environment) also route through CoBWeaverClaw.
        ...
        # directory's name via X-CoBWeaverClaw-Project (user override wins).
...
@click.option("--no-stop-proxy", is_flag=True, help="Do not stop the local CoBWeaverClaw proxy")
@click.option("--keep-mcp", is_flag=True, help="Keep CoBWeaverClaw MCP registrations")
...
def unwrap_claude(
    ...
    """Undo durable setup from ``cobweaverclaw_compress wrap claude``."""
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║          COBWEAVERCLAW UNWRAP: CLAUDE              ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if not keep_mcp:
        ...
        if registrar.detect():
            removed_cobweaverclaw_compress = registrar.unregister_server("cobweaverclaw_compress")
            removed_code_graph = registrar.unregister_server(_CBM_MCP_SERVER_NAME)
            tokensave_status = _remove_cobweaverclaw_compress_installed_tokensave_mcp(registrar)
            serena_status = _remove_cobweaverclaw_compress_installed_serena_mcp(registrar)
            if removed_cobweaverclaw_compress:
                click.echo("  Removed CoBWeaverClaw MCP retrieve tool from Claude.")
            else:
                click.echo("  CoBWeaverClaw MCP retrieve tool was not registered in Claude.")
            ...
            if tokensave_status == "removed":
                click.echo("  Removed CoBWeaverClaw-installed tokensave MCP server from Claude.")
            elif tokensave_status == "failed":
                click.echo(
                    "  tokensave MCP server matched CoBWeaverClaw ledger but could not be removed."
                )
            if serena_status == "removed":
                click.echo("  Removed CoBWeaverClaw-installed Serena MCP server from Claude.")
            elif serena_status == "failed":
                click.echo("  Serena MCP server matched CoBWeaverClaw ledger but could not be removed.")
        else:
    ...
    click.echo()
    click.echo("✓ Claude is no longer durably wrapped by CoBWeaverClaw.")
...
@click.option(
    "--backend",
    default=None,
    help="API backend for the proxy: 'anthropic', 'anyllm', 'litellm-vertex', etc. (env: COBWEAVERCLAW_BACKEND)",
)
@click.option(
    "--anyllm-provider",
    default=None,
    help="Provider for any-llm backend: openai, mistral, groq, etc. (env: COBWEAVERCLAW_ANYLLM_PROVIDER)",
)
@click.option(
    "--region", default=None, help="Cloud region for Bedrock/Vertex (env: COBWEAVERCLAW_REGION)"
)
@click.option(
    "--provider-type",
...
@click.option(
    ...
    is_flag=True,
    help=(
        "Experimental: route GitHub-authenticated Copilot CLI traffic through CoBWeaverClaw "
...
def copilot(
    ...
    """Launch GitHub Copilot CLI through CoBWeaverClaw proxy.

    \b
    Configures Copilot CLI BYOK provider variables so Copilot routes through
    the local CoBWeaverClaw proxy. In auto mode, the wrapper uses Anthropic-style
    ...
    Examples:
        cobweaverclaw_compress wrap copilot -- --model claude-sonnet-4-20250514
        cobweaverclaw_compress wrap copilot --backend anyllm --anyllm-provider groq -- --model gpt-4o
        cobweaverclaw_compress wrap copilot --provider-type openai --wire-api responses -- --model gpt-5.4
        cobweaverclaw_compress wrap copilot --subscription -- --model gpt-4.1
        cobweaverclaw_compress wrap copilot --no-context-tool -- --prompt "explain this file"
    ...
    if _should_use_copilot_oauth(
        ...
        if not client_bearer:
            raise click.ClickException(
                ...
                "token, but none could be resolved. Run `cobweaverclaw_compress copilot-auth login` first, or set "
        ...
        if not env.get("COPILOT_PROVIDER_API_KEY"):
            ...
            click.echo(
                ...
                f"  `cobweaverclaw_compress wrap copilot` uses Copilot's BYOK mode, which bypasses GitHub's\n"
                f"  Copilot API and routes requests directly to the model provider through the\n"
                f"  CoBWeaverClaw proxy. A GitHub Copilot subscription alone is not sufficient.\n\n"
                f"  Set one of:\n"
    ...
    if not subscription and not _copilot_model_configured(copilot_args, env):
        ...
        if _is_auto_model(raw_model):
            click.echo(
                ...
                "      cobweaverclaw_compress wrap copilot --subscription -- --model auto"
            )
            raise SystemExit(1)
        else:
            click.echo(
...
def unwrap_copilot(port: int, no_stop_proxy: bool) -> None:
    """Undo durable setup from ``cobweaverclaw_compress wrap copilot``."""
    instructions = Path.cwd() / ".github" / "copilot-instructions.md"
    if _remove_rtk_instructions(instructions):
        click.echo("  Removed CoBWeaverClaw rtk instructions from Copilot.")
    else:
        click.echo("  No CoBWeaverClaw rtk instructions found for Copilot.")
...
def codex(
    ...
    """Launch OpenAI Codex CLI through CoBWeaverClaw proxy.

    \b
    Sets OPENAI_BASE_URL to route all OpenAI API calls through CoBWeaverClaw.
    Sets up the selected CLI context tool so Codex uses token-optimized
    commands (60-90% savings on shell output). Also
    registers the cobweaverclaw_compress MCP server in the active Codex config file
    so Codex can call ``cobweaverclaw_compress_retrieve`` on compression markers.

    \b
    Examples:
        cobweaverclaw_compress wrap codex                         # Start proxy + context tool + mcp + codex
        cobweaverclaw_compress wrap codex -- "fix the bug"        # Pass prompt to codex
        cobweaverclaw_compress wrap codex --no-context-tool       # Skip CLI context-tool setup
        cobweaverclaw_compress wrap codex --no-mcp                # Skip MCP retrieve tool registration
        cobweaverclaw_compress wrap codex --no-tokensave          # Skip tokensave; fall back to Serena
        cobweaverclaw_compress wrap codex --serena                # Also register the Serena backup
        cobweaverclaw_compress wrap codex --no-serena             # Never register the Serena backup
        cobweaverclaw_compress wrap codex --port 9999             # Custom proxy port
        cobweaverclaw_compress wrap codex --backend anyllm --anyllm-provider groq
    """
    # Snapshot Codex config.toml BEFORE any wrap-time mutation so
    # `cobweaverclaw_compress unwrap codex` can restore the user's pre-wrap state
    # byte-for-byte. The snapshot is a no-op if the backup already exists
    # or if the file already has CoBWeaverClaw markers, so this is safe to
    ...
    # Register cobweaverclaw_compress MCP server in Codex config.toml so Codex can
    # call cobweaverclaw_compress_retrieve on compression markers from the proxy.
    if not no_mcp:
        from cobweaverclaw_compress.mcp_registry import CodexRegistrar
        ...
        _setup_cobweaverclaw_compress_mcp(CodexRegistrar(), port, verbose=verbose, force=True)
    elif verbose:
        click.echo("  Skipping MCP retrieve tool (--no-mcp)")
    ...
    if memory:
        ...
        try:
            ...
            from cobweaverclaw_compress.memory.sync import _build_sync_backend, sync_import
            from cobweaverclaw_compress.memory.sync_adapters.claude_code import (
                ClaudeCodeAdapter,
    ...
    # X-CoBWeaverClaw-Project header to COBWEAVERCLAW_PROJECT via env_http_headers, so
    # Codex sends it only when this var is set.  A user-set value wins.
    _codex_project = _project_name_from_cwd()
    if _codex_project and "COBWEAVERCLAW_PROJECT" not in env:
        env["COBWEAVERCLAW_PROJECT"] = _codex_project

    # Inject CoBWeaverClaw provider into Codex config so WebSocket traffic also
...
def aider(
    ...
    """Launch aider through CoBWeaverClaw proxy.

    \b
    Sets OPENAI_API_BASE to route all API calls through CoBWeaverClaw.
    Sets up the selected CLI context tool so aider uses token-optimized commands.

    \b
    Examples:
        cobweaverclaw_compress wrap aider                              # Start proxy + context tool + aider
        cobweaverclaw_compress wrap aider -- --model gpt-4o            # Use GPT-4o
        cobweaverclaw_compress wrap aider -- --model claude-sonnet-4   # Use Claude
        cobweaverclaw_compress wrap aider --no-context-tool            # Skip CLI context-tool setup
        cobweaverclaw_compress wrap aider --backend litellm-vertex --region us-central1
    """
...
def vibe(
    ...
    """Launch Mistral Vibe through CoBWeaverClaw proxy.

    \b
    Sets VIBE_PROVIDERS to route all Mistral API calls through CoBWeaverClaw.

    \b
    Examples:
        cobweaverclaw_compress wrap vibe                         # Start proxy + vibe
        cobweaverclaw_compress wrap vibe -- "fix the bug"        # Pass prompt to vibe
        cobweaverclaw_compress wrap vibe --port 9999             # Custom proxy port
        cobweaverclaw_compress wrap vibe --no-context-tool       # Skip CLI context-tool setup
    """
    if prepare_only:
        return
...
def cursor(
    ...
    """Start CoBWeaverClaw proxy for use with Cursor.
    ...
    \b
    Example:
        cobweaverclaw_compress wrap cursor                # Start proxy + context-tool instructions
        cobweaverclaw_compress wrap cursor --no-context-tool  # Proxy only, no CLI context tool
        cobweaverclaw_compress wrap cursor --port 9999    # Custom proxy port
    """
...
def cline(
    ...
    """Start CoBWeaverClaw proxy for use with Cline (VS Code extension).
    ...
    the API Base URL to point at the local CoBWeaverClaw proxy.

    \b
    Uninstall: there is no ``cobweaverclaw_compress unwrap cline`` subcommand. To remove the
    injected guidance, hand-edit ``.clinerules`` at the project root and
    delete everything between ``<!-- cobweaverclaw_compress:rtk-instructions -->`` and
    ``<!-- /cobweaverclaw_compress:rtk-instructions -->`` (inclusive). If ``lean-ctx`` mode
    ...
    \b
    Examples:
        cobweaverclaw_compress wrap cline                  # Start proxy + .clinerules instructions
        cobweaverclaw_compress wrap cline --no-context-tool # Proxy only, no CLI context tool
        cobweaverclaw_compress wrap cline --port 9999      # Custom proxy port
    """
...
def continue_dev(
    ...
    """Start CoBWeaverClaw proxy for use with Continue (VS Code / JetBrains).
    ...
    \b
    Uninstall: there is no ``cobweaverclaw_compress unwrap continue`` subcommand. To remove
    the injected guidance, hand-edit ``.continue/config.json`` and delete
    everything between ``<!-- cobweaverclaw_compress:rtk-instructions -->`` and
    ``<!-- /cobweaverclaw_compress:rtk-instructions -->`` (inclusive) from every
    ...
    \b
    Examples:
        cobweaverclaw_compress wrap continue                # Start proxy + inject systemMessage
        cobweaverclaw_compress wrap continue --no-context-tool   # Proxy only
        cobweaverclaw_compress wrap continue --port 9999    # Custom proxy port
        cobweaverclaw_compress wrap continue --config path/to/config.json
    """
...
def goose(
    ...
    """Launch Goose (Block) CLI through CoBWeaverClaw proxy.

    \b
    Sets OPENAI_BASE_URL and ANTHROPIC_BASE_URL to route Goose's API calls
    through CoBWeaverClaw. Sets up the selected CLI context tool by injecting RTK
    guidance into .goosehints at the project root (Goose reads this file as
    extra system context).

    \b
    Uninstall: there is no ``cobweaverclaw_compress unwrap goose`` subcommand. To remove the
    ...
    \b
    Examples:
        cobweaverclaw_compress wrap goose                          # Start proxy + context tool + goose
        cobweaverclaw_compress wrap goose -- session               # Start a Goose session
        cobweaverclaw_compress wrap goose -- --provider anthropic  # Pass args to goose
        cobweaverclaw_compress wrap goose --no-context-tool        # Skip CLI context-tool setup
    """
...
def openhands(
    ...
    """Launch OpenHands CLI through CoBWeaverClaw proxy.

    \b
    Sets OPENAI_BASE_URL / ANTHROPIC_BASE_URL to route OpenHands' API calls
    through CoBWeaverClaw. Instructions are injected via the
    ...
    ``<!-- cobweaverclaw_compress:rtk-instructions -->`` marker. To uninstall, simply do not
    ...
    \b
    Examples:
        cobweaverclaw_compress wrap openhands                # Start proxy + context tool + openhands
        cobweaverclaw_compress wrap openhands -- --task ...  # Pass args to openhands
        cobweaverclaw_compress wrap openhands --no-context-tool
    """
...
@click.option(
    "--plugin-spec",
    default="cobweaverclaw_compress-ai/openclaw",
    show_default=True,
...
@click.option(
    "--proxy-port", default=8787, type=click.IntRange(1, 65535), help="CoBWeaverClaw proxy port"
)
...
@click.option(
    ...
    multiple=True,
    help="OpenClaw provider id to route through CoBWeaverClaw (repeatable; default: openai-codex)",
)
@click.option(
    "--python-path",
    default=None,
...
@click.option(
    "--no-auto-start",
    is_flag=True,
    help="Disable plugin auto-start of local cobweaverclaw_compress proxy",
)
@click.option(
    "--no-restart",
    is_flag=True,
...
def openclaw(
    ...
    """Install and configure CoBWeaverClaw OpenClaw plugin in one command.
    ...
    \b
    Example:
      cobweaverclaw_compress wrap openclaw
      cobweaverclaw_compress wrap openclaw --plugin-path C:\\git\\cobweaverclaw_compress\\plugins\\openclaw
    """
    if prepare_only:
    ...
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║           COBWEAVERCLAW WRAP: OPENCLAW             ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    existing_entry = _read_openclaw_config_value(openclaw_bin, "plugins.entries.cobweaverclaw_compress")
    entry = _build_openclaw_plugin_entry(
        existing_entry=existing_entry,
        proxy_port=proxy_port,
        startup_timeout_ms=startup_timeout_ms,
    ...
    _set_openclaw_context_engine_slot(openclaw_bin, "cobweaverclaw_compress")
    _run_checked(
    ...
    inspect_result = _run_checked(
        [openclaw_bin, "plugins", "inspect", "cobweaverclaw_compress"],
        action="openclaw plugins inspect cobweaverclaw_compress",
    )
    if verbose and inspect_result.stdout.strip():
        click.echo(inspect_result.stdout.strip())

    click.echo()
    click.echo("✓ OpenClaw is configured to use CoBWeaverClaw context compression.")
    click.echo("  Plugin: cobweaverclaw_compress")
    click.echo("  Slot:   plugins.slots.contextEngine = cobweaverclaw_compress")
    click.echo()
...
@click.option("--no-mcp", is_flag=True, help="Skip cobweaverclaw_compress MCP server registration")
...
def opencode(
    ...
    """Launch OpenCode through CoBWeaverClaw proxy.

    \b
    Sets OPENCODE_CONFIG_CONTENT to route all OpenCode API calls through
    CoBWeaverClaw. Configures a cobweaverclaw_compress provider via @ai-sdk/openai-compatible.
    Also sets OPENAI_BASE_URL and ANTHROPIC_BASE_URL as fallbacks.

    \b
    Examples:
        cobweaverclaw_compress wrap opencode                         # Start proxy + context tool + opencode
        cobweaverclaw_compress wrap opencode -- "fix the bug"        # Pass prompt to opencode
        cobweaverclaw_compress wrap opencode --no-context-tool       # Skip CLI context-tool setup
        cobweaverclaw_compress wrap opencode --no-mcp                # Skip MCP retrieve tool registration
        cobweaverclaw_compress wrap opencode --no-serena             # Skip Serena MCP registration
        cobweaverclaw_compress wrap opencode --port 9999             # Custom proxy port
        cobweaverclaw_compress wrap opencode --backend anyllm --anyllm-provider groq
    """
    # Snapshot OpenCode config.json BEFORE any wrap-time mutation so
    # `cobweaverclaw_compress unwrap opencode` can restore the user's pre-wrap state.
    ...
    # Register cobweaverclaw_compress MCP server in OpenCode config so OpenCode can
    # call cobweaverclaw_compress_retrieve on compression markers from the proxy.
    if not no_mcp:
        from cobweaverclaw_compress.mcp_registry import OpencodeRegistrar

        _setup_cobweaverclaw_compress_mcp(OpencodeRegistrar(), port, verbose=verbose, force=True)
    elif verbose:
        click.echo("  Skipping MCP retrieve tool (--no-mcp)")
    ...
    # Inject CoBWeaverClaw provider into OpenCode config so traffic routes through proxy.
    inject_opencode_provider_config(port)
    if memory:
        mem_dir = Path.cwd() / ".cobweaverclaw_compress"
...
def unwrap_opencode(port: int, no_stop_proxy: bool) -> None:
    """Undo ``cobweaverclaw_compress wrap opencode`` edits to the active OpenCode config file.

    Behaviour:

    * If a pre-wrap backup (``opencode.json.cobweaverclaw_compress-backup``) exists, the
      original file is restored byte-for-byte and the backup is removed.
    * Otherwise, if the config file still contains the CoBWeaverClaw-managed
      block, that block is stripped out and the rest of the file is
      preserved.
    * If the config only ever contained CoBWeaverClaw-written content, the file
      is removed entirely so OpenCode falls back to its defaults.
    * If neither a backup nor a CoBWeaverClaw block is present, this is a safe
      no-op.
    """
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║         COBWEAVERCLAW UNWRAP: OPENCODE             ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if backup_file.exists():
        ...
        if _PROVIDER_MARKER_START in content or _MCP_MARKER_START in content:
            cleaned = strip_opencode_cobweaverclaw_compress_blocks(content)
            if cleaned.strip():
                _write_text(config_file, cleaned + "\n")
                click.echo(f"  Removed CoBWeaverClaw block from {config_file}; other content preserved.")
                status = "cleaned"
            else:
                config_file.unlink()
                click.echo(f"  Removed {config_file} (contained only CoBWeaverClaw-written config).")
                status = "removed"
        else:
            click.echo(f"  Nothing to undo: {config_file} has no CoBWeaverClaw wrap markers.")
            status = "noop"
    else:
        click.echo(f"  Nothing to undo: {config_file} does not exist.")
        status = "noop"

    # Remove Serena MCP if it was installed by CoBWeaverClaw.
    # Also remove the cobweaverclaw_compress MCP server itself.
    from cobweaverclaw_compress.mcp_registry import OpencodeRegistrar

    opencode_registrar = OpencodeRegistrar()
    if opencode_registrar.detect():
        if opencode_registrar.unregister_server("cobweaverclaw_compress"):
            click.echo("  Removed CoBWeaverClaw MCP server from OpenCode.")
        serena_status = _remove_cobweaverclaw_compress_installed_serena_mcp(opencode_registrar)
        if serena_status == "removed":
            click.echo("  Removed CoBWeaverClaw-installed Serena MCP server from OpenCode.")
        elif serena_status == "failed":
            click.echo("  Serena MCP server matched CoBWeaverClaw ledger but could not be removed.")

    click.echo()
    click.echo("✓ OpenCode is no longer routed through the CoBWeaverClaw proxy.")
...
def unwrap_openclaw(
    ...
    """Disable the CoBWeaverClaw OpenClaw plugin and restore the legacy engine slot."""
    if prepare_only:
        click.echo(
            json.dumps(
    ...
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║          COBWEAVERCLAW UNWRAP: OPENCLAW            ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    click.echo("  Disabling CoBWeaverClaw plugin and removing engine mapping...")
    ...
    click.echo()
    click.echo("✓ OpenClaw CoBWeaverClaw wrap removed.")
    click.echo("  Plugin: cobweaverclaw_compress (installed, disabled)")
...
def unwrap_codex(port: int, no_stop_proxy: bool) -> None:
    """Undo ``cobweaverclaw_compress wrap codex`` edits to the active Codex config file.

    Behaviour:

    * If a pre-wrap backup (``config.toml.cobweaverclaw_compress-backup``) exists, the
      original file is restored byte-for-byte and the backup is removed.
    * Otherwise, if the config file still contains the CoBWeaverClaw-managed
      block, that block is stripped out and the rest of the file is
      preserved.
    ...
    * If neither a backup nor a CoBWeaverClaw block is present, this is a safe
      ...
      it). When ``CODEX_HOME`` is unset, print a warning hint because CoBWeaverClaw
      may be looking at the default config while Codex was wrapped with a
      custom home.
    """
    click.echo()
    click.echo("  ╔═══════════════════════════════════════════════╗")
    click.echo("  ║           COBWEAVERCLAW UNWRAP: CODEX              ║")
    click.echo("  ╚═══════════════════════════════════════════════╝")
    click.echo()
    ...
    if codex_registrar.detect():
        tokensave_status = _remove_cobweaverclaw_compress_installed_tokensave_mcp(codex_registrar)
        if tokensave_status == "removed":
            click.echo("  Removed CoBWeaverClaw-installed tokensave MCP server from Codex.")
        elif tokensave_status == "failed":
            click.echo("  tokensave MCP server matched CoBWeaverClaw ledger but could not be removed.")

        serena_status = _remove_cobweaverclaw_compress_installed_serena_mcp(codex_registrar)
        if serena_status == "removed":
            click.echo("  Removed CoBWeaverClaw-installed Serena MCP server from Codex.")
        elif serena_status == "failed":
            click.echo("  Serena MCP server matched CoBWeaverClaw ledger but could not be removed.")
    ...
    # `codex` launch keeps following CoBWeaverClaw's "prefix shell commands with rtk"
    ...
    if _remove_rtk_instructions(_codex_home_dir() / "AGENTS.md"):
        click.echo("  Removed CoBWeaverClaw rtk instructions from Codex AGENTS.md.")

    if status in {"restored", "cleaned", "removed"}:
        # Hand the threads back to the native-provider menu so the full history
        # stays visible once Codex no longer routes through CoBWeaverClaw. Best-effort.
        retag_to_native(_codex_home_dir())

    click.echo()
    click.echo("✓ Codex is no longer routed through the CoBWeaverClaw proxy.")
    if not no_stop_proxy and status != "noop":
        _echo_unwrap_proxy_stop_status(_stop_local_proxy_for_unwrap(port), port)
    click.echo()
