...
Hand-mirrored port of `cobweaverclaw_compress_core::compression_policy::CompressionPolicy`
...
See `crates/cobweaverclaw_compress-core/src/compression_policy.rs` for the canonical
...
from cobweaverclaw_compress.proxy.auth_mode import AuthMode

# ── F2.2 per-mode default values (CONSERVATIVE pending bake telemetry) ──
# Mirrors the Rust ``pub(crate) const`` block in
# ``crates/cobweaverclaw_compress-core/src/compression_policy.rs``. Centralised here so
...
def policy_default_payg() -> CompressionPolicy:
    ...
    ``COBWEAVERCLAW_PROXY_AUTH_MODE_POLICY_ENFORCEMENT`` flag is disabled
...
_ENFORCEMENT_ENV = "COBWEAVERCLAW_PROXY_AUTH_MODE_POLICY_ENFORCEMENT"
...
def is_enforcement_enabled() -> bool:
    ...
    # (`cobweaverclaw_compress/telemetry/beacon.py::_OFF_VALUES`) so operators don't
