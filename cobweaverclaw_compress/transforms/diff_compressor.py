...
diff compression now goes through `cobweaverclaw_compress._core.DiffCompressor` (built
from `crates/cobweaverclaw_compress-py`). The byte-equality of the two implementations
was verified against 27 recorded fixtures before the Python source was
removed; the Rust crate has its own test coverage in `crates/cobweaverclaw_compress-core/`.
...
class DiffCompressor:
    """Rust-backed `DiffCompressor` (via PyO3 / `cobweaverclaw_compress._core`).
