...
Requires the [ml] extra: pip install cobweaverclaw_compress-ai[ml]

Usage:
    >>> from cobweaverclaw_compress.transforms.kompress_compressor import KompressCompressor
...
# Disable with COBWEAVERCLAW_KOMPRESS_MUST_KEEP=0.
...
_KOMPRESS_MUST_KEEP_ENV = "COBWEAVERCLAW_KOMPRESS_MUST_KEEP"
KOMPRESS_BACKEND_ENV = "COBWEAVERCLAW_KOMPRESS_BACKEND"
KOMPRESS_ONNX_FILENAME_ENV = "COBWEAVERCLAW_KOMPRESS_ONNX_FILENAME"
...
#
# An operator can pin an exact file via COBWEAVERCLAW_KOMPRESS_ONNX_FILENAME.
_DEFAULT_ONNX_FILENAMES = (
    "onnx/kompress-int8-wo.onnx",
    "onnx/kompress-fp32.onnx",
    "onnx/kompress-int8.onnx",
)
KOMPRESS_ONNX_INTRA_THREADS_ENV = "COBWEAVERCLAW_KOMPRESS_ONNX_INTRA_THREADS"
KOMPRESS_ONNX_INTER_THREADS_ENV = "COBWEAVERCLAW_KOMPRESS_ONNX_INTER_THREADS"
KOMPRESS_COREML_CACHE_DIR_ENV = "COBWEAVERCLAW_KOMPRESS_COREML_CACHE_DIR"
KOMPRESS_MAX_CONCURRENT_ENV = "COBWEAVERCLAW_KOMPRESS_MAX_CONCURRENT"
KOMPRESS_EXECUTION_SEMAPHORE_WAIT_MS_ENV = "COBWEAVERCLAW_KOMPRESS_EXECUTION_TIMEOUT_MS"
KOMPRESS_EXECUTION_SEMAPHORE_WAIT_MS_DEFAULT = 25
KOMPRESS_BATCH_SIZE_ENV = "COBWEAVERCLAW_KOMPRESS_BATCH_SIZE"
...
def _get_model_class() -> type:
    """Return the CoBWeaverClawCompressorModel class, importing torch on demand."""
    import torch
    import torch.nn as nn
    from transformers import AutoModel

    class CoBWeaverClawCompressorModel(nn.Module):
    ...
    return CoBWeaverClawCompressorModel
...
def _load_kompress_pytorch(
    ...
    with _kompress_lock:
        ...
        CoBWeaverClawCompressorModel = _get_model_class()
        model = CoBWeaverClawCompressorModel()
...
def _validate_pytorch_device(model: Any, tokenizer: Any, device: str) -> None:
    ...
    encoding = tokenizer(
        ["cobweaverclaw_compress", "kompress", "probe"],
...
def _load_kompress(
    ...
    COBWEAVERCLAW_KOMPRESS_BACKEND:
    ...
    # allowing opt-in MPS/CoreML experiments via COBWEAVERCLAW_KOMPRESS_BACKEND.
    if _is_onnx_available():
        try:
            return _load_kompress_onnx(model_id, use_coreml=False, allow_download=allow_download)
        except KompressModelNotCached:
    ...
    raise ImportError(
        "Kompress requires onnxruntime or torch. Install with: pip install cobweaverclaw_compress-ai[proxy]"
    )
...
# compression timeout (COBWEAVERCLAW_COMPRESSION_TIMEOUT_SECONDS, default 30s). The
...
class KompressCompressor(Transform):
    ...
    def compress(
        ...
        *,
        ...
        # disables. Env COBWEAVERCLAW_COMPRESSION_DEADLINE_MS overrides (default 20s).
        ...
        if deadline_s is None:
            try:
                deadline_s = max(
                    0.0,
                    float(os.environ.get("COBWEAVERCLAW_COMPRESSION_DEADLINE_MS", "20000")) / 1000.0,
                )
            except ValueError:
                deadline_s = 20.0
        ...
        try:
            ...
            for chunk_start in range(0, n_words, max_chunk_words):
                ...
                # reconstruct from context. Disable via COBWEAVERCLAW_KOMPRESS_MUST_KEEP=0.
