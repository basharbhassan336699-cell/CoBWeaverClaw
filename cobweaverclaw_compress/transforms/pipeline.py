"""Transform pipeline orchestration for CoBWeaverClaw SDK."""
...
from ..observability import get_cobweaverclaw_compress_tracer, get_otel_metrics
...
class TransformPipeline:
    ...
    def __init__(
        ...
        Args:
            config: CoBWeaverClaw configuration.
        ...
        self._breaker_threshold = _breaker_env("COBWEAVERCLAW_PIPELINE_BREAKER_THRESHOLD", 3, int)
        self._breaker_cooldown_s = _breaker_env("COBWEAVERCLAW_PIPELINE_BREAKER_COOLDOWN_S", 60.0, float)
    ...
    def _build_default_transforms(self) -> list[Transform]:
        ...
        # OPT-IN: enable via CoBWeaverClawConfig.intercept_tool_results, or for
        # non-config callers (CLI / SDK / tests) the env var
        # COBWEAVERCLAW_INTERCEPT_ENABLED=1. Off by default while this ships — lets
        ...
        if getattr(self.config, "intercept_tool_results", False) or _os.environ.get(
            "COBWEAVERCLAW_INTERCEPT_ENABLED"
        ):
        ...
        **kwargs: Any,
        ...
        if model_limit is None:
            raise ValueError(
                ...
                "configure model_context_limits in CoBWeaverClawClient."
            )
        ...
        tracer = get_cobweaverclaw_compress_tracer()
        span_attributes = {
            "cobweaverclaw_compress.model": model,
            "cobweaverclaw_compress.provider": provider_name or "unknown",
            "cobweaverclaw_compress.message_count": len(messages),
            "cobweaverclaw_compress.tokens.before": tokens_before,
        }
        pipeline_span_context = (
            tracer.start_as_current_span(
                "cobweaverclaw_compress.compression.pipeline",
        ...
        with pipeline_span_context as pipeline_span:
            ...
            for transform in self.transforms:
                ...
                transform_span_context = (
                    tracer.start_as_current_span(
                        "cobweaverclaw_compress.compression.transform",
                        attributes={
                            "cobweaverclaw_compress.model": model,
                            "cobweaverclaw_compress.provider": provider_name or "unknown",
                            "cobweaverclaw_compress.transform": transform.name,
                        },
                    )
                    if record_metrics
                    else nullcontext()
                ...
                with transform_span_context as transform_span:
                    ...
                    if transform_span is not None and transform_span.is_recording():
                        transform_span.set_attribute(
                            "cobweaverclaw_compress.tokens.before", tokens_before_transform
                        )
                        transform_span.set_attribute(
                            "cobweaverclaw_compress.tokens.after", tokens_after_transform
                        )
                        transform_span.set_attribute(
                            "cobweaverclaw_compress.tokens.saved",
                            tokens_before_transform - tokens_after_transform,
                        )
                        transform_span.set_attribute("cobweaverclaw_compress.duration_ms", duration_ms)
                        transform_span.set_attribute(
            ...
            if pipeline_span is not None and pipeline_span.is_recording():
                pipeline_span.set_attribute("cobweaverclaw_compress.tokens.after", tokens_after)
                pipeline_span.set_attribute("cobweaverclaw_compress.tokens.saved", total_saved)
                pipeline_span.set_attribute("cobweaverclaw_compress.duration_ms", pipeline_ms)
                pipeline_span.set_attribute("cobweaverclaw_compress.transforms_applied", len(all_transforms))
                pipeline_span.set_attribute("cobweaverclaw_compress.warnings", len(all_warnings))
