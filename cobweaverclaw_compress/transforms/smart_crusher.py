...
All array compression now goes through `cobweaverclaw_compress._core.SmartCrusher`
(built from `crates/cobweaverclaw_compress-py`). Byte-equality of the two
...
removed; the Rust crate has its own coverage in `crates/cobweaverclaw_compress-core/`
...
The `cobweaverclaw_compress._core` extension is a hard import: there is no Python
...
# `crates/cobweaverclaw_compress-core/.../compaction/mod.rs`.
...
class SmartCrusherConfig:
    ...
    # crates/cobweaverclaw_compress-core/src/transforms/smart_crusher/compaction/compactor.rs).
...
class SmartCrusher(Transform):
    """Rust-backed `SmartCrusher` (via PyO3 / `cobweaverclaw_compress._core`).
    ...
    def __init__(
        ...
        from cobweaverclaw_compress._core import (
        ...
        # `cobweaverclaw_compress.config.SmartCrusherConfig`, which also carries it.
        self._lossless_only = (
            bool(getattr(cfg, "lossless_only", False))
            if lossless_only is None
            else bool(lossless_only)
        )
        # `observer`: see `cobweaverclaw_compress.transforms.observability`. The
        ...
        # back (`cobweaverclaw_compress.proxy.server` does). Both `enabled=False` and
        ...
        self._rust_cfg_kwargs = {
            ...
            # `cobweaverclaw_compress.config.SmartCrusherConfig` (MCP server, SDK) or a
        ...
        # COBWEAVERCLAW_COMPACTION_FORMAT env var when the kwarg is None.
        # Ignored when with_compaction=False.
        resolved_format = compaction_format or os.environ.get(
            "COBWEAVERCLAW_COMPACTION_FORMAT", "csv-schema"
        )
    ...
    def _mirror_single_hash_to_python_store(
        ...
        # the Rust store's `DEFAULT_TTL` (see crates/cobweaverclaw_compress-core/src/
    ...
    def _notify_observer(self, original_tokens: int, compressed_tokens: int) -> None:
        ...
        `CompressionObserver` (see `cobweaverclaw_compress.transforms.observability`).
        ...
        **kwargs: Any,
        ...
        for msg_idx, msg in enumerate(result_messages):
            ...
            if msg.get("role") == "tool":
                # #1077: never re-compress cobweaverclaw_compress_retrieve results — they ARE
            ...
            if isinstance(content, list):
                for i, block in enumerate(content):
                    if not isinstance(block, dict) or block.get("type") != "tool_result":
                        continue
                    # #1077: skip cobweaverclaw_compress_retrieve results — compressing them
