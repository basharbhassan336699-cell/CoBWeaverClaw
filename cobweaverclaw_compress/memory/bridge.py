"""Memory Bridge: bidirectional bridge between markdown files and CoBWeaverClaw memory.

Imports markdown memory files (Claude Code MEMORY.md, ChatGPT facts, etc.)
into CoBWeaverClaw's semantic memory system, and exports CoBWeaverClaw memories back
...
Usage:
    ...
    from cobweaverclaw_compress.memory.bridge_config import BridgeConfig
    ...
    # Import markdown -> CoBWeaverClaw
    stats = await bridge.import_from_markdown()

    # Export CoBWeaverClaw -> markdown
...
from cobweaverclaw_compress.memory.bridge_parsers import (
    ParsedFile,
    ParsedSection,
...
if TYPE_CHECKING:
    ...
    from cobweaverclaw_compress.memory.models import Memory
...
class MemoryBridge:
    """Bidirectional bridge between markdown memory files and CoBWeaverClaw's
    semantic memory system.

    Supports import (md -> CoBWeaverClaw), export (CoBWeaverClaw -> md), and
    ...
    async def import_from_markdown(
        ...
        """Import markdown memory files into CoBWeaverClaw's vector store.
    ...
    async def _import_section(
        ...
        """Import a single parsed section into CoBWeaverClaw.
    ...
    async def export_to_markdown(
        ...
        """Export CoBWeaverClaw memories to a markdown file.
    ...
    async def sync(
        ...
        """Bidirectional sync between markdown files and CoBWeaverClaw.

        1. Import new/changed sections from md files.
        2. Export new organic CoBWeaverClaw memories back to md files.
        """
        paths = paths or self._config.md_paths
        user_id = user_id or self._config.user_id
        stats = SyncStats()
