"""Mem0 backend adapter for CoBWeaverClaw's hierarchical memory system.
...
from cobweaverclaw_compress.memory.ports import MemoryFilter, VectorFilter, VectorSearchResult
...
class Mem0Config:
    ...
    # Qdrant settings (defaults resolve from COBWEAVERCLAW_QDRANT_* env vars)
    ...
    # Collection settings
    collection_name: str = "cobweaverclaw_compress_memories"


class Mem0Backend:
    """
    Mem0 backend implementation for CoBWeaverClaw memory system.
    ...
    This adapter maps Mem0's API to CoBWeaverClaw's MemoryBackend interface:
    ...
    async def _ensure_client(self) -> Any:
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "mem0 package not installed. Install with: pip install 'cobweaverclaw_compress-ai[memory-stack]'"
                ) from None
    ...
    def _build_mem0_metadata(self, memory: Memory) -> dict[str, Any]:
        ...
        """
        metadata = {
            "cobweaverclaw_compress_id": memory.id,
    ...
    def _mem0_result_to_memory(self, result: dict[str, Any]) -> Memory:
        ...
        memory_id = metadata.get("cobweaverclaw_compress_id", result.get("id", str(uuid.uuid4())))
