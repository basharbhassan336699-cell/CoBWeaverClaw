...
from cobweaverclaw_compress.memory.sync_adapters.claude_code import encode_claude_project_path
from cobweaverclaw_compress.memory.writers.base import AgentWriter, MemoryEntry
...
class ClaudeCodeMemoryWriter(AgentWriter):
    ...
    def format_memories(self, memories: list[MemoryEntry]) -> str:
        ...
        lines = [
            "## CoBWeaverClaw Learned Context",
            "*Auto-maintained by CoBWeaverClaw — do not edit manually*",
            "",
        ]
    ...
    def export_topics(
        ...
        for category, entries in grouped.items():
            ...
            filename = f"cobweaverclaw_compress_{category}.md"
            lines = [
                "---",
                f"name: cobweaverclaw_compress-{category}",
                f"description: CoBWeaverClaw-learned {category.replace('_', ' ')} patterns",
                "type: reference",
                "---",
                "",
