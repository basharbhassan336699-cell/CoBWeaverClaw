"""Embedder implementations for CoBWeaverClaw Memory.
...
from cobweaverclaw_compress.onnx_runtime import create_cpu_session_options, hf_hub_download_local_first
...
# ``workers (COBWEAVERCLAW_EMBED_CONCURRENCY) x threads-per-encode
# (COBWEAVERCLAW_EMBED_NUM_THREADS)``. Applies to the CPU device only (GPU/MPS do
# their compute off-CPU).
_EMBED_THREADS_ENV = "COBWEAVERCLAW_EMBED_NUM_THREADS"
_DEFAULT_EMBED_THREADS = 1
_EMBED_CONCURRENCY_ENV = "COBWEAVERCLAW_EMBED_CONCURRENCY"
...
def _resolve_embed_thread_cap() -> int:
    """Resolve the per-encode CPU thread cap (``COBWEAVERCLAW_EMBED_NUM_THREADS``)."""
    return _resolve_positive_int_env(_EMBED_THREADS_ENV, _DEFAULT_EMBED_THREADS)


def _resolve_embed_concurrency() -> int:
    """Resolve the max concurrent CPU encodes (``COBWEAVERCLAW_EMBED_CONCURRENCY``).
