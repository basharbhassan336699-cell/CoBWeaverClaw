...
* ``USER``: one DB per ``x-cobweaverclaw_compress-user-id`` (no project axis).
...
class ProjectResolver:
    ...
    def resolve(self, ctx: RequestContext) -> tuple[str, str] | None:
        ...
        explicit = self._first_nonempty_header(ctx.headers, "x-cobweaverclaw_compress-project-id")
        if explicit:
        ...
        explicit_cwd = self._first_nonempty_header(ctx.headers, "x-cobweaverclaw_compress-cwd")
        if explicit_cwd:
        ...
        if ident is None:
            ...
            if fallback == "empty":
                ...
                logger.warning(
                    ...
                    "hint='set x-cobweaverclaw_compress-project-id or x-cobweaverclaw_compress-cwd header, "
