...
if TYPE_CHECKING:
    from cobweaverclaw_compress.memory.ports import Embedder, MemoryCache, MemoryStore, TextIndex, VectorIndex


# Extension groups for memory backends registered via setuptools entry points.
_MEMORY_STORE_GROUP = "cobweaverclaw_compress.memory_store"
_MEMORY_VECTOR_GROUP = "cobweaverclaw_compress.memory_vector"
_MEMORY_TEXT_GROUP = "cobweaverclaw_compress.memory_text"
...
def _load_external_backend(
    ...
    `cobweaverclaw_compress.cache.compression_store._create_default_ccr_backend`.
    """
    if not name:
        raise ValueError(
...
def _create_store(config: MemoryConfig) -> MemoryStore:
    ...
    if config.store_backend == StoreBackend.SQLITE:
        from cobweaverclaw_compress.memory.adapters.sqlite import SQLiteMemoryStore
...
def _create_embedder(config: MemoryConfig) -> Embedder:
    ...
    with _EMBEDDER_CACHE_LOCK:
        ...
        if config.embedder_backend == EmbedderBackend.LOCAL:
            from cobweaverclaw_compress.memory.adapters.embedders import LocalEmbedder

            embedder: Embedder = LocalEmbedder(model_name=config.embedder_model)

        elif config.embedder_backend == EmbedderBackend.ONNX:
            from cobweaverclaw_compress.memory.adapters.embedders import OnnxLocalEmbedder

            embedder = OnnxLocalEmbedder()

        elif config.embedder_backend == EmbedderBackend.OPENAI:
            from cobweaverclaw_compress.memory.adapters.embedders import OpenAIEmbedder
            ...
            from cobweaverclaw_compress.memory.adapters.embedders import OllamaEmbedder
...
def _create_vector_index(config: MemoryConfig) -> VectorIndex:
    ...
    if backend == VectorBackend.AUTO:
        from cobweaverclaw_compress.memory.adapters import HNSW_AVAILABLE, SQLITE_VEC_AVAILABLE
        ...
        if SQLITE_VEC_AVAILABLE:
            ...
            raise ValueError(
                ...
                "Or install the full proxy bundle: pip install cobweaverclaw_compress-ai[proxy]"
            )

    if backend == VectorBackend.SQLITE_VEC:
        from cobweaverclaw_compress.memory.adapters import SQLITE_VEC_AVAILABLE
        ...
        from cobweaverclaw_compress.memory.adapters.sqlite_vector import SQLiteVectorIndex
    ...
    if backend == VectorBackend.HNSW:
        from cobweaverclaw_compress.memory.adapters import HNSW_AVAILABLE
        ...
        from cobweaverclaw_compress.memory.adapters.hnsw import HNSWVectorIndex
...
def _create_text_index(config: MemoryConfig) -> TextIndex:
    ...
    if config.text_backend == TextBackend.FTS5:
        from cobweaverclaw_compress.memory.adapters.fts5 import FTS5TextIndex
...
def _create_cache(config: MemoryConfig) -> MemoryCache:
    ...
    """
    from cobweaverclaw_compress.memory.adapters.cache import LRUMemoryCache
