...
This module provides the easiest way to use CoBWeaverClaw's memory system.
...
if TYPE_CHECKING:
    from cobweaverclaw_compress.memory.ports import MemorySearchResult
...
class Memory:
    ...
    Args:
        ...
        db_path: Path for local database (only for "local" backend).
            Defaults to ~/.cobweaverclaw_compress/memory.db
        qdrant_url: Full Qdrant URL (only for "qdrant-neo4j" backend). When set,
            ...
            Defaults to the ``COBWEAVERCLAW_QDRANT_URL`` env var if unset.
        qdrant_host: Qdrant host (only for "qdrant-neo4j" backend). Defaults
            to the ``COBWEAVERCLAW_QDRANT_HOST`` env var or ``localhost``.
        qdrant_port: Qdrant port (only for "qdrant-neo4j" backend). Defaults
            to the ``COBWEAVERCLAW_QDRANT_PORT`` env var or ``6333``.
        qdrant_api_key: API key for hosted Qdrant. Defaults to
            ``COBWEAVERCLAW_QDRANT_API_KEY`` if unset.
    ...
    Examples:
        ...
        # Hosted Qdrant via URL + API key (or set COBWEAVERCLAW_QDRANT_URL /
        # COBWEAVERCLAW_QDRANT_API_KEY in the environment)
        memory = Memory(
    ...
    def __init__(
        ...
        if db_path is None:
            # Default: workspace memory.db (respects COBWEAVERCLAW_WORKSPACE_DIR)
        ...
        # ``None`` sentinels fall back to COBWEAVERCLAW_QDRANT_* env vars so that
