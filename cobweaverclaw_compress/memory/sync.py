...
Provides bidirectional sync between cobweaverclaw_compress's memory DB and any
...
Usage:
    from cobweaverclaw_compress.memory.sync import sync, SyncResult
    from cobweaverclaw_compress.memory.sync_adapters.claude_code import ClaudeCodeAdapter
...
logger = logging.getLogger("cobweaverclaw_compress.memory.sync")

# State file for fast no-op detection (workspace bucket, respects
# COBWEAVERCLAW_WORKSPACE_DIR). Resolved at import time, matching prior behavior.
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        ...
        Args:
            memories: List of dicts with keys: content, category, importance,
                      cobweaverclaw_compress_id, source_agent, content_hash.
...
async def sync(
    ...
    """Bidirectional sync between cobweaverclaw_compress DB and an agent's memory.
        ...
        to_export.append(
            {
                ...
                "cobweaverclaw_compress_id": mem.id,
...
# ---------------------------------------------------------------------------
# CLI entry point: python -m cobweaverclaw_compress.memory.sync --db ... --user ... --agent ...
# ---------------------------------------------------------------------------
...
def _build_sync_backend(db_path: str) -> Any:
    ...
    Match the proxy MCP server (see ``cobweaverclaw_compress/memory/mcp_server.py``): use the
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="CoBWeaverClaw memory sync")
    ...
    async def _run() -> None:
        ...
        if args.agent == "claude":
            ...
            from cobweaverclaw_compress.memory.sync_adapters.codex_agent import CodexAdapter
