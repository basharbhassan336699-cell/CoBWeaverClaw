"""CoBWeaverClaw Memory MCP Server.

A stdio MCP server that exposes cobweaverclaw_compress's memory backend as tools
...
Usage:
    ...
    python -m cobweaverclaw_compress.memory.mcp_server --db /path/to/.cobweaverclaw_compress/memory.db

    # Registered in Codex config.toml (done by `cobweaverclaw_compress wrap codex --memory`):
    [mcp_servers.cobweaverclaw_compress_memory]
    command = "python"
    args = ["-m", "cobweaverclaw_compress.memory.mcp_server", "--user", "alice"]
    # When --db is omitted, the server resolves .cobweaverclaw_compress/memory.db from cwd.
"""
...
logger = logging.getLogger("cobweaverclaw_compress.memory.mcp")
...
def create_memory_server(db_path: str, user_id: str = "default") -> Server:
    """Create an MCP server backed by cobweaverclaw_compress's local memory."""

    server = Server("cobweaverclaw_compress-memory")
...
def _memory_mcp_startup_context(
    ...
    active_project_db = (cwd / ".cobweaverclaw_compress" / "memory.db").resolve(strict=False)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CoBWeaverClaw Memory MCP Server")
    parser.add_argument(
        "--db",
        default=str(Path.cwd() / ".cobweaverclaw_compress" / "memory.db"),
