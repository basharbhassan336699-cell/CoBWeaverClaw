...
Syncs memories to/from a cobweaverclaw_compress-managed section in AGENTS.md.
...
Format in AGENTS.md:
    <!-- cobweaverclaw_compress:memory:start -->
    ## CoBWeaverClaw Shared Memory
    - fact 1
    - fact 2
    <!-- cobweaverclaw_compress:memory:end -->
"""
...
from cobweaverclaw_compress.memory.sync import AgentMemory, AgentMemoryAdapter

_MARKER_START = "<!-- cobweaverclaw_compress:memory:start -->"
_MARKER_END = "<!-- cobweaverclaw_compress:memory:end -->"
...
class CodexAdapter(AgentMemoryAdapter):
    ...
    async def read_memories(self) -> list[AgentMemory]:
        """Read memories from the cobweaverclaw_compress section of AGENTS.md."""
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        """Write memories into the cobweaverclaw_compress section of AGENTS.md."""
        if not memories:
            return 0

        # Build section content
        lines = ["## CoBWeaverClaw Shared Memory", ""]
        for mem in memories:
