...
``rtk`` — CoBWeaverClaw fetches the prebuilt release asset for the current
...
multi-minute compile is the wrong thing to trigger from ``cobweaverclaw_compress wrap``.
...
Supply-chain integrity:
    Because ``cobweaverclaw_compress wrap`` downloads and then *executes* this binary by
    ...
    code. When ``COBWEAVERCLAW_TOKENSAVE_VERSION`` overrides the pinned tag there
    is no pinned digest, so the download is refused unless the operator
    explicitly opts out of verification via
    ``COBWEAVERCLAW_TOKENSAVE_ALLOW_UNVERIFIED=1``.

Env vars:
    COBWEAVERCLAW_BINARIES_OFFLINE        if set, never reach the network (returns
                                     the already-installed binary or ``None``).
    COBWEAVERCLAW_TOKENSAVE_VERSION       override the pinned release tag.
    COBWEAVERCLAW_TOKENSAVE_ALLOW_UNVERIFIED  permit installing an asset that has
...
#: Pinned release. Override with COBWEAVERCLAW_TOKENSAVE_VERSION.
...
def _pinned_version() -> str:
    return os.environ.get("COBWEAVERCLAW_TOKENSAVE_VERSION", "").strip() or TOKENSAVE_VERSION
...
def _verify_asset_digest(filename: str, data: bytes) -> None:
    ...
    ``COBWEAVERCLAW_TOKENSAVE_ALLOW_UNVERIFIED``.
    """
    expected = TOKENSAVE_ASSET_DIGESTS.get(filename)
    if expected is None:
        if os.environ.get("COBWEAVERCLAW_TOKENSAVE_ALLOW_UNVERIFIED"):
            logger.warning(
                "tokensave asset %s has no pinned digest; installing unverified "
                "(COBWEAVERCLAW_TOKENSAVE_ALLOW_UNVERIFIED is set)",
                filename,
            )
            return
        raise RuntimeError(
            f"no pinned SHA-256 digest for tokensave asset {filename!r}; refusing to "
            "install unverified. Set COBWEAVERCLAW_TOKENSAVE_ALLOW_UNVERIFIED=1 to override."
        )
...
def download_tokensave(version: str | None = None) -> Path:
    ...
    try:
        from cobweaverclaw_compress._subprocess import run as _run
...
def ensure_tokensave(version: str | None = None) -> Path | None:
    ...
    if os.environ.get("COBWEAVERCLAW_BINARIES_OFFLINE"):
        logger.info("tokensave not installed and COBWEAVERCLAW_BINARIES_OFFLINE set — skipping download")
        return None
