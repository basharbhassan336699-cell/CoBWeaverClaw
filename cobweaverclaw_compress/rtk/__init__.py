"""rtk (Rust Token Killer) integration for CoBWeaverClaw.

rtk compresses CLI output (test results, git diffs, log dumps) before it
enters the LLM context window. CoBWeaverClaw downloads and manages the rtk binary.
"""
...
def _managed_rtk_candidates() -> list[Path]:
    """Return known CoBWeaverClaw-managed rtk binary paths."""
...
def get_rtk_path() -> Path | None:
    """Get path to rtk binary — check PATH first, then ~/.cobweaverclaw_compress/bin/."""
    # Check if rtk is already in PATH (e.g., installed via brew)
    system_rtk = shutil.which("rtk")
    if system_rtk:
        return Path(system_rtk)

    # Check CoBWeaverClaw-managed install
