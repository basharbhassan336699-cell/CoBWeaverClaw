...
This module provides tools to verify that CoBWeaverClaw preserves the semantic
meaning of prompts by comparing prompts before and after processing.

This is CRITICAL for proving that CoBWeaverClaw doesn't change the meaning of prompts
when they go through the proxy.

Usage:
    from cobweaverclaw_compress.evals.prompt_comparison import compare_prompts, PromptComparer

    # Simple comparison
    result = compare_prompts(
        original_prompt="What is the capital of France?",
        cobweaverclaw_compress_modified_prompt="What is the capital of France?",
    )
...
class PromptComparisonResult:
    ...
    Attributes:
        original: The original prompt before CoBWeaverClaw processing.
        cobweaverclaw_compress_modified: The prompt after CoBWeaverClaw processing.
    ...
    original: str
    cobweaverclaw_compress_modified: str
    are_equivalent: bool
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cobweaverclaw_compress_modified": self.cobweaverclaw_compress_modified,
    ...
    def from_dict(cls, data: dict[str, Any]) -> PromptComparisonResult:
        ...
        return cls(
            ...
            cobweaverclaw_compress_modified=data["cobweaverclaw_compress_modified"],
...
def compare_prompts(
    original_prompt: str,
    cobweaverclaw_compress_modified_prompt: str,
    judge_model: str = "gpt-4o",
    ...
    Args:
        original_prompt: The original prompt before CoBWeaverClaw processing.
        cobweaverclaw_compress_modified_prompt: The prompt after CoBWeaverClaw processing.
    ...
    Example:
        result = compare_prompts(
            ...
            cobweaverclaw_compress_modified_prompt="Explain quantum computing in simple terms.",
        )
    ...
    judge_prompt = SEMANTIC_EQUIVALENCE_JUDGE_PROMPT.format(
        ...
        modified_prompt=cobweaverclaw_compress_modified_prompt,
    )
    ...
    return PromptComparisonResult(
        ...
        cobweaverclaw_compress_modified=cobweaverclaw_compress_modified_prompt,
...
class PromptComparer:
    ...
    def compare(
        ...
        result = compare_prompts(
            original_prompt=original_prompt,
            cobweaverclaw_compress_modified_prompt=cobweaverclaw_compress_modified_prompt,
            judge_model=self.judge_model,
    ...
    def compare_and_log(
        ...
        Returns:
            PromptComparisonResult with the comparison outcome.
        """
        result = self.compare(original_prompt, cobweaverclaw_compress_modified_prompt, metadata)
...
def compare_messages(
    ...
    Args:
        original_messages: Original message list before CoBWeaverClaw.
        modified_messages: Modified message list after CoBWeaverClaw.
        judge_model: The OpenAI model to use as judge.
    ...
    result = compare_prompts(
        ...
        cobweaverclaw_compress_modified_prompt=modified_str,
        judge_model=judge_model,
        api_key=api_key,
        metadata={
...
def batch_compare_prompts(
    ...
    def compare_pair(
        ...
        result = compare_prompts(
            ...
            cobweaverclaw_compress_modified_prompt=modified,
            judge_model=judge_model,
            api_key=api_key,
...
def verify_cobweaverclaw_compress_preservation(
    original_messages: list[dict[str, Any]],
    cobweaverclaw_compress_messages: list[dict[str, Any]],
    judge_model: str = "gpt-4o",
    api_key: str | None = None,
    fail_on_difference: bool = False,
) -> PromptComparisonResult:
    """Verify that CoBWeaverClaw preserves the semantic meaning of a request.

    This is the main entry point for verifying CoBWeaverClaw's accuracy preservation.
    It compares the original messages with those modified by CoBWeaverClaw and
    raises an error if differences are found (when fail_on_difference=True).

    Args:
        original_messages: Messages before CoBWeaverClaw processing.
        cobweaverclaw_compress_messages: Messages after CoBWeaverClaw processing.
        judge_model: The model to use for judging.
        api_key: OpenAI API key.
    ...
    Example:
        # Capture messages before and after CoBWeaverClaw
        original = [{"role": "user", "content": "Hello, how are you?"}]
        after_cobweaverclaw_compress = [{"role": "user", "content": "Hello, how are you?"}]

        result = verify_cobweaverclaw_compress_preservation(
            original, after_cobweaverclaw_compress, fail_on_difference=True
        )
    """
    result = compare_messages(
        original_messages=original_messages,
        modified_messages=cobweaverclaw_compress_messages,
        judge_model=judge_model,
        api_key=api_key,
    )
    ...
    if fail_on_difference and not result.are_equivalent:
        raise ValueError(
            f"CoBWeaverClaw modified prompt semantics! "
