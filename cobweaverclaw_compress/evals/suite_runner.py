...
if TYPE_CHECKING:
    from cobweaverclaw_compress.evals.reports.report_card import SuiteResult
...
class BenchmarkSpec:
    ...
    pass_threshold: float = 0.98  # CoBWeaverClaw score >= baseline - (1-threshold)
...
BENCHMARK_SUITE: list[BenchmarkSpec] = [
    ...
    # -----------------------------------------------------------------------
    # Standard benchmarks via lm-eval harness (through CoBWeaverClaw proxy)
    BenchmarkSpec(
        name="GSM8K",
        category="reasoning",
        tier=1,
...
def _start_proxy(port: int) -> subprocess.Popen | None:
    """Start CoBWeaverClaw proxy as a subprocess. Returns process handle."""
    logger.info(f"Starting CoBWeaverClaw proxy on port {port}...")
    try:
        proc = subprocess.Popen(
            [sys.executable, "-m", "cobweaverclaw_compress.proxy.server", "--port", str(port)],
        ...
        for _ in range(30):
            ...
            if _check_proxy(port):
                logger.info(f"CoBWeaverClaw proxy ready on port {port}")
                return proc
...
class SuiteRunner:
    ...
    def __init__(
        ...
        self.cobweaverclaw_compress_port = cobweaverclaw_compress_port
    ...
    def _ensure_proxy(self) -> bool:
        """Ensure the CoBWeaverClaw proxy is running (needed for lm-eval benchmarks)."""
        if _check_proxy(self.cobweaverclaw_compress_port):
            return True
        if self.auto_start_proxy:
            self._proxy_proc = _start_proxy(self.cobweaverclaw_compress_port)
    ...
    def _cleanup_proxy(self) -> None:
        ...
        if self._proxy_proc:
            logger.info("Stopping CoBWeaverClaw proxy...")
    ...
    def _run_lm_eval_benchmark(
        ...
        from cobweaverclaw_compress.evals.comprehensive_benchmark import (
            compare_results,
            run_baseline_benchmark,
            run_cobweaverclaw_compress_benchmark,
        )
        ...
        # Run through CoBWeaverClaw proxy
        print("    Running through CoBWeaverClaw proxy...")
        cobweaverclaw_compress_results = run_cobweaverclaw_compress_benchmark(
            model=spec.model or self.model,
            tasks=tasks,
            limit=limit,
            cobweaverclaw_compress_port=self.cobweaverclaw_compress_port,
        )

        # Compare
        comparisons = compare_results(baseline_results, cobweaverclaw_compress_results)

        # Extract primary result
        baseline_score = None
        cobweaverclaw_compress_score = None
        delta = None
        passed = True
        ...
        if comparisons:
            ...
            cobweaverclaw_compress_score = c.cobweaverclaw_compress_score
            delta = c.delta
            passed = c.accuracy_preserved

        return {
            "baseline_score": baseline_score,
            "cobweaverclaw_compress_score": cobweaverclaw_compress_score,
            "delta": delta,
            "passed": passed,
            "n_samples": limit,
        }
    ...
    def _run_before_after_benchmark(
        ...
        from cobweaverclaw_compress.evals.core import EvalMode
        from cobweaverclaw_compress.evals.datasets import load_dataset_by_name, load_tool_output_samples
        ...
        proxy_url = (
            f"http://localhost:{self.cobweaverclaw_compress_port}" if _check_proxy(self.cobweaverclaw_compress_port) else None
        )
        runner = BeforeAfterRunner(
            llm_config=LLMConfig(
                ...
                model=spec.model or self.model,
                temperature=0.0,
                cobweaverclaw_compress_proxy_url=proxy_url,
            ),
    ...
    def run(self) -> SuiteResult:
        ...
        from cobweaverclaw_compress.evals.cost_tracker import CostTracker
        from cobweaverclaw_compress.evals.reports.report_card import BenchmarkRunResult, SuiteResult
        ...
        if has_lm_eval:
            ...
            if not proxy_available:
                print("WARNING: CoBWeaverClaw proxy not available. Skipping lm-eval benchmarks.")
                print("  Start with: cobweaverclaw_compress proxy --port 8787")
                    ...
                    if spec.runner_type == "lm_eval":
                        ...
                        results.append(
                            BenchmarkRunResult(
                                ...
                                cobweaverclaw_compress_score=raw.get("cobweaverclaw_compress_score"),
                                delta=raw.get("delta"),
    ...
    def _print_summary(self, result: SuiteResult, tracker: Any) -> None:
        ...
        from cobweaverclaw_compress.evals.reports.report_card import generate_markdown
