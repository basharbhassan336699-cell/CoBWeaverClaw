...
This module runs industry-standard benchmarks to prove CoBWeaverClaw preserves accuracy.

Approach:
1. Run benchmarks directly against LLM provider (baseline)
2. Run same benchmarks through CoBWeaverClaw proxy
...
Usage:
    ...
    python -m cobweaverclaw_compress.evals.comprehensive_benchmark --quick

    # Full benchmark
    python -m cobweaverclaw_compress.evals.comprehensive_benchmark --tasks mmlu,hellaswag

    # Compare with baseline
    python -m cobweaverclaw_compress.evals.comprehensive_benchmark --compare
"""
...
class ComparisonResult:
    """Comparison between baseline and CoBWeaverClaw results."""

    task: str
    metric: str
    baseline_score: float
    cobweaverclaw_compress_score: float

    @property
    def delta(self) -> float:
        return self.cobweaverclaw_compress_score - self.baseline_score

    @property
    def accuracy_preserved(self) -> bool:
        """True if CoBWeaverClaw score is within 2% of baseline."""
        return self.cobweaverclaw_compress_score >= self.baseline_score - 0.02

    @property
    def accuracy_improved(self) -> bool:
        """True if CoBWeaverClaw score exceeds baseline."""
        return self.cobweaverclaw_compress_score > self.baseline_score
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "cobweaverclaw_compress": round(self.cobweaverclaw_compress_score, 4),
...
class BenchmarkSuiteResult:
    ...
    cobweaverclaw_compress_results: list[BenchmarkResult] = field(default_factory=list)
    ...
    def avg_delta(self) -> float:
        """Average score delta (positive = CoBWeaverClaw better)."""
        if not self.comparisons:
            return 0.0
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "cobweaverclaw_compress": [r.to_dict() for r in self.cobweaverclaw_compress_results],
    ...
    def print_summary(self) -> None:
        ...
        print("\n" + "=" * 70)
        print("COBWEAVERCLAW COMPREHENSIVE BENCHMARK RESULTS")
        print("=" * 70)
        ...
        if self.comparisons:
            print(f"{'Task':<20} {'Baseline':>10} {'CoBWeaverClaw':>10} {'Delta':>10} {'Status':>10}")
            print("-" * 70)
            for c in self.comparisons:
                status = "PASS" if c.accuracy_preserved else "FAIL"
                delta_str = f"{c.delta:+.4f}"
                print(
                    f"{c.task:<20} {c.baseline_score:>10.4f} {c.cobweaverclaw_compress_score:>10.4f} {delta_str:>10} {status:>10}"
                )
            print("-" * 70)
            ...
            print("CoBWeaverClaw results only (no baseline comparison):")
            print(f"{'Task':<20} {'Score':>10} {'Metric':<15}")
            print("-" * 50)
            for r in self.cobweaverclaw_compress_results:
...
def run_lm_eval(
    ...
    Args:
        ...
        base_url: Base URL for API (for CoBWeaverClaw proxy)
...
def run_cobweaverclaw_compress_benchmark(
    model: str = "gpt-4o-mini",
    tasks: list[str] | None = None,
    limit: int | None = None,
    cobweaverclaw_compress_port: int = 8787,
) -> list[BenchmarkResult]:
    """Run benchmark through CoBWeaverClaw proxy."""
    logger.info(f"Running CoBWeaverClaw benchmark with {model} through proxy...")

    # lm_eval expects base_url to be the full path to chat/completions
    base_url = f"http://localhost:{cobweaverclaw_compress_port}/v1/chat/completions"
...
def compare_results(
    ...
    cobweaverclaw_compress: list[BenchmarkResult],
) -> list[ComparisonResult]:
    """Compare baseline and CoBWeaverClaw results."""
    comparisons = []

    # Match results by task
    baseline_by_task = {r.task: r for r in baseline}
    cobweaverclaw_compress_by_task = {r.task: r for r in cobweaverclaw_compress}

    for task in baseline_by_task:
        if task in cobweaverclaw_compress_by_task:
            b = baseline_by_task[task]
            h = cobweaverclaw_compress_by_task[task]
            comparisons.append(
                ComparisonResult(
                    task=task,
                    metric=b.metric,
                    baseline_score=b.score,
                    cobweaverclaw_compress_score=h.score,
                )
            )
...
def run_comprehensive_benchmark(
    ...
    Args:
        ...
        cobweaverclaw_compress_port: Port where CoBWeaverClaw proxy is running
    ...
    # This ensures a fair comparison - any speedup from CoBWeaverClaw is real, not from warm cache
    if compare_baseline:
    ...
    # Run CoBWeaverClaw benchmark (after baseline warms things up)
    suite.cobweaverclaw_compress_results = run_cobweaverclaw_compress_benchmark(
        model=model,
        tasks=tasks,
        limit=limit,
        cobweaverclaw_compress_port=cobweaverclaw_compress_port,
    )

    # Compare results
    if compare_baseline:
        suite.comparisons = compare_results(suite.baseline_results, suite.cobweaverclaw_compress_results)

    suite.total_duration_seconds = time.time() - start_time
    return suite


def check_cobweaverclaw_compress_proxy(port: int = 8787) -> bool:
    """Check if CoBWeaverClaw proxy is running."""
    import socket
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run comprehensive LLM benchmarks to verify CoBWeaverClaw accuracy"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--compare", "-c", action="store_true", help="Compare with baseline (run without CoBWeaverClaw)"
    )
    parser.add_argument(
        "--port", "-p", type=int, default=8787, help="CoBWeaverClaw proxy port (default: 8787)"
    )
    parser.add_argument("--output", "-o", default=None, help="Output file for results JSON")
    parser.add_argument(
        "--cobweaverclaw_compress-only", action="store_true", help="Only run through CoBWeaverClaw (skip baseline)"
    )
    ...
    if not check_cobweaverclaw_compress_proxy(args.port):
        print(f"ERROR: CoBWeaverClaw proxy not running on port {args.port}")
        print(f"Start it with: cobweaverclaw_compress proxy --port {args.port}")
        sys.exit(1)

    # Run benchmarks
    compare = args.compare and not args.cobweaverclaw_compress_only
    ...
    try:
        result = run_comprehensive_benchmark(
            ...
            cobweaverclaw_compress_port=args.port,
        )
