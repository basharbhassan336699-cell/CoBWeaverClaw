...
baseline vs CoBWeaverClaw accuracy + token savings.
"""
...
class BenchmarkRunResult:
    ...
    # For lm-eval (standard benchmarks): baseline vs cobweaverclaw_compress scores
    baseline_score: float | None = None
    cobweaverclaw_compress_score: float | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.cobweaverclaw_compress_score is not None:
            d["cobweaverclaw_compress_score"] = round(self.cobweaverclaw_compress_score, 4)
...
class SuiteResult:
    ...
    def standard_benchmarks(self) -> list[BenchmarkRunResult]:
        """Benchmarks with baseline vs cobweaverclaw_compress comparison."""
...
def generate_markdown(result: SuiteResult) -> str:
    ...
    lines = [
        "## CoBWeaverClaw Accuracy Report Card",
        "",
    ...
    if standard:
        lines.extend(
            [
                ...
                "",
                "| Benchmark | Category | N | Baseline | CoBWeaverClaw | Delta | Tokens Saved | Status |",
                "|-----------|----------|---|----------|----------|-------|--------------|--------|",
            ]
        )
        for b in standard:
            ...
            cobweaverclaw_compress_str = f"{b.cobweaverclaw_compress_score:.3f}" if b.cobweaverclaw_compress_score is not None else "N/A"
            ...
            lines.append(
                ...
                f"{cobweaverclaw_compress_str} | {delta_str} | {comp_str} | {status} |"
            )
        lines.append("")
...
def generate_html(result: SuiteResult) -> str:
    ...
    for b in standard:
        ...
        standard_rows += f"""        <tr>
            ...
            <td>{baseline_str}</td><td>{cobweaverclaw_compress_str}</td><td>{delta_str}</td>
            <td>{comp_str}</td>
...
<html>
<head>
    <title>CoBWeaverClaw Accuracy Report Card</title>
    <style>
        body {{
...
<body>
    <h1>CoBWeaverClaw Accuracy Report Card</h1>
    <div class="meta">
    ...
    {"<table><tr><th>Benchmark</th><th>Category</th><th>N</th><th>Baseline</th><th>CoBWeaverClaw</th><th>Delta</th><th>Saved</th><th>Status</th></tr>" if standard else ""}
    {standard_rows}
    ...
    <footer>Generated by <strong>CoBWeaverClaw Evaluation Framework</strong> | <code>pip install cobweaverclaw_compress-ai[evals]</code></footer>
</body>
</html>"""
