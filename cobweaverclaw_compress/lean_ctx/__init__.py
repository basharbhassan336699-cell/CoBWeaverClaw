"""lean-ctx integration for CoBWeaverClaw.

lean-ctx configures supported coding agents to route tool output through its
context-filtering layer. CoBWeaverClaw downloads and manages the lean-ctx binary.
"""
...
def _managed_lean_ctx_candidates() -> list[Path]:
    """Return known CoBWeaverClaw-managed lean-ctx binary paths."""
...
def get_lean_ctx_path() -> Path | None:
    """Get path to lean-ctx binary — check PATH first, then ~/.cobweaverclaw_compress/bin/."""
