...
DEFAULT_CCR_TTL_SECONDS = 1800  # session-scale; override via COBWEAVERCLAW_CCR_TTL_SECONDS
CCR_TTL_SECONDS_ENV = "COBWEAVERCLAW_CCR_TTL_SECONDS"
...
CCR_MISS_MESSAGE = (
    ...
    "(default 30 minutes; configurable via COBWEAVERCLAW_CCR_TTL_SECONDS)."
)
...
class CompressionStore:
    ...
    def _log_retrieval_payload(
        ...
        *,
        ...
        event = {
            "event": "cobweaverclaw_compress_retrieve",
            "hash": hash_key,
        ...
        logger.info(
            "event=cobweaverclaw_compress_retrieve %s",
...
_request_ccr_store: ContextVar[CompressionStore | None] = ContextVar(
    "cobweaverclaw_compress_request_ccr_store", default=None
)
...
def _create_default_ccr_backend() -> CompressionStoreBackend | None:
    """Create a CCR backend from env (e.g. COBWEAVERCLAW_CCR_BACKEND=redis).
    ...
    adapters via setuptools entry point 'cobweaverclaw_compress.ccr_backend'.
    Returns None to use InMemoryBackend.
    """
    backend_type = (os.environ.get("COBWEAVERCLAW_CCR_BACKEND") or "").strip().lower()
    ...
    try:
        ...
        all_eps = entry_points(group="cobweaverclaw_compress.ccr_backend")
        ep = next((e for e in all_eps if e.name == backend_type), None)
        if ep is None:
            logger.warning(
                "COBWEAVERCLAW_CCR_BACKEND=%s but no entry point cobweaverclaw_compress.ccr_backend[%s]",
                backend_type,
                backend_type,
            )
            return None
        fn = ep.load()
        kwargs = {
            "url": os.environ.get("COBWEAVERCLAW_REDIS_URL", ""),
            "tenant_prefix": os.environ.get("COBWEAVERCLAW_CCR_TENANT_PREFIX", ""),
        }
...
def get_compression_store(
    ...
    explicitly or created from env (COBWEAVERCLAW_CCR_BACKEND) when building the global.
    ...
    Args:
        ...
        default_ttl: Default TTL (only used on first call for global store).
            When omitted, COBWEAVERCLAW_CCR_TTL_SECONDS overrides the 1800-second default.
