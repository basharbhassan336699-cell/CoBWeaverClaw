...
- ``get_compression_store()`` (the proxy path) defaults to SQLite
  (restart-safe, shared across workers); ``COBWEAVERCLAW_CCR_BACKEND=memory``
...
Usage:
    from cobweaverclaw_compress.cache.backends import SQLiteBackend, CompressionStoreBackend
    from cobweaverclaw_compress.cache.compression_store import CompressionStore, get_compression_store
