...
- **Multi-worker sharing.** The database file (WAL mode) is shared
  across worker processes, so a `cobweaverclaw_compress_retrieve` call served by a
...
Set ``COBWEAVERCLAW_CCR_BACKEND=memory`` to opt back into the in-memory
backend, or ``COBWEAVERCLAW_CCR_SQLITE_PATH`` to relocate the database file
...
def default_db_path() -> Path:
    ...
    env = os.environ.get("COBWEAVERCLAW_CCR_SQLITE_PATH", "").strip()
    if env:
