...
cache_control breakpoints, growing-prefix strategy). If CoBWeaverClaw compresses
...
# cobweaverclaw_compress.cache.anthropic.ANTHROPIC_CACHE_TTL_SECONDS); the others are best-
...
class SessionTrackerStore:
    ...
    Keyed by session ID (from x-cobweaverclaw_compress-session-id header or computed hash).
    ...
    def compute_session_id(
        ...
        Priority:
        1. x-cobweaverclaw_compress-session-id header (explicit)
