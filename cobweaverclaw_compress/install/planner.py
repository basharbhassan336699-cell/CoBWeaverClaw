...
from cobweaverclaw_compress.providers.install_registry import build_install_target_envs
...
def build_manifest(
    *,
    ...
    base_env = {
        "COBWEAVERCLAW_PORT": str(port),
        "COBWEAVERCLAW_HOST": "127.0.0.1",
        "COBWEAVERCLAW_MODE": proxy_mode,
        "COBWEAVERCLAW_BACKEND": backend,
    }
    if anyllm_provider:
        base_env["COBWEAVERCLAW_ANYLLM_PROVIDER"] = anyllm_provider
    if region:
        base_env["COBWEAVERCLAW_REGION"] = region
    ...
    base_env["COBWEAVERCLAW_TELEMETRY"] = "on" if telemetry_enabled else "off"
    if memory_enabled:
        base_env["COBWEAVERCLAW_MEMORY_ENABLED"] = "1"
    ...
    container_name = f"cobweaverclaw_compress-{normalized_profile}"
    return DeploymentManifest(
        ...
        image=image,
        service_name=f"cobweaverclaw_compress-{normalized_profile}",
