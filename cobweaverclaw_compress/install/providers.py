...
from cobweaverclaw_compress.providers.install_registry import (
...
_ENV_MARKER_START = "# >>> cobweaverclaw_compress persistent env >>>"
_ENV_MARKER_END = "# <<< cobweaverclaw_compress persistent env <<<"
    ...
    for name, value in merged.items():
        previous = run(
            [
                ...
                "if ($null -eq $value) { '__COBWEAVERCLAW_UNSET__' } else { $value }",
            ],
            capture_output=True,
            text=True,
            check=True,
        ...
        mutations.append(
            ManagedMutation(
                ...
                data={
                    "name": name,
                    "scope": scope_name,
                    "previous": None if previous == "__COBWEAVERCLAW_UNSET__" else previous,
                },
            )
        )
    return mutations
