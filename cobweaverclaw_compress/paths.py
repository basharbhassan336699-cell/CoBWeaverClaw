"""Canonical filesystem contract for CoBWeaverClaw.

This module defines the single source of truth for where CoBWeaverClaw reads and
writes files. It introduces two canonical roots:

* ``COBWEAVERCLAW_CONFIG_DIR`` -- read-mostly configuration (defaults to
  ``~/.cobweaverclaw_compress/config``). Holds model catalogs, plugin settings, and other
  configuration that users or admins edit.
* ``COBWEAVERCLAW_WORKSPACE_DIR`` -- read-write state (defaults to ``~/.cobweaverclaw_compress``).
...
per-resource override (``COBWEAVERCLAW_SAVINGS_PATH``, ``COBWEAVERCLAW_TOIN_PATH``,
``COBWEAVERCLAW_SUBSCRIPTION_STATE_PATH``, ``COBWEAVERCLAW_MODEL_LIMITS``, ...)
...
COBWEAVERCLAW_CONFIG_DIR_ENV = "COBWEAVERCLAW_CONFIG_DIR"
COBWEAVERCLAW_WORKSPACE_DIR_ENV = "COBWEAVERCLAW_WORKSPACE_DIR"

# ---------------------------------------------------------------------------
# Legacy per-resource env vars (kept for backward compatibility)
# ---------------------------------------------------------------------------

COBWEAVERCLAW_SAVINGS_PATH_ENV = "COBWEAVERCLAW_SAVINGS_PATH"
COBWEAVERCLAW_SAVINGS_EVENTS_PATH_ENV = "COBWEAVERCLAW_SAVINGS_EVENTS_PATH"
COBWEAVERCLAW_TOIN_PATH_ENV = "COBWEAVERCLAW_TOIN_PATH"
COBWEAVERCLAW_SUBSCRIPTION_STATE_PATH_ENV = "COBWEAVERCLAW_SUBSCRIPTION_STATE_PATH"

# ---------------------------------------------------------------------------
# Default sub-path fragments
# ---------------------------------------------------------------------------

_WORKSPACE_DIR_DEFAULT = ".cobweaverclaw_compress"
...
def process_is_stateless() -> bool:
    ...
    True if ``set_process_stateless(True)`` was called OR the ``COBWEAVERCLAW_STATELESS``
    environment variable is set, so non-proxy entrypoints honor it too.
    """

    if _PROCESS_STATELESS:
        return True
    return _env("COBWEAVERCLAW_STATELESS").lower() in ("1", "true", "yes", "on")
...
def workspace_dir() -> Path:
    ...
    1. ``$COBWEAVERCLAW_WORKSPACE_DIR`` (trimmed, tilde-expanded) if set.
    2. ``~/.cobweaverclaw_compress`` otherwise.
    """

    env_value = _env(COBWEAVERCLAW_WORKSPACE_DIR_ENV)
    if env_value:
        return Path(env_value).expanduser()
...
def config_dir() -> Path:
    ...
    1. ``$COBWEAVERCLAW_CONFIG_DIR`` (trimmed, tilde-expanded) if set.
    2. ``$COBWEAVERCLAW_WORKSPACE_DIR/config`` when the workspace env var is set
       so that a single override relocates both roots coherently.
    3. ``~/.cobweaverclaw_compress/config`` otherwise.
    """

    env_value = _env(COBWEAVERCLAW_CONFIG_DIR_ENV)
    if env_value:
        return Path(env_value).expanduser()
    workspace_env = _env(COBWEAVERCLAW_WORKSPACE_DIR_ENV)
    if workspace_env:
...
def savings_path(explicit: str | os.PathLike[str] | None = None) -> Path:
    ...
    return _resolve(
        explicit,
        COBWEAVERCLAW_SAVINGS_PATH_ENV,
...
def toin_path(explicit: str | os.PathLike[str] | None = None) -> Path:
    ...
    ``~/.cobweaverclaw_compress/toin.json`` to preserve byte-for-byte backward compat.
    """

    return _resolve(
        explicit,
        COBWEAVERCLAW_TOIN_PATH_ENV,
...
def subscription_state_path(explicit: str | os.PathLike[str] | None = None) -> Path:
    ...
    return _resolve(
        explicit,
        COBWEAVERCLAW_SUBSCRIPTION_STATE_PATH_ENV,
...
def savings_events_path(explicit: str | os.PathLike[str] | None = None) -> Path:
    ...
    MCP processes, and is the source of truth for ``cobweaverclaw_compress savings``.
    """

    return _resolve(
        explicit,
        COBWEAVERCLAW_SAVINGS_EVENTS_PATH_ENV,
...
def log_dir() -> Path:
    """Return the directory for CoBWeaverClaw log files."""
...
def bin_dir() -> Path:
    """Return the directory where CoBWeaverClaw ships vendored binaries."""
...
def models_config_path() -> Path:
    ...
    Note: the ``COBWEAVERCLAW_MODEL_LIMITS`` env var is a *content* override
    ...
    location and deliberately ignores ``COBWEAVERCLAW_MODEL_LIMITS``.
    """
...
__all__ = [
    "COBWEAVERCLAW_CONFIG_DIR_ENV",
    "COBWEAVERCLAW_WORKSPACE_DIR_ENV",
    "COBWEAVERCLAW_SAVINGS_PATH_ENV",
    "COBWEAVERCLAW_SAVINGS_EVENTS_PATH_ENV",
    "COBWEAVERCLAW_TOIN_PATH_ENV",
    "COBWEAVERCLAW_SUBSCRIPTION_STATE_PATH_ENV",
