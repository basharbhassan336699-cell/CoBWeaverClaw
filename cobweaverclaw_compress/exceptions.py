"""Custom exceptions for CoBWeaverClaw.

This module provides explicit exception classes for better error handling
and debugging. All exceptions inherit from CoBWeaverClawError, making it easy
to catch all CoBWeaverClaw-related errors.

Example:
    from cobweaverclaw_compress import CoBWeaverClawClient, CoBWeaverClawError, ConfigurationError
...
class CoBWeaverClawError(Exception):
    """Base exception for all CoBWeaverClaw errors.

    All CoBWeaverClaw exceptions inherit from this class, making it easy
    to catch any CoBWeaverClaw-related error:

        try:
            client.chat.completions.create(...)
        except CoBWeaverClawError as e:
            # Handle any CoBWeaverClaw error
            pass
    """
...
class ConfigurationError(CoBWeaverClawError):
    """Raised when CoBWeaverClaw is misconfigured.
...
class ProviderError(CoBWeaverClawError):
...
class StorageError(CoBWeaverClawError):
...
class CompressionError(CoBWeaverClawError):
...
class TokenizationError(CoBWeaverClawError):
...
class CacheError(CoBWeaverClawError):
...
class ValidationError(CoBWeaverClawError):
...
class TransformError(CoBWeaverClawError):
